
#include "header.h"


static const int32_t num_class[] = {  6, };

int32_t get_num_target(void) {
  return N_TARGET;
}
void get_num_class(int32_t* out) {
  for (int i = 0; i < N_TARGET; ++i) {
    out[i] = num_class[i];
  }
}
int32_t get_num_feature(void) {
  return 36;
}
const char* get_threshold_type(void) {
  return "float32";
}
const char* get_leaf_output_type(void) {
  return "float32";
}


void predict(union Entry* data, int pred_margin, float* result) {
  unsigned int tmp;
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)49.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)57.5) ) ) {
            result[0] += 0;
            result[1] += 0.375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.625;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
            result[0] += 0.0038314176245210726;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9846743295019157;
            result[5] += 0.011494252873563218;
          } else {
            result[0] += 0;
            result[1] += 0.08163265306122448;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6326530612244898;
            result[5] += 0.2857142857142857;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)58) ) ) {
            result[0] += 0.4;
            result[1] += 0;
            result[2] += 0.03333333333333333;
            result[3] += 0;
            result[4] += 0.43333333333333335;
            result[5] += 0.13333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.04983748645720477;
            result[4] += 0.02275189599133261;
            result[5] += 0.9274106175514626;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
            result[0] += 0.036585365853658534;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.10975609756097561;
            result[4] += 0.04878048780487805;
            result[5] += 0.8048780487804879;
          } else {
            result[0] += 0.03673469387755103;
            result[1] += 0.028571428571428574;
            result[2] += 0.06938775510204083;
            result[3] += 0.526530612244898;
            result[4] += 0.016326530612244903;
            result[5] += 0.32244897959183677;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)88.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0.6666666666666667;
            result[2] += 0;
            result[3] += 0.11111111111111112;
            result[4] += 0.22222222222222224;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)68.5) ) ) {
            result[0] += 0.15625;
            result[1] += 0.2;
            result[2] += 0;
            result[3] += 0.04375;
            result[4] += 0.575;
            result[5] += 0.025;
          } else {
            result[0] += 0.8578391551584079;
            result[1] += 0.02437043054427295;
            result[2] += 0.022745735174654756;
            result[3] += 0.019496344435418363;
            result[4] += 0.043054427294882215;
            result[5] += 0.032493907392363935;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
            result[0] += 0.3008130081300813;
            result[1] += 0.024390243902439025;
            result[2] += 0.008130081300813009;
            result[3] += 0.08130081300813008;
            result[4] += 0.37398373983739835;
            result[5] += 0.21138211382113822;
          } else {
            result[0] += 0.046511627906976744;
            result[1] += 0.003875968992248062;
            result[2] += 0.2751937984496124;
            result[3] += 0.44573643410852715;
            result[4] += 0.011627906976744186;
            result[5] += 0.21705426356589147;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.36363636363636365;
            result[4] += 0;
            result[5] += 0.6363636363636364;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.2857142857142857;
            result[4] += 0;
            result[5] += 0.7142857142857143;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)99.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.26865671641791045;
            result[3] += 0.44776119402985076;
            result[4] += 0.04477611940298507;
            result[5] += 0.23880597014925373;
          } else {
            result[0] += 0;
            result[1] += 0.023809523809523808;
            result[2] += 0.8333333333333334;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0.09523809523809523;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)87) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.13333333333333333;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7604166666666667;
            result[3] += 0.22916666666666669;
            result[4] += 0;
            result[5] += 0.010416666666666668;
          } else {
            result[0] += 0.01875;
            result[1] += 0;
            result[2] += 0.89375;
            result[3] += 0.0875;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)79) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9136690647482014;
            result[3] += 0.08633093525179857;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)93.5) ) ) {
            result[0] += 0.2857142857142857;
            result[1] += 0;
            result[2] += 0.5714285714285714;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.14285714285714285;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9888888888888889;
            result[3] += 0.011111111111111112;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)114.5) ) ) {
            result[0] += 0.0038314176245210726;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9655172413793104;
            result[5] += 0.03065134099616858;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0;
            result[4] += 0.8333333333333334;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.13333333333333333;
            result[4] += 0.2;
            result[5] += 0.6666666666666666;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)59.5) ) ) {
            result[0] += 0.5256410256410257;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.38461538461538464;
            result[5] += 0.08974358974358974;
          } else {
            result[0] += 0;
            result[1] += 0.003080082135523614;
            result[2] += 0;
            result[3] += 0.04620123203285421;
            result[4] += 0.045174537987679675;
            result[5] += 0.9055441478439425;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.01904761904761905;
            result[3] += 0.29523809523809524;
            result[4] += 0.0380952380952381;
            result[5] += 0.6476190476190476;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8732394366197183;
            result[4] += 0;
            result[5] += 0.1267605633802817;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)70.5) ) ) {
            result[0] += 0.12280701754385964;
            result[1] += 0.38596491228070173;
            result[2] += 0;
            result[3] += 0.0043859649122807015;
            result[4] += 0.47368421052631576;
            result[5] += 0.013157894736842105;
          } else {
            result[0] += 0.8818681318681318;
            result[1] += 0.016483516483516484;
            result[2] += 0.0018315018315018315;
            result[3] += 0.0173992673992674;
            result[4] += 0.06684981684981685;
            result[5] += 0.015567765567765568;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)89.5) ) ) {
            result[0] += 0.03592814371257485;
            result[1] += 0.005988023952095809;
            result[2] += 0.3023952095808383;
            result[3] += 0.344311377245509;
            result[4] += 0.014970059880239521;
            result[5] += 0.2964071856287425;
          } else {
            result[0] += 0.7532467532467533;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.07792207792207792;
            result[4] += 0.15584415584415584;
            result[5] += 0.012987012987012988;
          }
        }
      } else {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)88.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)98.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0.029411764705882353;
            result[2] += 0.058823529411764705;
            result[3] += 0.17647058823529413;
            result[4] += 0.029411764705882353;
            result[5] += 0.7058823529411765;
          } else {
            result[0] += 0.4;
            result[1] += 0;
            result[2] += 0.6;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05172413793103448;
            result[3] += 0.7931034482758621;
            result[4] += 0;
            result[5] += 0.15517241379310345;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.17647058823529413;
            result[3] += 0.7058823529411765;
            result[4] += 0;
            result[5] += 0.11764705882352941;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8;
            result[3] += 0.16923076923076924;
            result[4] += 0;
            result[5] += 0.03076923076923077;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)95) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5333333333333333;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0.06666666666666667;
          } else {
            result[0] += 0.045454545454545456;
            result[1] += 0;
            result[2] += 0.8636363636363636;
            result[3] += 0.09090909090909091;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)123) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9243243243243243;
            result[3] += 0.07027027027027027;
            result[4] += 0;
            result[5] += 0.005405405405405406;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)86.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0018867924528301887;
            result[1] += 0;
            result[2] += 0.9830188679245283;
            result[3] += 0.01509433962264151;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)113.5) ) ) {
            result[0] += 0.004081632653061225;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9673469387755103;
            result[5] += 0.02857142857142857;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)58) ) ) {
            result[0] += 0.9880952380952381;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.011904761904761904;
            result[5] += 0;
          } else {
            result[0] += 0.053763440860215055;
            result[1] += 0.043010752688172046;
            result[2] += 0;
            result[3] += 0.07526881720430108;
            result[4] += 0.41935483870967744;
            result[5] += 0.40860215053763443;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0.03225806451612903;
            result[2] += 0;
            result[3] += 0.04838709677419355;
            result[4] += 0.7096774193548387;
            result[5] += 0.20967741935483872;
          } else {
            result[0] += 0.003416856492027335;
            result[1] += 0;
            result[2] += 0.0011389521640091116;
            result[3] += 0.03302961275626424;
            result[4] += 0.05922551252847381;
            result[5] += 0.9031890660592256;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)70.5) ) ) {
            result[0] += 0.07142857142857142;
            result[1] += 0;
            result[2] += 0.03571428571428571;
            result[3] += 0.24107142857142858;
            result[4] += 0;
            result[5] += 0.6517857142857143;
          } else {
            result[0] += 0;
            result[1] += 0.006269592476489028;
            result[2] += 0.006269592476489028;
            result[3] += 0.5987460815047022;
            result[4] += 0;
            result[5] += 0.3887147335423197;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)50) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)82.5) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9907407407407407;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.009259259259259259;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)102) ) ) {
            result[0] += 0.013513513513513514;
            result[1] += 0.02702702702702703;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9324324324324325;
            result[5] += 0.02702702702702703;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)70.5) ) ) {
            result[0] += 0.1527777777777778;
            result[1] += 0.2222222222222222;
            result[2] += 0;
            result[3] += 0.05555555555555555;
            result[4] += 0.4861111111111111;
            result[5] += 0.08333333333333333;
          } else {
            result[0] += 0.8875899280575541;
            result[1] += 0.006294964028776979;
            result[2] += 0.006294964028776979;
            result[3] += 0.026079136690647486;
            result[4] += 0.055755395683453245;
            result[5] += 0.01798561151079137;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)93) ) ) {
            result[0] += 0.019417475728155338;
            result[1] += 0.009708737864077669;
            result[2] += 0.5048543689320388;
            result[3] += 0.25728155339805825;
            result[4] += 0.009708737864077669;
            result[5] += 0.19902912621359223;
          } else {
            result[0] += 0.7586206896551725;
            result[1] += 0;
            result[2] += 0.20689655172413796;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.03448275862068966;
          }
        }
      }
    }
  } else {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)86) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09090909090909091;
            result[3] += 0.6363636363636364;
            result[4] += 0;
            result[5] += 0.2727272727272727;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.037037037037037035;
            result[3] += 0.037037037037037035;
            result[4] += 0.037037037037037035;
            result[5] += 0.8888888888888888;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)98) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8918918918918919;
            result[4] += 0;
            result[5] += 0.10810810810810811;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5384615384615384;
            result[3] += 0.3076923076923077;
            result[4] += 0;
            result[5] += 0.15384615384615385;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8444444444444444;
            result[3] += 0.13333333333333333;
            result[4] += 0;
            result[5] += 0.022222222222222223;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0.7777777777777778;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0.08888888888888889;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)94.5) ) ) {
            result[0] += 0.3333333333333333;
            result[1] += 0;
            result[2] += 0.5333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.13333333333333333;
          } else {
            result[0] += 0.014527845036319613;
            result[1] += 0;
            result[2] += 0.9346246973365617;
            result[3] += 0.04963680387409201;
            result[4] += 0;
            result[5] += 0.0012106537530266344;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)76) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.058823529411764705;
            result[4] += 0.47058823529411764;
            result[5] += 0.47058823529411764;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)59.5) ) ) {
            result[0] += 0.4146341463414634;
            result[1] += 0.17073170731707318;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1951219512195122;
            result[5] += 0.21951219512195122;
          } else {
            result[0] += 0;
            result[1] += 0.0012453300124533001;
            result[2] += 0.0024906600249066002;
            result[3] += 0.0547945205479452;
            result[4] += 0.038605230386052306;
            result[5] += 0.9028642590286425;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)84.5) ) ) {
            result[0] += 0.14233576642335766;
            result[1] += 0.032846715328467155;
            result[2] += 0.0036496350364963502;
            result[3] += 0.22992700729927007;
            result[4] += 0.072992700729927;
            result[5] += 0.5182481751824818;
          } else {
            result[0] += 0.015503875968992248;
            result[1] += 0.023255813953488372;
            result[2] += 0;
            result[3] += 0.7829457364341085;
            result[4] += 0.015503875968992248;
            result[5] += 0.16279069767441862;
          }
        }
      }
    } else {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)65) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)93) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)67) ) ) {
            result[0] += 0;
            result[1] += 0.10784313725490197;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8333333333333334;
            result[5] += 0.058823529411764705;
          } else {
            result[0] += 0.12;
            result[1] += 0.24;
            result[2] += 0;
            result[3] += 0.28;
            result[4] += 0.24;
            result[5] += 0.12;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)99.5) ) ) {
            result[0] += 0.07692307692307693;
            result[1] += 0.6538461538461539;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2692307692307692;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)63.5) ) ) {
            result[0] += 0.011363636363636364;
            result[1] += 0.4318181818181818;
            result[2] += 0;
            result[3] += 0.056818181818181816;
            result[4] += 0.4772727272727273;
            result[5] += 0.022727272727272728;
          } else {
            result[0] += 0.8598130841121495;
            result[1] += 0;
            result[2] += 0.0008496176720475786;
            result[3] += 0.03993203058623619;
            result[4] += 0.059473237043330504;
            result[5] += 0.03993203058623619;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)101.5) ) ) {
            result[0] += 0.03435114503816794;
            result[1] += 0.007633587786259542;
            result[2] += 0.20610687022900764;
            result[3] += 0.4083969465648855;
            result[4] += 0.05725190839694656;
            result[5] += 0.2862595419847328;
          } else {
            result[0] += 0.4090909090909091;
            result[1] += 0.006493506493506494;
            result[2] += 0.474025974025974;
            result[3] += 0.06493506493506493;
            result[4] += 0.01948051948051948;
            result[5] += 0.025974025974025976;
          }
        }
      }
    }
  } else {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)87) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.9;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16981132075471697;
            result[3] += 0.5660377358490566;
            result[4] += 0;
            result[5] += 0.2641509433962264;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.9;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.25;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)71) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)53.5) ) ) {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)99) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13333333333333333;
            result[3] += 0.6;
            result[4] += 0.06666666666666667;
            result[5] += 0.2;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)92) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09090909090909091;
            result[3] += 0.7272727272727273;
            result[4] += 0;
            result[5] += 0.18181818181818182;
          } else {
            result[0] += 0.006615214994487322;
            result[1] += 0.0011025358324145537;
            result[2] += 0.926130099228225;
            result[3] += 0.06063947078280045;
            result[4] += 0;
            result[5] += 0.005512679162072768;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)94.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)101) ) ) {
            result[0] += 0.015810276679841896;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8972332015810277;
            result[5] += 0.08695652173913043;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)58) ) ) {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0.0025806451612903226;
            result[1] += 0;
            result[2] += 0.003870967741935484;
            result[3] += 0.03741935483870968;
            result[4] += 0.02064516129032258;
            result[5] += 0.9354838709677419;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)87.5) ) ) {
            result[0] += 0.0611353711790393;
            result[1] += 0.034934497816593885;
            result[2] += 0.004366812227074236;
            result[3] += 0.1965065502183406;
            result[4] += 0.07860262008733625;
            result[5] += 0.6244541484716157;
          } else {
            result[0] += 0.060000000000000005;
            result[1] += 0.020000000000000004;
            result[2] += 0.020000000000000004;
            result[3] += 0.7000000000000001;
            result[4] += 0.03333333333333334;
            result[5] += 0.16666666666666669;
          }
        }
      }
    } else {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)38) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0457516339869281;
            result[1] += 0.09803921568627451;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8366013071895425;
            result[5] += 0.0196078431372549;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
            result[0] += 0.8253839935327405;
            result[1] += 0.012934518997574777;
            result[2] += 0.002425222312045271;
            result[3] += 0.028294260307194827;
            result[4] += 0.07679870654810024;
            result[5] += 0.054163298302344384;
          } else {
            result[0] += 0.23464912280701755;
            result[1] += 0.013157894736842105;
            result[2] += 0.19736842105263158;
            result[3] += 0.23684210526315788;
            result[4] += 0.03289473684210526;
            result[5] += 0.2850877192982456;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)63.5) ) ) {
            result[0] += 0.05405405405405406;
            result[1] += 0.8648648648648649;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.08108108108108109;
            result[5] += 0;
          } else {
            result[0] += 0.375;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  } else {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)101) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.2727272727272727;
            result[4] += 0;
            result[5] += 0.7272727272727273;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2711864406779661;
            result[3] += 0.6101694915254238;
            result[4] += 0;
            result[5] += 0.11864406779661017;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6949152542372882;
            result[3] += 0.2033898305084746;
            result[4] += 0;
            result[5] += 0.1016949152542373;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4375;
            result[3] += 0.5625;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8823529411764706;
            result[3] += 0.11764705882352941;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)107.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)89.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)121) ) ) {
            result[0] += 0.014373716632443531;
            result[1] += 0;
            result[2] += 0.891170431211499;
            result[3] += 0.09034907597535935;
            result[4] += 0;
            result[5] += 0.004106776180698152;
          } else {
            result[0] += 0.7777777777777778;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73.5) ) ) {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)81.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9906832298136646;
            result[3] += 0.009316770186335404;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)93.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)102) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)97) ) ) {
            result[0] += 0.0031645569620253164;
            result[1] += 0.0031645569620253164;
            result[2] += 0;
            result[3] += 0.0031645569620253164;
            result[4] += 0.9746835443037974;
            result[5] += 0.015822784810126583;
          } else {
            result[0] += 0;
            result[1] += 0.7777777777777778;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2222222222222222;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)52.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0.997624703087886;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0023752969121140144;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)51.5) ) ) {
            result[0] += 0.3;
            result[1] += 0.7;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.13043478260869565;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8695652173913043;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)86) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)56.5) ) ) {
            result[0] += 0.6444444444444445;
            result[1] += 0.022222222222222223;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3111111111111111;
            result[5] += 0.022222222222222223;
          } else {
            result[0] += 0.0010183299389002036;
            result[1] += 0.010183299389002037;
            result[2] += 0;
            result[3] += 0.06517311608961303;
            result[4] += 0.051934826883910386;
            result[5] += 0.8716904276985743;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.037037037037037035;
            result[4] += 0.1111111111111111;
            result[5] += 0.8518518518518519;
          } else {
            result[0] += 0.0078125;
            result[1] += 0;
            result[2] += 0.03125;
            result[3] += 0.765625;
            result[4] += 0.03125;
            result[5] += 0.1640625;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)65) ) ) {
            result[0] += 0.13636363636363635;
            result[1] += 0.30303030303030304;
            result[2] += 0.030303030303030304;
            result[3] += 0;
            result[4] += 0.48484848484848486;
            result[5] += 0.045454545454545456;
          } else {
            result[0] += 0.8848594741613781;
            result[1] += 0.009972801450589302;
            result[2] += 0.004533091568449683;
            result[3] += 0.024478694469628286;
            result[4] += 0.04895738893925657;
            result[5] += 0.027198549410698096;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.04905660377358491;
            result[1] += 0.026415094339622643;
            result[2] += 0.03773584905660377;
            result[3] += 0.3018867924528302;
            result[4] += 0.12830188679245283;
            result[5] += 0.45660377358490567;
          } else {
            result[0] += 0.47619047619047616;
            result[1] += 0;
            result[2] += 0.4095238095238095;
            result[3] += 0.05714285714285714;
            result[4] += 0;
            result[5] += 0.05714285714285714;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)92.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)69.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.024096385542168676;
            result[2] += 0.03614457831325301;
            result[3] += 0.060240963855421686;
            result[4] += 0.03614457831325301;
            result[5] += 0.8433734939759037;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0.375;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1388888888888889;
            result[3] += 0.7037037037037037;
            result[4] += 0;
            result[5] += 0.1574074074074074;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)82) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)109.5) ) ) {
            result[0] += 0.5909090909090909;
            result[1] += 0;
            result[2] += 0.3181818181818182;
            result[3] += 0;
            result[4] += 0.09090909090909091;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09090909090909091;
            result[3] += 0.5454545454545454;
            result[4] += 0.045454545454545456;
            result[5] += 0.3181818181818182;
          } else {
            result[0] += 0.013513513513513516;
            result[1] += 0;
            result[2] += 0.6666666666666667;
            result[3] += 0.2792792792792793;
            result[4] += 0.004504504504504505;
            result[5] += 0.03603603603603604;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)86.5) ) ) {
            result[0] += 0.016194331983805668;
            result[1] += 0;
            result[2] += 0.8785425101214575;
            result[3] += 0.0931174089068826;
            result[4] += 0;
            result[5] += 0.012145748987854251;
          } else {
            result[0] += 0.0037243947858473;
            result[1] += 0;
            result[2] += 0.9720670391061452;
            result[3] += 0.0223463687150838;
            result[4] += 0;
            result[5] += 0.00186219739292365;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)44) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6363636363636364;
            result[5] += 0.36363636363636365;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9832635983263598;
            result[5] += 0.016736401673640166;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.058823529411764705;
            result[4] += 0.7647058823529411;
            result[5] += 0.17647058823529413;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.026352288488210817;
            result[4] += 0.006934812760055479;
            result[5] += 0.9667128987517337;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
            result[0] += 0.6923076923076923;
            result[1] += 0.028846153846153848;
            result[2] += 0;
            result[3] += 0.0673076923076923;
            result[4] += 0.15384615384615385;
            result[5] += 0.057692307692307696;
          } else {
            result[0] += 0.01768172888015717;
            result[1] += 0.009823182711198428;
            result[2] += 0.007858546168958742;
            result[3] += 0.4086444007858546;
            result[4] += 0.060903732809430254;
            result[5] += 0.4950884086444008;
          }
        }
      }
    } else {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)56.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)48.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0.5714285714285714;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.42857142857142855;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0.125;
            result[2] += 0.03125;
            result[3] += 0.03125;
            result[4] += 0.75;
            result[5] += 0.0625;
          } else {
            result[0] += 0.11363636363636363;
            result[1] += 0.6363636363636364;
            result[2] += 0.20454545454545456;
            result[3] += 0;
            result[4] += 0.045454545454545456;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)66.5) ) ) {
            result[0] += 0.14507772020725387;
            result[1] += 0.3316062176165803;
            result[2] += 0;
            result[3] += 0.025906735751295335;
            result[4] += 0.46632124352331605;
            result[5] += 0.031088082901554404;
          } else {
            result[0] += 0.8845120859444943;
            result[1] += 0.0044762757385854975;
            result[2] += 0.010743061772605195;
            result[3] += 0.03133393017009849;
            result[4] += 0.05013428827215757;
            result[5] += 0.01880035810205909;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)111.5) ) ) {
            result[0] += 0.024291497975708502;
            result[1] += 0;
            result[2] += 0.3643724696356275;
            result[3] += 0.3643724696356275;
            result[4] += 0.02834008097165992;
            result[5] += 0.21862348178137653;
          } else {
            result[0] += 0.6625;
            result[1] += 0;
            result[2] += 0.05;
            result[3] += 0.05;
            result[4] += 0.1625;
            result[5] += 0.075;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)86) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)66.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0.07142857142857142;
            result[4] += 0;
            result[5] += 0.7857142857142857;
          } else {
            result[0] += 0.1;
            result[1] += 0;
            result[2] += 0.26666666666666666;
            result[3] += 0.4;
            result[4] += 0.03333333333333333;
            result[5] += 0.2;
          }
        }
      } else {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)95.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.19298245614035087;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0.14035087719298245;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.125;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)98) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)114.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.875;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.03333333333333333;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0.25;
            result[4] += 0.016666666666666666;
            result[5] += 0.03333333333333333;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)83) ) ) {
            result[0] += 0.6666666666666667;
            result[1] += 0;
            result[2] += 0.11111111111111112;
            result[3] += 0.22222222222222224;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)77) ) ) {
            result[0] += 0.7142857142857143;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0037174721189591076;
            result[1] += 0;
            result[2] += 0.9479553903345725;
            result[3] += 0.04584882280049566;
            result[4] += 0;
            result[5] += 0.0024783147459727386;
          }
        }
      }
    }
  }
  if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)48.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.008771929824561403;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9824561403508771;
            result[5] += 0.008771929824561403;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)89) ) ) {
            result[0] += 0.04081632653061224;
            result[1] += 0.02040816326530612;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5102040816326531;
            result[5] += 0.42857142857142855;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)52.5) ) ) {
            result[0] += 0.9411764705882353;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.058823529411764705;
            result[5] += 0;
          } else {
            result[0] += 0.0048543689320388345;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.044902912621359224;
            result[4] += 0.044902912621359224;
            result[5] += 0.9053398058252428;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)83.5) ) ) {
            result[0] += 0.10256410256410256;
            result[1] += 0.08333333333333333;
            result[2] += 0;
            result[3] += 0.16025641025641027;
            result[4] += 0.03205128205128205;
            result[5] += 0.6217948717948718;
          } else {
            result[0] += 0.0446927374301676;
            result[1] += 0.00558659217877095;
            result[2] += 0.0111731843575419;
            result[3] += 0.6536312849162011;
            result[4] += 0.00558659217877095;
            result[5] += 0.27932960893854747;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)71.5) ) ) {
            result[0] += 0.24742268041237114;
            result[1] += 0.13917525773195877;
            result[2] += 0.005154639175257732;
            result[3] += 0.06701030927835051;
            result[4] += 0.5309278350515464;
            result[5] += 0.010309278350515464;
          } else {
            result[0] += 0.8960674157303371;
            result[1] += 0.0056179775280898875;
            result[2] += 0.0018726591760299626;
            result[3] += 0.02247191011235955;
            result[4] += 0.060861423220973786;
            result[5] += 0.013108614232209739;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)110.5) ) ) {
            result[0] += 0.06415094339622641;
            result[1] += 0.018867924528301886;
            result[2] += 0.18867924528301888;
            result[3] += 0.23018867924528302;
            result[4] += 0.07924528301886792;
            result[5] += 0.4188679245283019;
          } else {
            result[0] += 0.8999999999999998;
            result[1] += 0.016666666666666663;
            result[2] += 0.016666666666666663;
            result[3] += 0;
            result[4] += 0.06666666666666665;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8;
            result[5] += 0.2;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03125;
            result[4] += 0.0625;
            result[5] += 0.90625;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0.13333333333333333;
            result[2] += 0.06666666666666667;
            result[3] += 0.4666666666666667;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.017241379310344827;
            result[3] += 0.9482758620689655;
            result[4] += 0;
            result[5] += 0.034482758620689655;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.26666666666666666;
            result[4] += 0.03333333333333333;
            result[5] += 0.7;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2926829268292683;
            result[3] += 0.5365853658536586;
            result[4] += 0;
            result[5] += 0.17073170731707318;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)75.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.45454545454545453;
            result[4] += 0.09090909090909091;
            result[5] += 0.45454545454545453;
          } else {
            result[0] += 0.08695652173913043;
            result[1] += 0.17391304347826086;
            result[2] += 0.6086956521739131;
            result[3] += 0.13043478260869565;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)89) ) ) {
            result[0] += 0;
            result[1] += 0.4;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0.4;
            result[5] += 0;
          } else {
            result[0] += 0.9545454545454546;
            result[1] += 0;
            result[2] += 0.045454545454545456;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0.008547008547008548;
            result[2] += 0.5299145299145299;
            result[3] += 0.37606837606837606;
            result[4] += 0;
            result[5] += 0.08547008547008547;
          } else {
            result[0] += 0.011278195488721804;
            result[1] += 0.007518796992481203;
            result[2] += 0.8609022556390977;
            result[3] += 0.09398496240601503;
            result[4] += 0.007518796992481203;
            result[5] += 0.018796992481203006;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9155844155844156;
            result[3] += 0.08441558441558442;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9936974789915967;
            result[3] += 0.0063025210084033615;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
    if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)94.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.025;
            result[3] += 0;
            result[4] += 0.875;
            result[5] += 0.1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0.6666666666666666;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)65) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)66.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)60.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
            result[0] += 0.0016207455429497568;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.006482982171799027;
            result[4] += 0.019448946515397084;
            result[5] += 0.9724473257698542;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.017241379310344827;
            result[3] += 0.22413793103448276;
            result[4] += 0.02586206896551724;
            result[5] += 0.7327586206896551;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)56.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)59.5) ) ) {
            result[0] += 0.94;
            result[1] += 0;
            result[2] += 0.02;
            result[3] += 0;
            result[4] += 0.04;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)85.5) ) ) {
            result[0] += 0.004629629629629629;
            result[1] += 0;
            result[2] += 0.009259259259259259;
            result[3] += 0.08333333333333333;
            result[4] += 0.1527777777777778;
            result[5] += 0.75;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.011278195488721804;
            result[3] += 0.5902255639097744;
            result[4] += 0.03007518796992481;
            result[5] += 0.3684210526315789;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)93) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)44.5) ) ) {
            result[0] += 0.08;
            result[1] += 0.8;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.12;
            result[5] += 0;
          } else {
            result[0] += 0.020618556701030927;
            result[1] += 0.17525773195876287;
            result[2] += 0;
            result[3] += 0.030927835051546393;
            result[4] += 0.7525773195876289;
            result[5] += 0.020618556701030927;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0.8461538461538461;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.15384615384615385;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
            result[0] += 0.10000000000000002;
            result[1] += 0.4428571428571429;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4428571428571429;
            result[5] += 0.014285714285714287;
          } else {
            result[0] += 0.9057093425605537;
            result[1] += 0.005190311418685121;
            result[2] += 0.006055363321799308;
            result[3] += 0.014705882352941176;
            result[4] += 0.05536332179930796;
            result[5] += 0.012975778546712802;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)93) ) ) {
            result[0] += 0.026041666666666668;
            result[1] += 0.08333333333333333;
            result[2] += 0.078125;
            result[3] += 0.359375;
            result[4] += 0.036458333333333336;
            result[5] += 0.4166666666666667;
          } else {
            result[0] += 0.7333333333333334;
            result[1] += 0;
            result[2] += 0.13333333333333336;
            result[3] += 0;
            result[4] += 0.12000000000000001;
            result[5] += 0.013333333333333336;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)95.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)68.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1774193548387097;
            result[3] += 0.5887096774193549;
            result[4] += 0;
            result[5] += 0.23387096774193547;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.14285714285714285;
            result[4] += 0.14285714285714285;
            result[5] += 0.7142857142857143;
          } else {
            result[0] += 0.03225806451612903;
            result[1] += 0;
            result[2] += 0.6989247311827957;
            result[3] += 0.1935483870967742;
            result[4] += 0;
            result[5] += 0.07526881720430108;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.85;
            result[4] += 0;
            result[5] += 0.05;
          } else {
            result[0] += 0.014619883040935672;
            result[1] += 0;
            result[2] += 0.7894736842105263;
            result[3] += 0.16374269005847952;
            result[4] += 0;
            result[5] += 0.03216374269005848;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
            result[0] += 0.36363636363636365;
            result[1] += 0;
            result[2] += 0.5454545454545454;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.09090909090909091;
          } else {
            result[0] += 0.0016722408026755853;
            result[1] += 0;
            result[2] += 0.9632107023411371;
            result[3] += 0.03511705685618729;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0.004;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.984;
            result[5] += 0.012;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.6666666666666666;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.75;
            result[5] += 0.25;
          } else {
            result[0] += 0.002898550724637681;
            result[1] += 0;
            result[2] += 0.0014492753623188406;
            result[3] += 0.021739130434782608;
            result[4] += 0.02463768115942029;
            result[5] += 0.9492753623188406;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)58) ) ) {
            result[0] += 0.8275862068965517;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1724137931034483;
            result[5] += 0;
          } else {
            result[0] += 0.004761904761904762;
            result[1] += 0.009523809523809525;
            result[2] += 0.006349206349206349;
            result[3] += 0.37142857142857144;
            result[4] += 0.09047619047619047;
            result[5] += 0.5174603174603175;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)65) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)50) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)109) ) ) {
            result[0] += 0.03614457831325301;
            result[1] += 0.1927710843373494;
            result[2] += 0;
            result[3] += 0.024096385542168676;
            result[4] += 0.7349397590361446;
            result[5] += 0.012048192771084338;
          } else {
            result[0] += 0.03125;
            result[1] += 0.90625;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0625;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)69.5) ) ) {
            result[0] += 0.13043478260869565;
            result[1] += 0.30434782608695654;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5652173913043478;
            result[5] += 0;
          } else {
            result[0] += 0.9030710172744721;
            result[1] += 0.008637236084452975;
            result[2] += 0.007677543186180422;
            result[3] += 0.019193857965451054;
            result[4] += 0.046065259117082535;
            result[5] += 0.015355086372360844;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89) ) ) {
            result[0] += 0.0392156862745098;
            result[1] += 0.0196078431372549;
            result[2] += 0.28627450980392155;
            result[3] += 0.43137254901960786;
            result[4] += 0.00784313725490196;
            result[5] += 0.21568627450980393;
          } else {
            result[0] += 0.7256637168141593;
            result[1] += 0;
            result[2] += 0.1592920353982301;
            result[3] += 0;
            result[4] += 0.11504424778761062;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)52.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04;
            result[3] += 0.6;
            result[4] += 0.08;
            result[5] += 0.28;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10638297872340426;
            result[3] += 0.0851063829787234;
            result[4] += 0.02127659574468085;
            result[5] += 0.7872340425531915;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)125) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.896551724137931;
            result[3] += 0.06896551724137931;
            result[4] += 0;
            result[5] += 0.034482758620689655;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.17391304347826086;
            result[3] += 0.8260869565217391;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6923076923076923;
            result[3] += 0.3076923076923077;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.375;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8461538461538461;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.028846153846153848;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)96.5) ) ) {
            result[0] += 0.009523809523809525;
            result[1] += 0;
            result[2] += 0.8761904761904762;
            result[3] += 0.09523809523809523;
            result[4] += 0;
            result[5] += 0.01904761904761905;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.918918918918919;
            result[3] += 0.08108108108108109;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9896265560165975;
            result[3] += 0.01037344398340249;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)81) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)65) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)63.5) ) ) {
            result[0] += 0.004524886877828056;
            result[1] += 0.009049773755656111;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9728506787330318;
            result[5] += 0.013574660633484165;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4;
            result[5] += 0.6;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)56.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.75;
            result[5] += 0.25;
          } else {
            result[0] += 0.001336898395721925;
            result[1] += 0;
            result[2] += 0.00267379679144385;
            result[3] += 0.034759358288770054;
            result[4] += 0.02540106951871658;
            result[5] += 0.9358288770053476;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)63.5) ) ) {
            result[0] += 0.6250000000000001;
            result[1] += 0.06818181818181819;
            result[2] += 0;
            result[3] += 0.011363636363636366;
            result[4] += 0.25000000000000006;
            result[5] += 0.04545454545454546;
          } else {
            result[0] += 0.005952380952380953;
            result[1] += 0.029761904761904764;
            result[2] += 0.033730158730158735;
            result[3] += 0.34920634920634924;
            result[4] += 0.0376984126984127;
            result[5] += 0.5436507936507937;
          }
        }
      }
    } else {
      if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)49.5) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)94.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0.25;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.75;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0.9885057471264368;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.011494252873563218;
            result[5] += 0;
          } else {
            result[0] += 0.5;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)70.5) ) ) {
            result[0] += 0.08928571428571429;
            result[1] += 0.39285714285714285;
            result[2] += 0.0035714285714285713;
            result[3] += 0.06785714285714285;
            result[4] += 0.4142857142857143;
            result[5] += 0.03214285714285714;
          } else {
            result[0] += 0.8784140969162996;
            result[1] += 0.007929515418502203;
            result[2] += 0.015859030837004406;
            result[3] += 0.02026431718061674;
            result[4] += 0.06343612334801763;
            result[5] += 0.014096916299559472;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)113.5) ) ) {
            result[0] += 0.03814713896457766;
            result[1] += 0.01907356948228883;
            result[2] += 0.2888283378746594;
            result[3] += 0.3242506811989101;
            result[4] += 0.04904632152588556;
            result[5] += 0.28065395095367845;
          } else {
            result[0] += 0.838709677419355;
            result[1] += 0.03225806451612904;
            result[2] += 0.048387096774193554;
            result[3] += 0;
            result[4] += 0.048387096774193554;
            result[5] += 0.03225806451612904;
          }
        }
      }
    }
  } else {
    if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)116.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.25;
            result[4] += 0.625;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.08108108108108109;
            result[3] += 0.08108108108108109;
            result[4] += 0;
            result[5] += 0.8378378378378378;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)99.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.20000000000000004;
            result[3] += 0.7571428571428572;
            result[4] += 0;
            result[5] += 0.042857142857142864;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6037735849056604;
            result[3] += 0.3584905660377358;
            result[4] += 0;
            result[5] += 0.03773584905660377;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.92;
            result[3] += 0.08;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)94.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)69) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)110.5) ) ) {
            result[0] += 0.125;
            result[1] += 0;
            result[2] += 0.875;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.2;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0.06666666666666667;
            result[5] += 0.4;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8611111111111112;
            result[3] += 0.13095238095238096;
            result[4] += 0;
            result[5] += 0.007936507936507936;
          } else {
            result[0] += 0.0020242914979757085;
            result[1] += 0;
            result[2] += 0.979757085020243;
            result[3] += 0.018218623481781375;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.024875621890547265;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9601990049751243;
            result[5] += 0.014925373134328358;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.023255813953488372;
            result[4] += 0.5116279069767442;
            result[5] += 0.46511627906976744;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)96) ) ) {
            result[0] += 0;
            result[1] += 0.28571428571428575;
            result[2] += 0;
            result[3] += 0.4285714285714286;
            result[4] += 0.28571428571428575;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)86) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
            result[0] += 0.7719298245614035;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.17543859649122806;
            result[5] += 0.05263157894736842;
          } else {
            result[0] += 0.005923000987166831;
            result[1] += 0.004935834155972359;
            result[2] += 0.0009871668311944718;
            result[3] += 0.07897334649555775;
            result[4] += 0.049358341559723594;
            result[5] += 0.859822309970385;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)72.5) ) ) {
            result[0] += 0.02857142857142857;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.2;
            result[4] += 0.02857142857142857;
            result[5] += 0.7428571428571429;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8504672897196262;
            result[4] += 0.009345794392523364;
            result[5] += 0.14018691588785046;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0.1414141414141414;
            result[2] += 0.020202020202020204;
            result[3] += 0;
            result[4] += 0.8080808080808081;
            result[5] += 0.030303030303030304;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0.00273224043715847;
            result[1] += 0.9972677595628415;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)70.5) ) ) {
            result[0] += 0.1833333333333333;
            result[1] += 0.19166666666666662;
            result[2] += 0.024999999999999994;
            result[3] += 0.016666666666666663;
            result[4] += 0.5499999999999999;
            result[5] += 0.033333333333333326;
          } else {
            result[0] += 0.8811715481171548;
            result[1] += 0.0041841004184100415;
            result[2] += 0.00502092050209205;
            result[3] += 0.04351464435146443;
            result[4] += 0.048535564853556486;
            result[5] += 0.017573221757322177;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0.030434782608695653;
            result[1] += 0.008695652173913044;
            result[2] += 0.08695652173913043;
            result[3] += 0.33043478260869563;
            result[4] += 0.08695652173913043;
            result[5] += 0.45652173913043476;
          } else {
            result[0] += 0.5483870967741936;
            result[1] += 0;
            result[2] += 0.33333333333333337;
            result[3] += 0.03225806451612904;
            result[4] += 0.06451612903225808;
            result[5] += 0.021505376344086027;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.07526881720430108;
            result[3] += 0.7419354838709677;
            result[4] += 0;
            result[5] += 0.1827956989247312;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.38;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0.22;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        result[0] += 0;
        result[1] += 0;
        result[2] += 1;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)99) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)98.5) ) ) {
            result[0] += 0.08823529411764706;
            result[1] += 0;
            result[2] += 0.14705882352941177;
            result[3] += 0.2647058823529412;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.625;
            result[3] += 0.375;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7829787234042553;
            result[3] += 0.1829787234042553;
            result[4] += 0.00851063829787234;
            result[5] += 0.02553191489361702;
          } else {
            result[0] += 0.6153846153846154;
            result[1] += 0;
            result[2] += 0.38461538461538464;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)94.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)84) ) ) {
            result[0] += 0.005434782608695652;
            result[1] += 0;
            result[2] += 0.8967391304347826;
            result[3] += 0.09239130434782608;
            result[4] += 0;
            result[5] += 0.005434782608695652;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9850746268656716;
            result[3] += 0.014925373134328358;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)97) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9612068965517241;
            result[5] += 0.03879310344827586;
          } else {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)42.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)86) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
            result[0] += 0.7333333333333333;
            result[1] += 0.06666666666666667;
            result[2] += 0;
            result[3] += 0.02857142857142857;
            result[4] += 0.09523809523809523;
            result[5] += 0.0761904761904762;
          } else {
            result[0] += 0.0019212295869356388;
            result[1] += 0.002881844380403458;
            result[2] += 0.004803073967339097;
            result[3] += 0.07204610951008646;
            result[4] += 0.04899135446685879;
            result[5] += 0.8693563880883766;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)75.5) ) ) {
            result[0] += 0.012422360248447204;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.782608695652174;
            result[4] += 0.031055900621118012;
            result[5] += 0.17391304347826086;
          } else {
            result[0] += 0.0625;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.325;
            result[4] += 0.0375;
            result[5] += 0.575;
          }
        }
      }
    } else {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)91) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.25;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0.25;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
            result[0] += 0.12315270935960591;
            result[1] += 0.2315270935960591;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6354679802955665;
            result[5] += 0.009852216748768473;
          } else {
            result[0] += 0.9055118110236221;
            result[1] += 0.0029527559055118114;
            result[2] += 0.011811023622047246;
            result[3] += 0.029527559055118113;
            result[4] += 0.04035433070866142;
            result[5] += 0.009842519685039372;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)111.5) ) ) {
            result[0] += 0.03715170278637771;
            result[1] += 0.015479876160990712;
            result[2] += 0.32507739938080493;
            result[3] += 0.25386996904024767;
            result[4] += 0.06501547987616099;
            result[5] += 0.30340557275541796;
          } else {
            result[0] += 0.8620689655172413;
            result[1] += 0;
            result[2] += 0.1206896551724138;
            result[3] += 0;
            result[4] += 0.017241379310344827;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)94.5) ) ) {
      if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)106.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.01492537313432836;
            result[2] += 0.08955223880597016;
            result[3] += 0.5671641791044777;
            result[4] += 0.02985074626865672;
            result[5] += 0.2985074626865672;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.17857142857142858;
            result[3] += 0.03571428571428571;
            result[4] += 0.07142857142857142;
            result[5] += 0.7142857142857143;
          } else {
            result[0] += 0.23076923076923078;
            result[1] += 0;
            result[2] += 0.3076923076923077;
            result[3] += 0.3076923076923077;
            result[4] += 0.15384615384615385;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)104.5) ) ) {
            result[0] += 0.07692307692307693;
            result[1] += 0;
            result[2] += 0.38461538461538464;
            result[3] += 0.46153846153846156;
            result[4] += 0;
            result[5] += 0.07692307692307693;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3111111111111111;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0.022222222222222223;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)102.5) ) ) {
            result[0] += 0.02631578947368421;
            result[1] += 0;
            result[2] += 0.8947368421052632;
            result[3] += 0.02631578947368421;
            result[4] += 0;
            result[5] += 0.05263157894736842;
          } else {
            result[0] += 0.5555555555555556;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0.2222222222222222;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)86.5) ) ) {
            result[0] += 0.009090909090909092;
            result[1] += 0;
            result[2] += 0.7000000000000001;
            result[3] += 0.28181818181818186;
            result[4] += 0;
            result[5] += 0.009090909090909092;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9182692307692307;
            result[3] += 0.07692307692307693;
            result[4] += 0;
            result[5] += 0.004807692307692308;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7692307692307693;
            result[3] += 0.23076923076923078;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9787610619469026;
            result[3] += 0.021238938053097345;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
    if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)81) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)94.5) ) ) {
            result[0] += 0.021739130434782608;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6304347826086957;
            result[5] += 0.34782608695652173;
          } else {
            result[0] += 0;
            result[1] += 0.2222222222222222;
            result[2] += 0;
            result[3] += 0.7777777777777778;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    } else {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)64.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)66) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)82) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)61.5) ) ) {
            result[0] += 0.014084507042253521;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.056338028169014086;
            result[4] += 0.352112676056338;
            result[5] += 0.5774647887323944;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.04277456647398844;
            result[4] += 0.03236994219653179;
            result[5] += 0.9248554913294798;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
            result[0] += 0.007407407407407408;
            result[1] += 0;
            result[2] += 0.022222222222222223;
            result[3] += 0.1259259259259259;
            result[4] += 0.022222222222222223;
            result[5] += 0.8222222222222222;
          } else {
            result[0] += 0.00900900900900901;
            result[1] += 0;
            result[2] += 0.01801801801801802;
            result[3] += 0.5855855855855857;
            result[4] += 0.004504504504504505;
            result[5] += 0.3828828828828829;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)65) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)50) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0.16666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8333333333333334;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)96.5) ) ) {
            result[0] += 0.034782608695652174;
            result[1] += 0.05217391304347826;
            result[2] += 0.043478260869565216;
            result[3] += 0;
            result[4] += 0.8347826086956521;
            result[5] += 0.034782608695652174;
          } else {
            result[0] += 0.07692307692307694;
            result[1] += 0.826923076923077;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.09615384615384617;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)66.5) ) ) {
            result[0] += 0.18055555555555558;
            result[1] += 0.36111111111111116;
            result[2] += 0;
            result[3] += 0.01388888888888889;
            result[4] += 0.41666666666666674;
            result[5] += 0.02777777777777778;
          } else {
            result[0] += 0.8912639405204461;
            result[1] += 0.01486988847583643;
            result[2] += 0.0009293680297397769;
            result[3] += 0.022304832713754646;
            result[4] += 0.05483271375464684;
            result[5] += 0.015799256505576207;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)88.5) ) ) {
            result[0] += 0.010810810810810811;
            result[1] += 0;
            result[2] += 0.11351351351351352;
            result[3] += 0.4486486486486487;
            result[4] += 0.02702702702702703;
            result[5] += 0.4;
          } else {
            result[0] += 0.7936507936507936;
            result[1] += 0;
            result[2] += 0.12698412698412698;
            result[3] += 0;
            result[4] += 0.07936507936507936;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)77) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.15625;
            result[4] += 0;
            result[5] += 0.84375;
          } else {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2621951219512195;
            result[3] += 0.5487804878048781;
            result[4] += 0;
            result[5] += 0.18902439024390244;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9375;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.0625;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.43478260869565216;
            result[3] += 0.43478260869565216;
            result[4] += 0.043478260869565216;
            result[5] += 0.08695652173913043;
          } else {
            result[0] += 0.010714285714285716;
            result[1] += 0;
            result[2] += 0.8142857142857144;
            result[3] += 0.16071428571428575;
            result[4] += 0;
            result[5] += 0.014285714285714287;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)82.5) ) ) {
            result[0] += 0.16279069767441862;
            result[1] += 0;
            result[2] += 0.8023255813953488;
            result[3] += 0.023255813953488372;
            result[4] += 0;
            result[5] += 0.011627906976744186;
          } else {
            result[0] += 0.0030534351145038168;
            result[1] += 0;
            result[2] += 0.9618320610687023;
            result[3] += 0.03206106870229008;
            result[4] += 0;
            result[5] += 0.0030534351145038168;
          }
        }
      }
    }
  }
  if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)72.5) ) ) {
    if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)111) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.4;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)59.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)58.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0.03225806451612903;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8709677419354839;
            result[5] += 0.0967741935483871;
          } else {
            result[0] += 0.03333333333333333;
            result[1] += 0.06666666666666667;
            result[2] += 0;
            result[3] += 0.1;
            result[4] += 0.26666666666666666;
            result[5] += 0.5333333333333333;
          }
        }
      }
    } else {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6363636363636364;
            result[5] += 0.36363636363636365;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.0218408736349454;
            result[4] += 0.0046801872074883;
            result[5] += 0.9734789391575663;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.043478260869565216;
            result[4] += 0.9565217391304348;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.09389671361502347;
            result[4] += 0.07981220657276995;
            result[5] += 0.8262910798122066;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.014705882352941176;
            result[3] += 0.11764705882352941;
            result[4] += 0.014705882352941176;
            result[5] += 0.8529411764705882;
          } else {
            result[0] += 0.039603960396039604;
            result[1] += 0.019801980198019802;
            result[2] += 0.0297029702970297;
            result[3] += 0.39603960396039606;
            result[4] += 0.0297029702970297;
            result[5] += 0.48514851485148514;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8601398601398601;
            result[4] += 0.013986013986013986;
            result[5] += 0.1258741258741259;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      }
    }
  } else {
    if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)99) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0.11578947368421053;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8631578947368421;
            result[5] += 0.021052631578947368;
          } else {
            result[0] += 0;
            result[1] += 0.9;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)69.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)100.5) ) ) {
            result[0] += 0.23943661971830985;
            result[1] += 0.07042253521126761;
            result[2] += 0;
            result[3] += 0.056338028169014086;
            result[4] += 0.5492957746478874;
            result[5] += 0.08450704225352113;
          } else {
            result[0] += 0;
            result[1] += 0.7333333333333333;
            result[2] += 0;
            result[3] += 0.06666666666666667;
            result[4] += 0;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
            result[0] += 0.9281818181818182;
            result[1] += 0.008181818181818182;
            result[2] += 0;
            result[3] += 0.01818181818181818;
            result[4] += 0.031818181818181815;
            result[5] += 0.013636363636363636;
          } else {
            result[0] += 0.22297297297297297;
            result[1] += 0.033783783783783786;
            result[2] += 0.16891891891891891;
            result[3] += 0.24324324324324326;
            result[4] += 0.12837837837837837;
            result[5] += 0.20270270270270271;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.08108108108108109;
            result[3] += 0.2702702702702703;
            result[4] += 0.6486486486486487;
            result[5] += 0;
          } else {
            result[0] += 0.00423728813559322;
            result[1] += 0.00423728813559322;
            result[2] += 0.1271186440677966;
            result[3] += 0.5720338983050848;
            result[4] += 0.00847457627118644;
            result[5] += 0.2838983050847458;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)105) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3793103448275862;
            result[3] += 0.3448275862068966;
            result[4] += 0;
            result[5] += 0.27586206896551724;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9047619047619048;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0.047619047619047616;
          }
        }
      } else {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)99.5) ) ) {
            result[0] += 0.039473684210526314;
            result[1] += 0.3815789473684211;
            result[2] += 0.14473684210526316;
            result[3] += 0.07894736842105263;
            result[4] += 0.25;
            result[5] += 0.10526315789473684;
          } else {
            result[0] += 0.7857142857142858;
            result[1] += 0.04761904761904762;
            result[2] += 0.14285714285714288;
            result[3] += 0.02380952380952381;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0.2;
          } else {
            result[0] += 0.020746887966804982;
            result[1] += 0;
            result[2] += 0.8858921161825727;
            result[3] += 0.07572614107883818;
            result[4] += 0.003112033195020747;
            result[5] += 0.014522821576763488;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)65) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9955555555555555;
            result[5] += 0.0044444444444444444;
          } else {
            result[0] += 0;
            result[1] += 0.4;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.18181818181818182;
            result[5] += 0.8181818181818182;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0.25882352941176473;
            result[1] += 0.35294117647058826;
            result[2] += 0;
            result[3] += 0.023529411764705882;
            result[4] += 0.29411764705882354;
            result[5] += 0.07058823529411765;
          } else {
            result[0] += 0.002442002442002442;
            result[1] += 0.004884004884004884;
            result[2] += 0;
            result[3] += 0.026862026862026864;
            result[4] += 0.052503052503052504;
            result[5] += 0.9133089133089133;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.014285714285714285;
            result[3] += 0.14285714285714285;
            result[4] += 0;
            result[5] += 0.8428571428571429;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.07207207207207207;
            result[3] += 0.6306306306306306;
            result[4] += 0.013513513513513514;
            result[5] += 0.28378378378378377;
          }
        }
      }
    } else {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)41.5) ) ) {
            result[0] += 0;
            result[1] += 0.7666666666666667;
            result[2] += 0;
            result[3] += 0.13333333333333333;
            result[4] += 0.1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.2;
            result[4] += 0.8;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)68.5) ) ) {
            result[0] += 0.11486486486486487;
            result[1] += 0.1554054054054054;
            result[2] += 0;
            result[3] += 0.013513513513513514;
            result[4] += 0.6756756756756757;
            result[5] += 0.04054054054054054;
          } else {
            result[0] += 0.8164251207729468;
            result[1] += 0.01610305958132045;
            result[2] += 0.0008051529790660225;
            result[3] += 0.01932367149758454;
            result[4] += 0.08373590982286634;
            result[5] += 0.06360708534621579;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)88.5) ) ) {
            result[0] += 0.008670520231213872;
            result[1] += 0.02601156069364162;
            result[2] += 0.23410404624277456;
            result[3] += 0.4421965317919075;
            result[4] += 0.011560693641618497;
            result[5] += 0.2774566473988439;
          } else {
            result[0] += 0.7241379310344828;
            result[1] += 0;
            result[2] += 0.11206896551724138;
            result[3] += 0.04310344827586207;
            result[4] += 0.1206896551724138;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)75.5) ) ) {
      if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)98) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.05263157894736842;
            result[4] += 0;
            result[5] += 0.9473684210526315;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)72) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)89) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)81.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)90.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05;
            result[3] += 0.9;
            result[4] += 0;
            result[5] += 0.05;
          }
        }
      }
    } else {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)103) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)93) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.18181818181818182;
            result[4] += 0;
            result[5] += 0.8181818181818182;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0.011627906976744188;
            result[2] += 0.5348837209302326;
            result[3] += 0.4186046511627908;
            result[4] += 0;
            result[5] += 0.034883720930232565;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.85;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.025;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)91.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.45454545454545453;
            result[3] += 0.2727272727272727;
            result[4] += 0.09090909090909091;
            result[5] += 0.18181818181818182;
          } else {
            result[0] += 0.006195786864931847;
            result[1] += 0;
            result[2] += 0.9442379182156134;
            result[3] += 0.04584882280049566;
            result[4] += 0;
            result[5] += 0.0037174721189591076;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)96) ) ) {
            result[0] += 0.00684931506849315;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.958904109589041;
            result[5] += 0.03424657534246575;
          } else {
            result[0] += 0;
            result[1] += 0.9375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0625;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)69) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3684210526315789;
            result[5] += 0.631578947368421;
          } else {
            result[0] += 0.8559322033898306;
            result[1] += 0;
            result[2] += 0.0423728813559322;
            result[3] += 0;
            result[4] += 0.03389830508474576;
            result[5] += 0.06779661016949153;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03495630461922597;
            result[4] += 0.03870162297128589;
            result[5] += 0.9263420724094882;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.19047619047619047;
            result[4] += 0.38095238095238093;
            result[5] += 0.42857142857142855;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)52) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.03508771929824561;
            result[1] += 0.03070175438596491;
            result[2] += 0.06140350877192982;
            result[3] += 0.33771929824561403;
            result[4] += 0.06140350877192982;
            result[5] += 0.47368421052631576;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)99.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)52.5) ) ) {
            result[0] += 0.051282051282051266;
            result[1] += 0.769230769230769;
            result[2] += 0.07692307692307691;
            result[3] += 0;
            result[4] += 0.10256410256410253;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.06779661016949153;
            result[2] += 0;
            result[3] += 0.01694915254237288;
            result[4] += 0.9152542372881356;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
            result[0] += 0.1625;
            result[1] += 0.0625;
            result[2] += 0.025;
            result[3] += 0.1875;
            result[4] += 0.0625;
            result[5] += 0.5;
          } else {
            result[0] += 0.8944911297852473;
            result[1] += 0.01587301587301587;
            result[2] += 0.0037348272642390283;
            result[3] += 0.02240896358543417;
            result[4] += 0.05788982259570494;
            result[5] += 0.005602240896358543;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.013888888888888888;
            result[2] += 0.06018518518518518;
            result[3] += 0.5648148148148148;
            result[4] += 0.05092592592592592;
            result[5] += 0.3101851851851852;
          } else {
            result[0] += 0.47770700636942676;
            result[1] += 0.10828025477707007;
            result[2] += 0.27388535031847133;
            result[3] += 0.03184713375796178;
            result[4] += 0.07643312101910828;
            result[5] += 0.03184713375796178;
          }
        }
      }
    }
  } else {
    if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)99) ) ) {
      if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)58.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03225806451612903;
            result[4] += 0;
            result[5] += 0.967741935483871;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3793103448275862;
            result[4] += 0.13793103448275862;
            result[5] += 0.4827586206896552;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.06818181818181818;
            result[3] += 0.7159090909090909;
            result[4] += 0;
            result[5] += 0.2159090909090909;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)93.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)65) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.22857142857142856;
            result[3] += 0.2571428571428571;
            result[4] += 0.02857142857142857;
            result[5] += 0.4857142857142857;
          } else {
            result[0] += 0.05555555555555555;
            result[1] += 0;
            result[2] += 0.8888888888888888;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.05555555555555555;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)95) ) ) {
            result[0] += 0.002840909090909091;
            result[1] += 0;
            result[2] += 0.7840909090909091;
            result[3] += 0.18181818181818182;
            result[4] += 0;
            result[5] += 0.03125;
          } else {
            result[0] += 0.55;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.05;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)93.5) ) ) {
            result[0] += 0.25;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.75;
          } else {
            result[0] += 0.0016501650165016504;
            result[1] += 0;
            result[2] += 0.9603960396039605;
            result[3] += 0.03630363036303631;
            result[4] += 0;
            result[5] += 0.0016501650165016504;
          }
        }
      }
    }
  }
  if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)38.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)74.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)50.5) ) ) {
            result[0] += 0.2857142857142857;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9822222222222222;
            result[5] += 0.017777777777777778;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)79) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)58) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)54) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7777777777777778;
            result[5] += 0.2222222222222222;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.010802469135802469;
            result[4] += 0.013888888888888888;
            result[5] += 0.9753086419753086;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.1187214611872146;
            result[4] += 0.1187214611872146;
            result[5] += 0.7625570776255708;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0.53125;
            result[1] += 0.34375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.03125;
            result[5] += 0.09375;
          } else {
            result[0] += 0;
            result[1] += 0.011627906976744186;
            result[2] += 0.046511627906976744;
            result[3] += 0.18604651162790697;
            result[4] += 0.046511627906976744;
            result[5] += 0.7093023255813954;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)80) ) ) {
            result[0] += 0.10447761194029853;
            result[1] += 0;
            result[2] += 0.1492537313432836;
            result[3] += 0.25373134328358216;
            result[4] += 0.13432835820895525;
            result[5] += 0.35820895522388063;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.036036036036036036;
            result[3] += 0.8558558558558559;
            result[4] += 0;
            result[5] += 0.10810810810810811;
          }
        }
      }
    }
  } else {
    if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)91) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0.05660377358490566;
            result[2] += 0;
            result[3] += 0.05660377358490566;
            result[4] += 0.8679245283018868;
            result[5] += 0.018867924528301886;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0.4;
            result[2] += 0;
            result[3] += 0.2;
            result[4] += 0.4;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9878048780487805;
            result[2] += 0;
            result[3] += 0.0024390243902439024;
            result[4] += 0.00975609756097561;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)65) ) ) {
            result[0] += 0.5871559633027522;
            result[1] += 0.06422018348623852;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3211009174311926;
            result[5] += 0.02752293577981651;
          } else {
            result[0] += 0.009708737864077669;
            result[1] += 0.04854368932038835;
            result[2] += 0.024271844660194174;
            result[3] += 0.3446601941747573;
            result[4] += 0.014563106796116505;
            result[5] += 0.558252427184466;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
            result[0] += 0.0196078431372549;
            result[1] += 0.5098039215686274;
            result[2] += 0;
            result[3] += 0.0392156862745098;
            result[4] += 0.4117647058823529;
            result[5] += 0.0196078431372549;
          } else {
            result[0] += 0.8311793214862682;
            result[1] += 0.009693053311793215;
            result[2] += 0.04038772213247173;
            result[3] += 0.03877221324717286;
            result[4] += 0.05977382875605816;
            result[5] += 0.020193861066235864;
          }
        }
      }
    } else {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)84.5) ) ) {
            result[0] += 0.13636363636363635;
            result[1] += 0.13636363636363635;
            result[2] += 0;
            result[3] += 0.18181818181818182;
            result[4] += 0.18181818181818182;
            result[5] += 0.36363636363636365;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.20833333333333334;
            result[3] += 0.6666666666666666;
            result[4] += 0.03125;
            result[5] += 0.09375;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)95) ) ) {
            result[0] += 0.06382978723404255;
            result[1] += 0;
            result[2] += 0.6170212765957447;
            result[3] += 0.1702127659574468;
            result[4] += 0;
            result[5] += 0.14893617021276595;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)85.5) ) ) {
            result[0] += 0.027777777777777776;
            result[1] += 0.027777777777777776;
            result[2] += 0.6666666666666666;
            result[3] += 0.1388888888888889;
            result[4] += 0;
            result[5] += 0.1388888888888889;
          } else {
            result[0] += 0.7142857142857143;
            result[1] += 0;
            result[2] += 0.23809523809523808;
            result[3] += 0;
            result[4] += 0.047619047619047616;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0.7142857142857143;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.013527575442247659;
            result[1] += 0;
            result[2] += 0.9115504682622269;
            result[3] += 0.07075962539021852;
            result[4] += 0.001040582726326743;
            result[5] += 0.003121748178980229;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)47.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0.004166666666666667;
            result[2] += 0;
            result[3] += 0.004166666666666667;
            result[4] += 0.9541666666666667;
            result[5] += 0.0375;
          } else {
            result[0] += 0;
            result[1] += 0.7692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.23076923076923078;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)67.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7954545454545454;
            result[5] += 0.20454545454545456;
          } else {
            result[0] += 0.0013623978201634877;
            result[1] += 0.005449591280653951;
            result[2] += 0.0013623978201634877;
            result[3] += 0.05313351498637602;
            result[4] += 0.02452316076294278;
            result[5] += 0.9141689373297003;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)76) ) ) {
            result[0] += 0.3089005235602094;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.015706806282722512;
            result[4] += 0.3507853403141361;
            result[5] += 0.32460732984293195;
          } else {
            result[0] += 0.036855036855036855;
            result[1] += 0;
            result[2] += 0.06142506142506143;
            result[3] += 0.49385749385749383;
            result[4] += 0.004914004914004914;
            result[5] += 0.40294840294840295;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)66.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)50) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)76) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.995085995085995;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004914004914004914;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)50) ) ) {
            result[0] += 0;
            result[1] += 0.9090909090909091;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.09090909090909091;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.15730337078651685;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8426966292134831;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)68.5) ) ) {
            result[0] += 0.3064516129032258;
            result[1] += 0.2903225806451613;
            result[2] += 0;
            result[3] += 0.016129032258064516;
            result[4] += 0.27419354838709675;
            result[5] += 0.11290322580645161;
          } else {
            result[0] += 0.9091734786557675;
            result[1] += 0.014532243415077202;
            result[2] += 0.005449591280653951;
            result[3] += 0.01725703905540418;
            result[4] += 0.04450499545867393;
            result[5] += 0.009082652134423252;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.03056768558951965;
            result[1] += 0.008733624454148471;
            result[2] += 0.043668122270742356;
            result[3] += 0.38427947598253276;
            result[4] += 0.13100436681222707;
            result[5] += 0.4017467248908297;
          } else {
            result[0] += 0.3529411764705882;
            result[1] += 0;
            result[2] += 0.503267973856209;
            result[3] += 0.0849673202614379;
            result[4] += 0.019607843137254898;
            result[5] += 0.039215686274509796;
          }
        }
      }
    }
  } else {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)106) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.03225806451612903;
            result[3] += 0.6451612903225806;
            result[4] += 0;
            result[5] += 0.3225806451612903;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.11764705882352941;
            result[4] += 0.058823529411764705;
            result[5] += 0.8235294117647058;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)101.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.18181818181818182;
            result[3] += 0.7272727272727273;
            result[4] += 0;
            result[5] += 0.09090909090909091;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.26666666666666666;
            result[3] += 0.06666666666666667;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.823529411764706;
            result[3] += 0.13725490196078435;
            result[4] += 0;
            result[5] += 0.03921568627450981;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)98.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)95) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5454545454545454;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0.05454545454545454;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9354838709677419;
            result[3] += 0.06451612903225806;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)75.5) ) ) {
            result[0] += 0.03333333333333333;
            result[1] += 0.16666666666666666;
            result[2] += 0.8;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7241379310344828;
            result[3] += 0.27586206896551724;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.007575757575757576;
            result[1] += 0;
            result[2] += 0.9570707070707071;
            result[3] += 0.03282828282828283;
            result[4] += 0;
            result[5] += 0.0025252525252525255;
          }
        }
      }
    }
  }
  if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)74.5) ) ) {
    if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.02040816326530612;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.02040816326530612;
            result[4] += 0.7551020408163265;
            result[5] += 0.20408163265306123;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)99) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0.11111111111111112;
            result[2] += 0;
            result[3] += 0.6666666666666667;
            result[4] += 0.22222222222222224;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)65.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0.75;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5238095238095238;
            result[5] += 0.47619047619047616;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0014144271570014145;
            result[3] += 0.028288543140028287;
            result[4] += 0.008486562942008486;
            result[5] += 0.9618104667609618;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)56.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0.4166666666666667;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5833333333333334;
            result[5] += 0;
          } else {
            result[0] += 0.9871794871794872;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.01282051282051282;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)84.5) ) ) {
            result[0] += 0.017391304347826087;
            result[1] += 0.014492753623188406;
            result[2] += 0.017391304347826087;
            result[3] += 0.19420289855072465;
            result[4] += 0.11304347826086956;
            result[5] += 0.6434782608695652;
          } else {
            result[0] += 0.005076142131979695;
            result[1] += 0.005076142131979695;
            result[2] += 0.015228426395939087;
            result[3] += 0.6802030456852792;
            result[4] += 0.015228426395939087;
            result[5] += 0.27918781725888325;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)52.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)73) ) ) {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.2;
            result[1] += 0.8;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)69.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)52) ) ) {
            result[0] += 0;
            result[1] += 0.7323943661971831;
            result[2] += 0;
            result[3] += 0.04225352112676056;
            result[4] += 0.16901408450704225;
            result[5] += 0.056338028169014086;
          } else {
            result[0] += 0.07377049180327869;
            result[1] += 0.08196721311475409;
            result[2] += 0;
            result[3] += 0.04918032786885246;
            result[4] += 0.7786885245901639;
            result[5] += 0.01639344262295082;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
            result[0] += 0.8812154696132595;
            result[1] += 0.006445672191528543;
            result[2] += 0.004604051565377532;
            result[3] += 0.025782688766114174;
            result[4] += 0.054327808471454866;
            result[5] += 0.027624309392265185;
          } else {
            result[0] += 0.3489583333333333;
            result[1] += 0.010416666666666666;
            result[2] += 0.09895833333333333;
            result[3] += 0.20833333333333334;
            result[4] += 0.109375;
            result[5] += 0.22395833333333334;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13043478260869565;
            result[3] += 0.2608695652173913;
            result[4] += 0;
            result[5] += 0.6086956521739131;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.6555555555555556;
            result[4] += 0;
            result[5] += 0.14444444444444443;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5486725663716814;
            result[3] += 0.35398230088495575;
            result[4] += 0.008849557522123894;
            result[5] += 0.08849557522123894;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9642857142857143;
            result[3] += 0.03571428571428571;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)95) ) ) {
            result[0] += 0.03409090909090909;
            result[1] += 0;
            result[2] += 0.7272727272727273;
            result[3] += 0.13636363636363635;
            result[4] += 0.011363636363636364;
            result[5] += 0.09090909090909091;
          } else {
            result[0] += 0.8;
            result[1] += 0;
            result[2] += 0.05;
            result[3] += 0.15;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)86.5) ) ) {
            result[0] += 0.003484320557491289;
            result[1] += 0;
            result[2] += 0.8501742160278746;
            result[3] += 0.13240418118466898;
            result[4] += 0;
            result[5] += 0.013937282229965157;
          } else {
            result[0] += 0.010526315789473684;
            result[1] += 0;
            result[2] += 0.9701754385964912;
            result[3] += 0.017543859649122806;
            result[4] += 0;
            result[5] += 0.0017543859649122807;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0.004464285714285714;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9732142857142857;
            result[5] += 0.022321428571428572;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0.9433962264150944;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.05660377358490566;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6551724137931034;
            result[5] += 0.3448275862068966;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0012642225031605564;
            result[3] += 0.02402022756005057;
            result[4] += 0.021491782553729456;
            result[5] += 0.9532237673830595;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.09895833333333333;
            result[4] += 0.15625;
            result[5] += 0.7447916666666666;
          } else {
            result[0] += 0;
            result[1] += 0.007067137809187279;
            result[2] += 0.0035335689045936395;
            result[3] += 0.5512367491166078;
            result[4] += 0.05653710247349823;
            result[5] += 0.38162544169611307;
          }
        }
      }
    } else {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)107.5) ) ) {
            result[0] += 0.7721423164269493;
            result[1] += 0.0348221044663134;
            result[2] += 0.011355034065102196;
            result[3] += 0.029523088569265707;
            result[4] += 0.12339137017411052;
            result[5] += 0.028766086298258896;
          } else {
            result[0] += 0;
            result[1] += 0.9787234042553191;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.02127659574468085;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
            result[0] += 0.0040650406504065045;
            result[1] += 0;
            result[2] += 0.22764227642276422;
            result[3] += 0.44715447154471544;
            result[4] += 0.08130081300813008;
            result[5] += 0.23983739837398374;
          } else {
            result[0] += 0.34269662921348315;
            result[1] += 0.07865168539325842;
            result[2] += 0.4044943820224719;
            result[3] += 0.0056179775280898875;
            result[4] += 0.06741573033707865;
            result[5] += 0.10112359550561797;
          }
        }
      } else {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    }
  } else {
    if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)105.5) ) ) {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0.125;
            result[2] += 0.0625;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0.3125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0.746031746031746;
            result[4] += 0.031746031746031744;
            result[5] += 0.07936507936507936;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.576271186440678;
            result[3] += 0.4067796610169492;
            result[4] += 0;
            result[5] += 0.01694915254237288;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)73) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8301886792452831;
            result[3] += 0.1320754716981132;
            result[4] += 0;
            result[5] += 0.03773584905660377;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)83.5) ) ) {
        result[0] += 0;
        result[1] += 0;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 1;
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.75;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.25;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7058823529411765;
            result[3] += 0.20588235294117646;
            result[4] += 0;
            result[5] += 0.08823529411764706;
          } else {
            result[0] += 0.005979073243647235;
            result[1] += 0;
            result[2] += 0.9566517189835575;
            result[3] += 0.0343796711509716;
            result[4] += 0;
            result[5] += 0.0029895366218236174;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)63.5) ) ) {
            result[0] += 0.0047169811320754715;
            result[1] += 0.018867924528301886;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9716981132075472;
            result[5] += 0.0047169811320754715;
          } else {
            result[0] += 0.7647058823529411;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.17647058823529413;
            result[5] += 0.058823529411764705;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.0273972602739726;
            result[1] += 0.0136986301369863;
            result[2] += 0;
            result[3] += 0.0136986301369863;
            result[4] += 0.3013698630136986;
            result[5] += 0.6438356164383562;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0015060240963855422;
            result[3] += 0.012048192771084338;
            result[4] += 0.012048192771084338;
            result[5] += 0.9743975903614458;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0.08571428571428572;
            result[2] += 0;
            result[3] += 0.17142857142857143;
            result[4] += 0.10357142857142858;
            result[5] += 0.6392857142857142;
          } else {
            result[0] += 0.005988023952095809;
            result[1] += 0.011976047904191617;
            result[2] += 0.059880239520958084;
            result[3] += 0.6047904191616766;
            result[4] += 0.03592814371257485;
            result[5] += 0.281437125748503;
          }
        }
      }
    } else {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)69.5) ) ) {
            result[0] += 0.061946902654867256;
            result[1] += 0.18584070796460178;
            result[2] += 0;
            result[3] += 0.04424778761061947;
            result[4] += 0.6194690265486725;
            result[5] += 0.08849557522123894;
          } else {
            result[0] += 0.23756906077348067;
            result[1] += 0.0027624309392265192;
            result[2] += 0.011049723756906077;
            result[3] += 0.19060773480662985;
            result[4] += 0.13259668508287292;
            result[5] += 0.425414364640884;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)68.5) ) ) {
            result[0] += 0.05426356589147287;
            result[1] += 0.3643410852713178;
            result[2] += 0.015503875968992248;
            result[3] += 0.05426356589147287;
            result[4] += 0.49612403100775193;
            result[5] += 0.015503875968992248;
          } else {
            result[0] += 0.813953488372093;
            result[1] += 0.016611295681063124;
            result[2] += 0.03737541528239203;
            result[3] += 0.04401993355481727;
            result[4] += 0.0606312292358804;
            result[5] += 0.027408637873754152;
          }
        }
      } else {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)82.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)97.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.6;
            result[4] += 0;
            result[5] += 0.4;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)70) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0.5555555555555556;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.22222222222222224;
            result[3] += 0.763888888888889;
            result[4] += 0;
            result[5] += 0.01388888888888889;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.03333333333333333;
            result[4] += 0;
            result[5] += 0.4666666666666667;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.17857142857142858;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0.07142857142857142;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)119) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8532110091743119;
            result[3] += 0.12844036697247707;
            result[4] += 0;
            result[5] += 0.01834862385321101;
          } else {
            result[0] += 0.55;
            result[1] += 0;
            result[2] += 0.3;
            result[3] += 0;
            result[4] += 0.15;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)86) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.26666666666666666;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0.06666666666666667;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8421052631578948;
            result[3] += 0.052631578947368425;
            result[4] += 0;
            result[5] += 0.10526315789473685;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)105) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9814814814814815;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.018518518518518517;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5833333333333334;
            result[3] += 0.4166666666666667;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)103.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0.5555555555555556;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)71) ) ) {
            result[0] += 0.6666666666666666;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0013089005235602095;
            result[1] += 0;
            result[2] += 0.9568062827225131;
            result[3] += 0.03664921465968586;
            result[4] += 0;
            result[5] += 0.005235602094240838;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9914163090128756;
            result[5] += 0.008583690987124463;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)58.5) ) ) {
            result[0] += 0.8392857142857143;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.16071428571428573;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.171875;
            result[2] += 0.015625;
            result[3] += 0;
            result[4] += 0.28125;
            result[5] += 0.53125;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.0182370820668693;
            result[4] += 0.025835866261398176;
            result[5] += 0.9559270516717325;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05;
            result[3] += 0.25;
            result[4] += 0.2;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0.022222222222222223;
            result[2] += 0.003703703703703704;
            result[3] += 0.17037037037037037;
            result[4] += 0.11481481481481481;
            result[5] += 0.6888888888888889;
          } else {
            result[0] += 0.02074688796680498;
            result[1] += 0.029045643153526972;
            result[2] += 0.06639004149377593;
            result[3] += 0.45228215767634855;
            result[4] += 0.04564315352697095;
            result[5] += 0.38589211618257263;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)99.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)45.5) ) ) {
            result[0] += 0;
            result[1] += 0.9583333333333334;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.041666666666666664;
            result[5] += 0;
          } else {
            result[0] += 0.03125;
            result[1] += 0.03125;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9375;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)57.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)74.5) ) ) {
            result[0] += 0.3525641025641026;
            result[1] += 0.057692307692307696;
            result[2] += 0.01282051282051282;
            result[3] += 0.19230769230769232;
            result[4] += 0.21153846153846154;
            result[5] += 0.17307692307692307;
          } else {
            result[0] += 0.9083333333333334;
            result[1] += 0.01851851851851852;
            result[2] += 0.0009259259259259261;
            result[3] += 0.012962962962962964;
            result[4] += 0.057407407407407414;
            result[5] += 0.0018518518518518521;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)84) ) ) {
            result[0] += 0.02142857142857143;
            result[1] += 0.03571428571428571;
            result[2] += 0.15;
            result[3] += 0.40714285714285714;
            result[4] += 0.007142857142857143;
            result[5] += 0.37857142857142856;
          } else {
            result[0] += 0.47368421052631576;
            result[1] += 0;
            result[2] += 0.3684210526315789;
            result[3] += 0.09649122807017543;
            result[4] += 0.05263157894736842;
            result[5] += 0.008771929824561403;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)90.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.061224489795918366;
            result[3] += 0.02040816326530612;
            result[4] += 0.08163265306122448;
            result[5] += 0.8367346938775511;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5714285714285714;
            result[4] += 0;
            result[5] += 0.42857142857142855;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.0625;
            result[4] += 0;
            result[5] += 0.9375;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09523809523809523;
            result[3] += 0.6547619047619048;
            result[4] += 0;
            result[5] += 0.25;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0.05128205128205128;
            result[2] += 0.2564102564102564;
            result[3] += 0.6153846153846154;
            result[4] += 0;
            result[5] += 0.07692307692307693;
          } else {
            result[0] += 0.0273972602739726;
            result[1] += 0;
            result[2] += 0.726027397260274;
            result[3] += 0.136986301369863;
            result[4] += 0.0684931506849315;
            result[5] += 0.0410958904109589;
          }
        }
      }
    } else {
      if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)95) ) ) {
            result[0] += 0.005747126436781609;
            result[1] += 0;
            result[2] += 0.7183908045977011;
            result[3] += 0.2471264367816092;
            result[4] += 0;
            result[5] += 0.028735632183908046;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5454545454545454;
            result[3] += 0.45454545454545453;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9634146341463414;
            result[3] += 0.036585365853658534;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)91) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9196428571428571;
            result[3] += 0.08035714285714286;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.95;
            result[3] += 0.05;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)91) ) ) {
            result[0] += 0.02027027027027027;
            result[1] += 0.013513513513513514;
            result[2] += 0.006756756756756757;
            result[3] += 0.006756756756756757;
            result[4] += 0.8918918918918919;
            result[5] += 0.060810810810810814;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)69.5) ) ) {
            result[0] += 0.08;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.44;
            result[5] += 0.48;
          } else {
            result[0] += 0.954248366013072;
            result[1] += 0;
            result[2] += 0.006535947712418301;
            result[3] += 0;
            result[4] += 0.0392156862745098;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0011074197120708748;
            result[3] += 0.05094130675526024;
            result[4] += 0.05426356589147287;
            result[5] += 0.893687707641196;
          } else {
            result[0] += 0.010050251256281407;
            result[1] += 0.11557788944723618;
            result[2] += 0.010050251256281407;
            result[3] += 0.19597989949748743;
            result[4] += 0.271356783919598;
            result[5] += 0.3969849246231156;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.020905923344947737;
            result[3] += 0.6968641114982579;
            result[4] += 0;
            result[5] += 0.28222996515679444;
          } else {
            result[0] += 0.06349206349206349;
            result[1] += 0;
            result[2] += 0.3492063492063492;
            result[3] += 0.25396825396825395;
            result[4] += 0.015873015873015872;
            result[5] += 0.31746031746031744;
          }
        }
      }
    } else {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)69.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)51.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.006696428571428571;
            result[1] += 0.9910714285714286;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002232142857142857;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)106) ) ) {
            result[0] += 0.14772727272727276;
            result[1] += 0.09090909090909093;
            result[2] += 0.034090909090909095;
            result[3] += 0.011363636363636366;
            result[4] += 0.6704545454545455;
            result[5] += 0.04545454545454546;
          } else {
            result[0] += 0.03225806451612904;
            result[1] += 0.8064516129032259;
            result[2] += 0;
            result[3] += 0.06451612903225808;
            result[4] += 0.09677419354838711;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73.5) ) ) {
            result[0] += 0.0891089108910891;
            result[1] += 0.32673267326732675;
            result[2] += 0.1188118811881188;
            result[3] += 0.06930693069306931;
            result[4] += 0.0297029702970297;
            result[5] += 0.36633663366336633;
          } else {
            result[0] += 0.9184726522187823;
            result[1] += 0.007223942208462332;
            result[2] += 0.0196078431372549;
            result[3] += 0.006191950464396285;
            result[4] += 0.04437564499484004;
            result[5] += 0.0041279669762641896;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)93) ) ) {
            result[0] += 0.03496503496503497;
            result[1] += 0.04195804195804196;
            result[2] += 0.3706293706293706;
            result[3] += 0.32867132867132864;
            result[4] += 0.03496503496503497;
            result[5] += 0.1888111888111888;
          } else {
            result[0] += 0.6923076923076924;
            result[1] += 0;
            result[2] += 0.07692307692307694;
            result[3] += 0;
            result[4] += 0.20000000000000004;
            result[5] += 0.030769230769230774;
          }
        }
      }
    }
  } else {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)86) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)69) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09090909090909091;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0.1590909090909091;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0.21428571428571427;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)75) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3953488372093023;
            result[3] += 0.4883720930232558;
            result[4] += 0;
            result[5] += 0.11627906976744186;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)110) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8867924528301887;
            result[3] += 0.11320754716981132;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.75;
            result[5] += 0.25;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8951219512195122;
            result[3] += 0.08780487804878048;
            result[4] += 0;
            result[5] += 0.01707317073170732;
          } else {
            result[0] += 0.00936768149882904;
            result[1] += 0;
            result[2] += 0.9836065573770492;
            result[3] += 0.00702576112412178;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)71.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)101) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0.0091324200913242;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9908675799086758;
            result[5] += 0;
          } else {
            result[0] += 0.11111111111111112;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666667;
            result[5] += 0.22222222222222224;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
            result[0] += 0.18181818181818185;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666667;
            result[5] += 0.15151515151515155;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.0332512315270936;
            result[4] += 0.03078817733990148;
            result[5] += 0.9359605911330049;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)85.5) ) ) {
            result[0] += 0.09345794392523364;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.11214953271028037;
            result[4] += 0.07476635514018691;
            result[5] += 0.719626168224299;
          } else {
            result[0] += 0.01984126984126984;
            result[1] += 0.08333333333333333;
            result[2] += 0.05952380952380952;
            result[3] += 0.5;
            result[4] += 0.051587301587301584;
            result[5] += 0.2857142857142857;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)58.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)88) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)38) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0.022222222222222223;
            result[4] += 0.3111111111111111;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9976415094339622;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0023584905660377358;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)84.5) ) ) {
            result[0] += 0.563302752293578;
            result[1] += 0.03486238532110092;
            result[2] += 0;
            result[3] += 0.07155963302752294;
            result[4] += 0.22752293577981653;
            result[5] += 0.10275229357798166;
          } else {
            result[0] += 0.9375796178343949;
            result[1] += 0;
            result[2] += 0.006369426751592357;
            result[3] += 0.01019108280254777;
            result[4] += 0.03694267515923567;
            result[5] += 0.008917197452229299;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0.03322259136212625;
            result[1] += 0.013289036544850499;
            result[2] += 0.09966777408637874;
            result[3] += 0.5282392026578073;
            result[4] += 0.06976744186046512;
            result[5] += 0.2558139534883721;
          } else {
            result[0] += 0.3297872340425532;
            result[1] += 0;
            result[2] += 0.5159574468085107;
            result[3] += 0.07978723404255321;
            result[4] += 0.04255319148936171;
            result[5] += 0.03191489361702128;
          }
        }
      }
    }
  } else {
    if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)79.5) ) ) {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.058823529411764705;
            result[3] += 0.17647058823529413;
            result[4] += 0.029411764705882353;
            result[5] += 0.7352941176470589;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7272727272727273;
            result[4] += 0.18181818181818182;
            result[5] += 0.09090909090909091;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)75) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)95) ) ) {
            result[0] += 0.029850746268656716;
            result[1] += 0.05970149253731343;
            result[2] += 0.6865671641791045;
            result[3] += 0.13432835820895522;
            result[4] += 0;
            result[5] += 0.08955223880597014;
          } else {
            result[0] += 0.7222222222222222;
            result[1] += 0;
            result[2] += 0.2777777777777778;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)95) ) ) {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)101.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)110) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7718446601941747;
            result[3] += 0.20388349514563106;
            result[4] += 0;
            result[5] += 0.024271844660194174;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0.8333333333333334;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0015384615384615385;
            result[1] += 0;
            result[2] += 0.9584615384615385;
            result[3] += 0.036923076923076927;
            result[4] += 0;
            result[5] += 0.003076923076923077;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)69.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9829545454545454;
            result[5] += 0.017045454545454544;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.44680851063829785;
            result[5] += 0.5531914893617021;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0.16666666666666666;
            result[2] += 0;
            result[3] += 0.1111111111111111;
            result[4] += 0.6296296296296297;
            result[5] += 0.09259259259259259;
          } else {
            result[0] += 0.9259259259259259;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.018518518518518517;
            result[4] += 0.05555555555555555;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)58) ) ) {
            result[0] += 0.14634146341463414;
            result[1] += 0.024390243902439025;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5853658536585366;
            result[5] += 0.24390243902439024;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0011534025374855825;
            result[3] += 0.05536332179930796;
            result[4] += 0.04267589388696655;
            result[5] += 0.9008073817762399;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)73) ) ) {
            result[0] += 0.008620689655172414;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3103448275862069;
            result[4] += 0.017241379310344827;
            result[5] += 0.6637931034482759;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5642857142857143;
            result[4] += 0;
            result[5] += 0.4357142857142857;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)105.5) ) ) {
            result[0] += 0.020202020202020204;
            result[1] += 0.0707070707070707;
            result[2] += 0;
            result[3] += 0.020202020202020204;
            result[4] += 0.8282828282828283;
            result[5] += 0.06060606060606061;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)54.5) ) ) {
            result[0] += 0;
            result[1] += 0.9922480620155039;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.007751937984496124;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)65) ) ) {
            result[0] += 0.13043478260869565;
            result[1] += 0.5362318840579711;
            result[2] += 0;
            result[3] += 0.057971014492753624;
            result[4] += 0.2753623188405797;
            result[5] += 0;
          } else {
            result[0] += 0.8929527207850132;
            result[1] += 0.008920606601248883;
            result[2] += 0.0008920606601248883;
            result[3] += 0.019625334522747544;
            result[4] += 0.06779661016949151;
            result[5] += 0.009812667261373772;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)87.5) ) ) {
            result[0] += 0.03608247422680412;
            result[1] += 0.05154639175257732;
            result[2] += 0.0979381443298969;
            result[3] += 0.36082474226804123;
            result[4] += 0.08247422680412371;
            result[5] += 0.3711340206185567;
          } else {
            result[0] += 0.7927927927927928;
            result[1] += 0.018018018018018018;
            result[2] += 0.09009009009009009;
            result[3] += 0.009009009009009009;
            result[4] += 0.09009009009009009;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)95.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.24;
            result[4] += 0;
            result[5] += 0.76;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.11347517730496455;
            result[3] += 0.7517730496453902;
            result[4] += 0;
            result[5] += 0.13475177304964542;
          }
        }
      } else {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)103.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.34375;
            result[3] += 0.46875;
            result[4] += 0.046875;
            result[5] += 0.140625;
          } else {
            result[0] += 0.03333333333333334;
            result[1] += 0;
            result[2] += 0.7000000000000001;
            result[3] += 0.13333333333333336;
            result[4] += 0;
            result[5] += 0.13333333333333336;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)118.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9523809523809523;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.6666666666666666;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)103) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)72.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.02409638554216868;
            result[1] += 0;
            result[2] += 0.5180722891566266;
            result[3] += 0.32530120481927716;
            result[4] += 0;
            result[5] += 0.13253012048192775;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8377192982456141;
            result[3] += 0.15789473684210525;
            result[4] += 0;
            result[5] += 0.0043859649122807015;
          } else {
            result[0] += 0.28125;
            result[1] += 0;
            result[2] += 0.71875;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)82) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8297872340425532;
            result[3] += 0.10638297872340426;
            result[4] += 0;
            result[5] += 0.06382978723404255;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.36363636363636365;
            result[3] += 0.6363636363636364;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.006557377049180328;
            result[1] += 0.001639344262295082;
            result[2] += 0.9672131147540983;
            result[3] += 0.022950819672131147;
            result[4] += 0;
            result[5] += 0.001639344262295082;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)88.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)59.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)37) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.004347826086956523;
            result[4] += 0.9869565217391305;
            result[5] += 0.008695652173913045;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
            result[0] += 0.09090909090909091;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8787878787878788;
            result[5] += 0.030303030303030304;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.045454545454545456;
            result[4] += 0.2727272727272727;
            result[5] += 0.6818181818181818;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)56.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)65) ) ) {
            result[0] += 0.14285714285714285;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0.14285714285714285;
          } else {
            result[0] += 0.9875;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0125;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)84.5) ) ) {
            result[0] += 0.001451378809869376;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.020319303338171262;
            result[4] += 0.03773584905660377;
            result[5] += 0.9404934687953556;
          } else {
            result[0] += 0.014336917562724014;
            result[1] += 0.06093189964157706;
            result[2] += 0.010752688172043012;
            result[3] += 0.24372759856630824;
            result[4] += 0.04659498207885305;
            result[5] += 0.6236559139784946;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)88) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)61) ) ) {
            result[0] += 0;
            result[1] += 0.07692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9230769230769231;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)81.5) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9910112359550561;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.008988764044943821;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)65) ) ) {
            result[0] += 0.7393715341959335;
            result[1] += 0.04251386321626617;
            result[2] += 0;
            result[3] += 0.005545286506469501;
            result[4] += 0.20147874306839186;
            result[5] += 0.011090573012939002;
          } else {
            result[0] += 0.07427536231884058;
            result[1] += 0.03804347826086957;
            result[2] += 0.04710144927536232;
            result[3] += 0.4057971014492754;
            result[4] += 0.10144927536231885;
            result[5] += 0.3333333333333333;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
            result[0] += 0.22580645161290322;
            result[1] += 0;
            result[2] += 0.5483870967741935;
            result[3] += 0.1935483870967742;
            result[4] += 0;
            result[5] += 0.03225806451612903;
          } else {
            result[0] += 0.980327868852459;
            result[1] += 0;
            result[2] += 0.013114754098360656;
            result[3] += 0;
            result[4] += 0.006557377049180328;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)82) ) ) {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)83) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.01694915254237288;
            result[3] += 0.7966101694915254;
            result[4] += 0;
            result[5] += 0.1864406779661017;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.17647058823529413;
            result[3] += 0.35294117647058826;
            result[4] += 0;
            result[5] += 0.47058823529411764;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9;
            result[3] += 0.1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)74.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666669;
            result[3] += 0.6666666666666667;
            result[4] += 0;
            result[5] += 0.16666666666666669;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7682119205298014;
            result[3] += 0.18543046357615894;
            result[4] += 0;
            result[5] += 0.046357615894039736;
          }
        }
      }
    } else {
      if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)93.5) ) ) {
            result[0] += 0.0625;
            result[1] += 0;
            result[2] += 0.375;
            result[3] += 0.3125;
            result[4] += 0;
            result[5] += 0.25;
          } else {
            result[0] += 0.03125;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0.10416666666666667;
            result[4] += 0;
            result[5] += 0.03125;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8901960784313726;
            result[3] += 0.10588235294117648;
            result[4] += 0;
            result[5] += 0.003921568627450981;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)87.5) ) ) {
            result[0] += 0.004629629629629629;
            result[1] += 0;
            result[2] += 0.9444444444444444;
            result[3] += 0.046296296296296294;
            result[4] += 0;
            result[5] += 0.004629629629629629;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9920634920634921;
            result[3] += 0.007936507936507936;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)97) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)65) ) ) {
            result[0] += 0.004016064257028112;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9759036144578314;
            result[5] += 0.020080321285140562;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4444444444444444;
            result[5] += 0.5555555555555556;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)49.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)55.5) ) ) {
            result[0] += 0.7884615384615385;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1730769230769231;
            result[5] += 0.03846153846153847;
          } else {
            result[0] += 0;
            result[1] += 0.009560229445506692;
            result[2] += 0.009560229445506692;
            result[3] += 0.05640535372848948;
            result[4] += 0.0372848948374761;
            result[5] += 0.887189292543021;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.08;
            result[4] += 0.22666666666666666;
            result[5] += 0.6933333333333334;
          } else {
            result[0] += 0.010869565217391304;
            result[1] += 0.010869565217391304;
            result[2] += 0.10869565217391304;
            result[3] += 0.6086956521739131;
            result[4] += 0;
            result[5] += 0.2608695652173913;
          }
        }
      }
    } else {
      if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)50.5) ) ) {
            result[0] += 0.016129032258064516;
            result[1] += 0.9032258064516129;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.08064516129032258;
            result[5] += 0;
          } else {
            result[0] += 0.22043010752688172;
            result[1] += 0.03225806451612903;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7204301075268817;
            result[5] += 0.026881720430107527;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)74.5) ) ) {
            result[0] += 0.8731060606060606;
            result[1] += 0.001893939393939394;
            result[2] += 0.00946969696969697;
            result[3] += 0.029356060606060608;
            result[4] += 0.05303030303030303;
            result[5] += 0.03314393939393939;
          } else {
            result[0] += 0.3862660944206007;
            result[1] += 0.017167381974248924;
            result[2] += 0.15879828326180254;
            result[3] += 0.24463519313304716;
            result[4] += 0.07296137339055793;
            result[5] += 0.12017167381974246;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)81) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)73) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)98) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.42857142857142855;
            result[1] += 0.14285714285714285;
            result[2] += 0.42857142857142855;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0.08333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8333333333333334;
            result[5] += 0.08333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.1;
            result[4] += 0;
            result[5] += 0.8;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.12500000000000003;
            result[3] += 0.7171052631578948;
            result[4] += 0.006578947368421053;
            result[5] += 0.15131578947368424;
          } else {
            result[0] += 0;
            result[1] += 0.08333333333333333;
            result[2] += 0.6666666666666666;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)103) ) ) {
            result[0] += 0.0625;
            result[1] += 0.0625;
            result[2] += 0.125;
            result[3] += 0.03125;
            result[4] += 0.1875;
            result[5] += 0.53125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)90) ) ) {
            result[0] += 0.13333333333333333;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0.4666666666666667;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8823529411764706;
            result[3] += 0.09803921568627451;
            result[4] += 0.0196078431372549;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)93) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0.2;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.2;
            result[4] += 0.4;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.011363636363636366;
            result[1] += 0;
            result[2] += 0.8030303030303031;
            result[3] += 0.17045454545454547;
            result[4] += 0;
            result[5] += 0.015151515151515154;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8943661971830986;
            result[3] += 0.1056338028169014;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0019841269841269845;
            result[1] += 0;
            result[2] += 0.9841269841269842;
            result[3] += 0.01388888888888889;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)88.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)41) ) ) {
            result[0] += 0;
            result[1] += 0.8461538461538461;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.15384615384615385;
            result[5] += 0;
          } else {
            result[0] += 0.0076045627376425855;
            result[1] += 0.0076045627376425855;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9657794676806084;
            result[5] += 0.019011406844106463;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.23809523809523808;
            result[5] += 0.7619047619047619;
          } else {
            result[0] += 0.05357142857142857;
            result[1] += 0.03571428571428571;
            result[2] += 0;
            result[3] += 0.10714285714285714;
            result[4] += 0.6785714285714286;
            result[5] += 0.125;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)60) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9333333333333333;
            result[5] += 0.06666666666666667;
          } else {
            result[0] += 0.9836065573770492;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.01639344262295082;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0012836970474967907;
            result[3] += 0.04621309370988447;
            result[4] += 0.04236200256739409;
            result[5] += 0.9101412066752247;
          } else {
            result[0] += 0.028169014084507043;
            result[1] += 0.018779342723004695;
            result[2] += 0.028169014084507043;
            result[3] += 0.26291079812206575;
            result[4] += 0.1643192488262911;
            result[5] += 0.49765258215962443;
          }
        }
      }
    } else {
      if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)58) ) ) {
            result[0] += 0.8114754098360656;
            result[1] += 0.040983606557377046;
            result[2] += 0.02459016393442623;
            result[3] += 0.01639344262295082;
            result[4] += 0.08196721311475409;
            result[5] += 0.02459016393442623;
          } else {
            result[0] += 0.030991735537190084;
            result[1] += 0.010330578512396695;
            result[2] += 0.047520661157024795;
            result[3] += 0.44008264462809915;
            result[4] += 0.04132231404958678;
            result[5] += 0.4297520661157025;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)71.5) ) ) {
            result[0] += 0.032;
            result[1] += 0.608;
            result[2] += 0;
            result[3] += 0.008;
            result[4] += 0.336;
            result[5] += 0.016;
          } else {
            result[0] += 0.8351931330472103;
            result[1] += 0.018025751072961373;
            result[2] += 0.048068669527897;
            result[3] += 0.03862660944206009;
            result[4] += 0.03862660944206009;
            result[5] += 0.02145922746781116;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0.6923076923076923;
            result[2] += 0;
            result[3] += 0.3076923076923077;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.3333333333333333;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)82) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)107) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16091954022988506;
            result[3] += 0.735632183908046;
            result[4] += 0;
            result[5] += 0.10344827586206896;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.42857142857142855;
            result[3] += 0.19047619047619047;
            result[4] += 0;
            result[5] += 0.38095238095238093;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)98) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)107) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9019607843137255;
            result[3] += 0.0392156862745098;
            result[4] += 0;
            result[5] += 0.058823529411764705;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)67.5) ) ) {
        result[0] += 1;
        result[1] += 0;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.625;
            result[3] += 0.35;
            result[4] += 0;
            result[5] += 0.025;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8771929824561404;
            result[3] += 0.10087719298245615;
            result[4] += 0;
            result[5] += 0.02192982456140351;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8980891719745223;
            result[3] += 0.10191082802547771;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.004081632653061225;
            result[1] += 0;
            result[2] += 0.9918367346938776;
            result[3] += 0.0020408163265306124;
            result[4] += 0;
            result[5] += 0.0020408163265306124;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)109) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)61.5) ) ) {
            result[0] += 0.009174311926605505;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.981651376146789;
            result[5] += 0.009174311926605505;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8461538461538461;
            result[5] += 0.15384615384615385;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.0015408320493066256;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.015408320493066256;
            result[4] += 0.023112480739599383;
            result[5] += 0.9599383667180277;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
            result[0] += 0.6736842105263158;
            result[1] += 0.06315789473684211;
            result[2] += 0;
            result[3] += 0.07368421052631578;
            result[4] += 0.06315789473684211;
            result[5] += 0.12631578947368421;
          } else {
            result[0] += 0.00482315112540193;
            result[1] += 0.012861736334405145;
            result[2] += 0.00964630225080386;
            result[3] += 0.3327974276527331;
            result[4] += 0.10610932475884244;
            result[5] += 0.5337620578778135;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)54) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89) ) ) {
            result[0] += 0;
            result[1] += 0.4444444444444444;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5555555555555556;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9974358974358974;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002564102564102564;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)50) ) ) {
            result[0] += 0;
            result[1] += 0.9130434782608695;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.08695652173913043;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.06557377049180328;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9016393442622951;
            result[5] += 0.03278688524590164;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)66.5) ) ) {
            result[0] += 0.14285714285714285;
            result[1] += 0.39285714285714285;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.40476190476190477;
            result[5] += 0.05952380952380952;
          } else {
            result[0] += 0.9000846740050804;
            result[1] += 0.004233700254022015;
            result[2] += 0.012701100762066046;
            result[3] += 0.023708721422523286;
            result[4] += 0.04064352243861134;
            result[5] += 0.018628281117696866;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)85.5) ) ) {
            result[0] += 0.029069767441860465;
            result[1] += 0.011627906976744186;
            result[2] += 0.28488372093023256;
            result[3] += 0.38953488372093026;
            result[4] += 0.06976744186046512;
            result[5] += 0.21511627906976744;
          } else {
            result[0] += 0.5263157894736842;
            result[1] += 0.06315789473684211;
            result[2] += 0.25263157894736843;
            result[3] += 0.042105263157894736;
            result[4] += 0.08421052631578947;
            result[5] += 0.031578947368421054;
          }
        }
      }
    }
  } else {
    if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)83.5) ) ) {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10975609756097561;
            result[3] += 0.3048780487804878;
            result[4] += 0.036585365853658534;
            result[5] += 0.5487804878048781;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16129032258064518;
            result[3] += 0.6774193548387097;
            result[4] += 0;
            result[5] += 0.16129032258064518;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.2;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0;
            result[4] += 0.4;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)92.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.35211267605633806;
            result[3] += 0.5352112676056339;
            result[4] += 0;
            result[5] += 0.11267605633802819;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7692307692307693;
            result[3] += 0.23076923076923078;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)77) ) ) {
            result[0] += 0.13333333333333333;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0;
            result[4] += 0.13333333333333333;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8863636363636364;
            result[3] += 0.11363636363636363;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.55;
            result[3] += 0.45;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8216216216216217;
            result[3] += 0.15135135135135136;
            result[4] += 0;
            result[5] += 0.02702702702702703;
          } else {
            result[0] += 0.005822416302765648;
            result[1] += 0;
            result[2] += 0.9650655021834061;
            result[3] += 0.023289665211062592;
            result[4] += 0;
            result[5] += 0.005822416302765648;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)65) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80) ) ) {
            result[0] += 0.02074688796680498;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9585062240663901;
            result[5] += 0.02074688796680498;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)68.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.02702702702702703;
            result[4] += 0.1891891891891892;
            result[5] += 0.7837837837837838;
          } else {
            result[0] += 0.6557377049180327;
            result[1] += 0.01639344262295082;
            result[2] += 0;
            result[3] += 0.13114754098360656;
            result[4] += 0.18032786885245902;
            result[5] += 0.01639344262295082;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.027777777777777776;
            result[4] += 0.6666666666666666;
            result[5] += 0.3055555555555556;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03985507246376811;
            result[4] += 0.020531400966183576;
            result[5] += 0.9396135265700483;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.010752688172043012;
            result[2] += 0;
            result[3] += 0.0967741935483871;
            result[4] += 0.07526881720430108;
            result[5] += 0.8172043010752689;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.005050505050505051;
            result[3] += 0.5808080808080808;
            result[4] += 0;
            result[5] += 0.41414141414141414;
          }
        }
      }
    } else {
      if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)54) ) ) {
            result[0] += 0.31896551724137934;
            result[1] += 0.5431034482758621;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.13793103448275862;
            result[5] += 0;
          } else {
            result[0] += 0.049079754601227;
            result[1] += 0.055214723926380375;
            result[2] += 0;
            result[3] += 0.018404907975460127;
            result[4] += 0.8527607361963191;
            result[5] += 0.0245398773006135;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
            result[0] += 0.8676975945017184;
            result[1] += 0.006872852233676977;
            result[2] += 0.01202749140893471;
            result[3] += 0.03178694158075602;
            result[4] += 0.03780068728522337;
            result[5] += 0.043814432989690726;
          } else {
            result[0] += 0.12718204488778054;
            result[1] += 0.012468827930174564;
            result[2] += 0.30673316708229426;
            result[3] += 0.2793017456359102;
            result[4] += 0.04987531172069826;
            result[5] += 0.22443890274314215;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)109.5) ) ) {
            result[0] += 0;
            result[1] += 0.5714285714285714;
            result[2] += 0.21428571428571427;
            result[3] += 0.21428571428571427;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.625;
            result[1] += 0.125;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)103) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)81.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)45.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)76) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0967741935483871;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.9032258064516129;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.12195121951219512;
            result[3] += 0.7560975609756098;
            result[4] += 0.024390243902439025;
            result[5] += 0.0975609756097561;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.875;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.125;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.625;
            result[4] += 0;
            result[5] += 0.25;
          } else {
            result[0] += 0.013333333333333336;
            result[1] += 0.013333333333333336;
            result[2] += 0.7733333333333334;
            result[3] += 0.1466666666666667;
            result[4] += 0;
            result[5] += 0.053333333333333344;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)93.5) ) ) {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)76) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0.125;
            result[2] += 0.125;
            result[3] += 0.1875;
            result[4] += 0;
            result[5] += 0.5625;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0.8571428571428571;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)69.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.7142857142857143;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)87.5) ) ) {
            result[0] += 0.012269938650306749;
            result[1] += 0;
            result[2] += 0.8558282208588958;
            result[3] += 0.11349693251533742;
            result[4] += 0.003067484662576687;
            result[5] += 0.015337423312883436;
          } else {
            result[0] += 0.005964214711729622;
            result[1] += 0;
            result[2] += 0.9721669980119284;
            result[3] += 0.019880715705765408;
            result[4] += 0;
            result[5] += 0.0019880715705765406;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)95) ) ) {
            result[0] += 0.005291005291005292;
            result[1] += 0.010582010582010583;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9735449735449736;
            result[5] += 0.010582010582010583;
          } else {
            result[0] += 0;
            result[1] += 0.7692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.23076923076923078;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)77.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0.5;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0.024390243902439025;
            result[2] += 0;
            result[3] += 0.024390243902439025;
            result[4] += 0.6585365853658537;
            result[5] += 0.2926829268292683;
          } else {
            result[0] += 0.005656108597285068;
            result[1] += 0;
            result[2] += 0.003393665158371041;
            result[3] += 0.0418552036199095;
            result[4] += 0.03959276018099547;
            result[5] += 0.9095022624434389;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)101) ) ) {
            result[0] += 0.013029315960912053;
            result[1] += 0.06840390879478828;
            result[2] += 0;
            result[3] += 0.42671009771986973;
            result[4] += 0.026058631921824105;
            result[5] += 0.46579804560260585;
          } else {
            result[0] += 0.04081632653061224;
            result[1] += 0;
            result[2] += 0.6530612244897959;
            result[3] += 0.10204081632653061;
            result[4] += 0.04081632653061224;
            result[5] += 0.16326530612244897;
          }
        }
      }
    } else {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)68.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)101.5) ) ) {
            result[0] += 0.1;
            result[1] += 0.1;
            result[2] += 0.022222222222222223;
            result[3] += 0.03333333333333333;
            result[4] += 0.6944444444444444;
            result[5] += 0.05;
          } else {
            result[0] += 0.02040816326530612;
            result[1] += 0.8775510204081632;
            result[2] += 0.02040816326530612;
            result[3] += 0.08163265306122448;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
            result[0] += 0.829024186822352;
            result[1] += 0.015846538782318603;
            result[2] += 0.017514595496246874;
            result[3] += 0.03669724770642203;
            result[4] += 0.06672226855713095;
            result[5] += 0.034195162635529616;
          } else {
            result[0] += 0.24742268041237114;
            result[1] += 0.005154639175257732;
            result[2] += 0.11855670103092783;
            result[3] += 0.29896907216494845;
            result[4] += 0.12371134020618557;
            result[5] += 0.20618556701030927;
          }
        }
      } else {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)106.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)88.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8571428571428571;
            result[4] += 0;
            result[5] += 0.14285714285714285;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.058823529411764705;
            result[4] += 0.08823529411764706;
            result[5] += 0.8529411764705882;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)81.5) ) ) {
            result[0] += 0;
            result[1] += 0.21428571428571427;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07142857142857142;
            result[5] += 0.7142857142857143;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5555555555555556;
            result[4] += 0;
            result[5] += 0.4444444444444444;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.03508771929824561;
            result[3] += 0.9473684210526315;
            result[4] += 0;
            result[5] += 0.017543859649122806;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2127659574468085;
            result[3] += 0.5957446808510638;
            result[4] += 0;
            result[5] += 0.19148936170212766;
          } else {
            result[0] += 0.05263157894736842;
            result[1] += 0;
            result[2] += 0.6240601503759399;
            result[3] += 0.2706766917293233;
            result[4] += 0;
            result[5] += 0.05263157894736842;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)100) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.2222222222222222;
            result[4] += 0;
            result[5] += 0.7777777777777778;
          } else {
            result[0] += 0.08333333333333333;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0.3333333333333333;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)82) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7573221757322176;
            result[3] += 0.2175732217573222;
            result[4] += 0;
            result[5] += 0.02510460251046025;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)88.5) ) ) {
            result[0] += 0.07692307692307693;
            result[1] += 0;
            result[2] += 0.6923076923076923;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.23076923076923078;
          } else {
            result[0] += 0.0032154340836012866;
            result[1] += 0;
            result[2] += 0.9710610932475885;
            result[3] += 0.025723472668810293;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)77) ) ) {
    if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)47.5) ) ) {
            result[0] += 0.6666666666666666;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.019801980198019802;
            result[1] += 0.006600660066006601;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9570957095709571;
            result[5] += 0.0165016501650165;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)56.5) ) ) {
            result[0] += 0.9367088607594937;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.06329113924050633;
            result[5] += 0;
          } else {
            result[0] += 0.05714285714285714;
            result[1] += 0.24285714285714285;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3142857142857143;
            result[5] += 0.38571428571428573;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03229813664596273;
            result[4] += 0.05217391304347826;
            result[5] += 0.915527950310559;
          } else {
            result[0] += 0.016666666666666666;
            result[1] += 0.18333333333333332;
            result[2] += 0.016666666666666666;
            result[3] += 0.08333333333333333;
            result[4] += 0.38333333333333336;
            result[5] += 0.31666666666666665;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)101.5) ) ) {
            result[0] += 0.02577319587628866;
            result[1] += 0;
            result[2] += 0.020618556701030927;
            result[3] += 0.35051546391752575;
            result[4] += 0.020618556701030927;
            result[5] += 0.5824742268041238;
          } else {
            result[0] += 0.13636363636363635;
            result[1] += 0.022727272727272728;
            result[2] += 0.6136363636363636;
            result[3] += 0.045454545454545456;
            result[4] += 0;
            result[5] += 0.18181818181818182;
          }
        }
      }
    } else {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)100.5) ) ) {
            result[0] += 0.03225806451612904;
            result[1] += 0.11290322580645162;
            result[2] += 0;
            result[3] += 0.14516129032258068;
            result[4] += 0.6774193548387097;
            result[5] += 0.03225806451612904;
          } else {
            result[0] += 0.024096385542168676;
            result[1] += 0.891566265060241;
            result[2] += 0;
            result[3] += 0.012048192771084338;
            result[4] += 0.07228915662650602;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
            result[0] += 0.8760107816711591;
            result[1] += 0.004492362982929022;
            result[2] += 0.0008984725965858043;
            result[3] += 0.022461814914645106;
            result[4] += 0.05390835579514826;
            result[5] += 0.0422282120395328;
          } else {
            result[0] += 0.2857142857142857;
            result[1] += 0.02040816326530612;
            result[2] += 0.1836734693877551;
            result[3] += 0.3299319727891156;
            result[4] += 0.034013605442176874;
            result[5] += 0.14625850340136054;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)92.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)94) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.038461538461538464;
            result[4] += 0;
            result[5] += 0.9615384615384616;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05555555555555555;
            result[3] += 0;
            result[4] += 0.8888888888888888;
            result[5] += 0.05555555555555555;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)86) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.38461538461538464;
            result[4] += 0.3076923076923077;
            result[5] += 0.3076923076923077;
          } else {
            result[0] += 0.1875;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.03125;
            result[5] += 0.78125;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10270270270270271;
            result[3] += 0.6648648648648648;
            result[4] += 0.016216216216216217;
            result[5] += 0.21621621621621623;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5645161290322581;
            result[3] += 0.24193548387096775;
            result[4] += 0;
            result[5] += 0.1935483870967742;
          }
        }
      }
    } else {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)105.5) ) ) {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)91.5) ) ) {
            result[0] += 0.14285714285714285;
            result[1] += 0;
            result[2] += 0.07142857142857142;
            result[3] += 0.42857142857142855;
            result[4] += 0;
            result[5] += 0.35714285714285715;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6460176991150443;
            result[3] += 0.3274336283185841;
            result[4] += 0;
            result[5] += 0.02654867256637168;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7884615384615384;
            result[3] += 0.19230769230769232;
            result[4] += 0;
            result[5] += 0.019230769230769232;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9444444444444445;
            result[3] += 0.04166666666666667;
            result[4] += 0;
            result[5] += 0.01388888888888889;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)99.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0;
            result[4] += 0.16666666666666666;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)84.5) ) ) {
            result[0] += 0.019230769230769232;
            result[1] += 0;
            result[2] += 0.8557692307692307;
            result[3] += 0.11538461538461539;
            result[4] += 0.009615384615384616;
            result[5] += 0;
          } else {
            result[0] += 0.0017391304347826088;
            result[1] += 0;
            result[2] += 0.9739130434782609;
            result[3] += 0.020869565217391306;
            result[4] += 0;
            result[5] += 0.0034782608695652175;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)59.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.011428571428571429;
            result[4] += 0.9885714285714285;
            result[5] += 0;
          } else {
            result[0] += 0.39655172413793105;
            result[1] += 0.034482758620689655;
            result[2] += 0;
            result[3] += 0.017241379310344827;
            result[4] += 0.2413793103448276;
            result[5] += 0.3103448275862069;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.005733944954128441;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.040137614678899085;
            result[4] += 0.06077981651376147;
            result[5] += 0.893348623853211;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)89) ) ) {
            result[0] += 0.3902439024390244;
            result[1] += 0.024390243902439025;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5853658536585366;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.8214285714285715;
            result[2] += 0;
            result[3] += 0.07142857142857144;
            result[4] += 0.07142857142857144;
            result[5] += 0.03571428571428572;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0.015957446808510637;
            result[2] += 0.05851063829787234;
            result[3] += 0.2712765957446808;
            result[4] += 0.02127659574468085;
            result[5] += 0.6329787234042553;
          } else {
            result[0] += 0.02631578947368421;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7105263157894737;
            result[4] += 0.006578947368421052;
            result[5] += 0.2565789473684211;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)87) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)98) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)73.5) ) ) {
            result[0] += 0.1518987341772152;
            result[1] += 0.029535864978902954;
            result[2] += 0.02109704641350211;
            result[3] += 0.20253164556962025;
            result[4] += 0.2911392405063291;
            result[5] += 0.3037974683544304;
          } else {
            result[0] += 0.8547387068201948;
            result[1] += 0.01771479185119575;
            result[2] += 0.005314437555358724;
            result[3] += 0.010628875110717449;
            result[4] += 0.1054030115146147;
            result[5] += 0.006200177147918512;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)89.5) ) ) {
            result[0] += 0.02258064516129032;
            result[1] += 0.04838709677419355;
            result[2] += 0.2903225806451613;
            result[3] += 0.3774193548387097;
            result[4] += 0.01935483870967742;
            result[5] += 0.24193548387096775;
          } else {
            result[0] += 0.7701149425287356;
            result[1] += 0;
            result[2] += 0.05747126436781609;
            result[3] += 0.05747126436781609;
            result[4] += 0.11494252873563218;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)83.5) ) ) {
      if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3;
            result[3] += 0.1;
            result[4] += 0.1;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.5;
          }
        }
      } else {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)103) ) ) {
            result[0] += 0.04225352112676056;
            result[1] += 0.014084507042253521;
            result[2] += 0.15492957746478872;
            result[3] += 0.36619718309859156;
            result[4] += 0.028169014084507043;
            result[5] += 0.39436619718309857;
          } else {
            result[0] += 0.07317073170731707;
            result[1] += 0;
            result[2] += 0.5609756097560976;
            result[3] += 0.36585365853658536;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)114.5) ) ) {
            result[0] += 0.03125;
            result[1] += 0;
            result[2] += 0.75625;
            result[3] += 0.2;
            result[4] += 0;
            result[5] += 0.0125;
          } else {
            result[0] += 0.7333333333333333;
            result[1] += 0;
            result[2] += 0.26666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5593220338983051;
            result[3] += 0.38983050847457634;
            result[4] += 0;
            result[5] += 0.050847457627118654;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.86;
            result[3] += 0.14;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)91) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7000000000000001;
            result[3] += 0.28571428571428575;
            result[4] += 0;
            result[5] += 0.014285714285714287;
          } else {
            result[0] += 0.038461538461538464;
            result[1] += 0;
            result[2] += 0.9423076923076923;
            result[3] += 0.019230769230769232;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9958333333333333;
            result[3] += 0.004166666666666667;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)81) ) ) {
    if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100) ) ) {
            result[0] += 0.011299435028248588;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.96045197740113;
            result[5] += 0.02824858757062147;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)52) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.04285714285714286;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.44285714285714284;
            result[5] += 0.5142857142857142;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)57.5) ) ) {
            result[0] += 0.28571428571428575;
            result[1] += 0.3928571428571429;
            result[2] += 0;
            result[3] += 0.07142857142857144;
            result[4] += 0.14285714285714288;
            result[5] += 0.10714285714285715;
          } else {
            result[0] += 0.002369668246445498;
            result[1] += 0.001184834123222749;
            result[2] += 0.002369668246445498;
            result[3] += 0.05687203791469194;
            result[4] += 0.023696682464454975;
            result[5] += 0.9135071090047393;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
            result[0] += 0.45901639344262296;
            result[1] += 0.21311475409836064;
            result[2] += 0;
            result[3] += 0.01639344262295082;
            result[4] += 0.29508196721311475;
            result[5] += 0.01639344262295082;
          } else {
            result[0] += 0;
            result[1] += 0.009345794392523364;
            result[2] += 0.07165109034267912;
            result[3] += 0.5389408099688473;
            result[4] += 0.03115264797507788;
            result[5] += 0.34890965732087226;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)103) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)65) ) ) {
            result[0] += 0.6646464646464647;
            result[1] += 0.03838383838383839;
            result[2] += 0.0020202020202020206;
            result[3] += 0.026262626262626265;
            result[4] += 0.2545454545454546;
            result[5] += 0.014141414141414144;
          } else {
            result[0] += 0.04728132387706856;
            result[1] += 0.04018912529550828;
            result[2] += 0.1182033096926714;
            result[3] += 0.29550827423167847;
            result[4] += 0.1276595744680851;
            result[5] += 0.37115839243498816;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)77) ) ) {
            result[0] += 0.8798076923076923;
            result[1] += 0.05048076923076923;
            result[2] += 0.015625;
            result[3] += 0.002403846153846154;
            result[4] += 0.05048076923076923;
            result[5] += 0.001201923076923077;
          } else {
            result[0] += 0.19801980198019797;
            result[1] += 0.0693069306930693;
            result[2] += 0.5544554455445544;
            result[3] += 0.0693069306930693;
            result[4] += 0.0693069306930693;
            result[5] += 0.0396039603960396;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)107) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)70) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.4;
            result[1] += 0.2;
            result[2] += 0.4;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)91.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)64.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.08571428571428572;
            result[3] += 0.08571428571428572;
            result[4] += 0;
            result[5] += 0.8285714285714286;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)110.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0;
            result[4] += 0.2222222222222222;
            result[5] += 0.6666666666666666;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)82) ) ) {
            result[0] += 0.015625;
            result[1] += 0;
            result[2] += 0.265625;
            result[3] += 0.640625;
            result[4] += 0;
            result[5] += 0.078125;
          } else {
            result[0] += 0.022222222222222223;
            result[1] += 0;
            result[2] += 0.7111111111111111;
            result[3] += 0.24444444444444444;
            result[4] += 0;
            result[5] += 0.022222222222222223;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)99.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0.7777777777777778;
            result[4] += 0;
            result[5] += 0.05555555555555555;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7142857142857143;
            result[3] += 0.2857142857142857;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.034883720930232565;
            result[1] += 0;
            result[2] += 0.9069767441860466;
            result[3] += 0.04651162790697675;
            result[4] += 0;
            result[5] += 0.011627906976744188;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.02702702702702703;
            result[1] += 0;
            result[2] += 0.8648648648648649;
            result[3] += 0.10810810810810811;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9708265802269044;
            result[3] += 0.024311183144246355;
            result[4] += 0;
            result[5] += 0.004862236628849271;
          } else {
            result[0] += 0.2727272727272727;
            result[1] += 0;
            result[2] += 0.7272727272727273;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)101) ) ) {
            result[0] += 0.01234567901234568;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9341563786008231;
            result[5] += 0.05349794238683128;
          } else {
            result[0] += 0;
            result[1] += 0.65;
            result[2] += 0;
            result[3] += 0.25;
            result[4] += 0.1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)58) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.07142857142857142;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.11904761904761904;
            result[4] += 0.30952380952380953;
            result[5] += 0.5;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 0.1875;
            result[2] += 0;
            result[3] += 0.1875;
            result[4] += 0.53125;
            result[5] += 0.09375;
          } else {
            result[0] += 0.009825327510917031;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.05458515283842795;
            result[4] += 0.034934497816593885;
            result[5] += 0.9006550218340611;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.22972972972972974;
            result[4] += 0;
            result[5] += 0.7702702702702703;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7593984962406015;
            result[4] += 0;
            result[5] += 0.24060150375939848;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)44) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.16296296296296298;
            result[2] += 0;
            result[3] += 0.022222222222222223;
            result[4] += 0.8074074074074075;
            result[5] += 0.007407407407407408;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0.9925;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0075;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)72.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)57.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.027472527472527472;
            result[2] += 0.10989010989010989;
            result[3] += 0.3956043956043956;
            result[4] += 0.03296703296703297;
            result[5] += 0.4340659340659341;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)107) ) ) {
            result[0] += 0.565772669220945;
            result[1] += 0.038314176245210725;
            result[2] += 0.08045977011494253;
            result[3] += 0.1417624521072797;
            result[4] += 0.10472541507024266;
            result[5] += 0.06896551724137931;
          } else {
            result[0] += 0.8808864265927978;
            result[1] += 0.0221606648199446;
            result[2] += 0.04847645429362881;
            result[3] += 0.00554016620498615;
            result[4] += 0.04155124653739612;
            result[5] += 0.0013850415512465374;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)104.5) ) ) {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)94.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.058823529411764705;
            result[3] += 0.4411764705882353;
            result[4] += 0.014705882352941176;
            result[5] += 0.4852941176470588;
          } else {
            result[0] += 0;
            result[1] += 0.0625;
            result[2] += 0.75;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.0625;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8888888888888888;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.1111111111111111;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0.3;
            result[4] += 0;
            result[5] += 0.3;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13636363636363635;
            result[3] += 0.8181818181818182;
            result[4] += 0;
            result[5] += 0.045454545454545456;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0;
            result[4] += 0.4;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.767123287671233;
            result[3] += 0.2123287671232877;
            result[4] += 0;
            result[5] += 0.020547945205479454;
          }
        }
      }
    } else {
      if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0.7142857142857143;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0.14285714285714285;
            result[2] += 0;
            result[3] += 0.42857142857142855;
            result[4] += 0.42857142857142855;
            result[5] += 0;
          } else {
            result[0] += 0.013888888888888888;
            result[1] += 0;
            result[2] += 0.8611111111111112;
            result[3] += 0.11574074074074074;
            result[4] += 0;
            result[5] += 0.009259259259259259;
          }
        }
      } else {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)81) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9579439252336449;
            result[3] += 0.04205607476635514;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9903536977491961;
            result[3] += 0.00964630225080386;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)81) ) ) {
    if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)97.5) ) ) {
            result[0] += 0.01937984496124031;
            result[1] += 0.007751937984496124;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9689922480620154;
            result[5] += 0.003875968992248062;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0.9887640449438202;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.011235955056179775;
            result[5] += 0;
          } else {
            result[0] += 0.14814814814814814;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6296296296296297;
            result[5] += 0.2222222222222222;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10526315789473684;
            result[3] += 0;
            result[4] += 0.7894736842105263;
            result[5] += 0.10526315789473684;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.023746701846965697;
            result[4] += 0.03825857519788918;
            result[5] += 0.9379947229551451;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0.0056179775280898875;
            result[2] += 0;
            result[3] += 0.07865168539325842;
            result[4] += 0.16853932584269662;
            result[5] += 0.7471910112359551;
          } else {
            result[0] += 0.07346938775510205;
            result[1] += 0.0163265306122449;
            result[2] += 0.02857142857142857;
            result[3] += 0.42448979591836733;
            result[4] += 0.11020408163265306;
            result[5] += 0.3469387755102041;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)58.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)49) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.18181818181818182;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8181818181818182;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9957081545064378;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004291845493562232;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)107) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
            result[0] += 0.14156626506024098;
            result[1] += 0.02710843373493976;
            result[2] += 0.0030120481927710845;
            result[3] += 0.39759036144578314;
            result[4] += 0.0783132530120482;
            result[5] += 0.35240963855421686;
          } else {
            result[0] += 0.48956661316211886;
            result[1] += 0.03370786516853933;
            result[2] += 0.17014446227929378;
            result[3] += 0.11556982343499199;
            result[4] += 0.12520064205457468;
            result[5] += 0.06581059390048155;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)86) ) ) {
            result[0] += 0.16666666666666666;
            result[1] += 0.2777777777777778;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5555555555555556;
            result[5] += 0;
          } else {
            result[0] += 0.9403202328966519;
            result[1] += 0.005822416302765647;
            result[2] += 0.021834061135371174;
            result[3] += 0.0014556040756914118;
            result[4] += 0.030567685589519642;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)81.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8484848484848485;
            result[4] += 0;
            result[5] += 0.15151515151515152;
          } else {
            result[0] += 0.3333333333333333;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)99) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)113.5) ) ) {
            result[0] += 0;
            result[1] += 0.03125;
            result[2] += 0.0625;
            result[3] += 0.21875;
            result[4] += 0.125;
            result[5] += 0.5625;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)103) ) ) {
            result[0] += 0.043478260869565216;
            result[1] += 0.13043478260869565;
            result[2] += 0.6956521739130435;
            result[3] += 0.08695652173913043;
            result[4] += 0;
            result[5] += 0.043478260869565216;
          } else {
            result[0] += 0.7692307692307693;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.23076923076923078;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5362318840579711;
            result[3] += 0.3188405797101449;
            result[4] += 0;
            result[5] += 0.14492753623188406;
          } else {
            result[0] += 0.005319148936170213;
            result[1] += 0;
            result[2] += 0.8670212765957447;
            result[3] += 0.12234042553191489;
            result[4] += 0;
            result[5] += 0.005319148936170213;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)93) ) ) {
            result[0] += 0.75;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0030120481927710845;
            result[1] += 0;
            result[2] += 0.9743975903614458;
            result[3] += 0.022590361445783132;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)74.5) ) ) {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)62.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)57) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)96) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9949748743718593;
            result[5] += 0.005025125628140704;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8571428571428571;
            result[5] += 0.14285714285714285;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)70) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9375;
            result[5] += 0.0625;
          }
        }
      }
    } else {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)69.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.09090909090909091;
            result[4] += 0.8181818181818182;
            result[5] += 0.09090909090909091;
          } else {
            result[0] += 0.0012239902080783353;
            result[1] += 0.0024479804161566705;
            result[2] += 0;
            result[3] += 0.02692778457772338;
            result[4] += 0.04039167686658507;
            result[5] += 0.9290085679314566;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05309734513274336;
            result[3] += 0.21238938053097345;
            result[4] += 0.008849557522123894;
            result[5] += 0.7256637168141593;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.9565217391304348;
            result[4] += 0;
            result[5] += 0.043478260869565216;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)58) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0.2222222222222222;
            result[4] += 0.3333333333333333;
            result[5] += 0.1111111111111111;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)83.5) ) ) {
            result[0] += 0.012121212121212121;
            result[1] += 0;
            result[2] += 0.01818181818181818;
            result[3] += 0.20606060606060606;
            result[4] += 0.16363636363636364;
            result[5] += 0.6;
          } else {
            result[0] += 0.008230452674897122;
            result[1] += 0.008230452674897122;
            result[2] += 0.03703703703703704;
            result[3] += 0.54320987654321;
            result[4] += 0.01234567901234568;
            result[5] += 0.3909465020576132;
          }
        }
      }
    }
  } else {
    if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)90.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
            result[0] += 0.013157894736842105;
            result[1] += 0.15789473684210525;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8157894736842105;
            result[5] += 0.013157894736842105;
          } else {
            result[0] += 0.047619047619047616;
            result[1] += 0.7619047619047619;
            result[2] += 0.14285714285714285;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)57) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.6774193548387096;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3225806451612903;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)69) ) ) {
            result[0] += 0.71;
            result[1] += 0.105;
            result[2] += 0;
            result[3] += 0.015;
            result[4] += 0.15;
            result[5] += 0.02;
          } else {
            result[0] += 0.0784313725490196;
            result[1] += 0;
            result[2] += 0.20098039215686275;
            result[3] += 0.28431372549019607;
            result[4] += 0.049019607843137254;
            result[5] += 0.3872549019607843;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)49.5) ) ) {
            result[0] += 0.15384615384615385;
            result[1] += 0.8076923076923077;
            result[2] += 0;
            result[3] += 0.038461538461538464;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.906148867313916;
            result[1] += 0.0075512405609493;
            result[2] += 0.023732470334412087;
            result[3] += 0.012944983818770229;
            result[4] += 0.046386192017259985;
            result[5] += 0.0032362459546925572;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.047619047619047616;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0.9047619047619048;
          } else {
            result[0] += 0.009803921568627449;
            result[1] += 0.05392156862745097;
            result[2] += 0.4901960784313724;
            result[3] += 0.3578431372549019;
            result[4] += 0.019607843137254898;
            result[5] += 0.06862745098039215;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.8333333333333334;
            result[1] += 0.020833333333333332;
            result[2] += 0.125;
            result[3] += 0;
            result[4] += 0.020833333333333332;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.21428571428571427;
            result[3] += 0.7142857142857143;
            result[4] += 0;
            result[5] += 0.07142857142857142;
          } else {
            result[0] += 0.0967741935483871;
            result[1] += 0;
            result[2] += 0.7419354838709677;
            result[3] += 0.14516129032258066;
            result[4] += 0;
            result[5] += 0.016129032258064516;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0.3;
            result[4] += 0;
            result[5] += 0.3;
          } else {
            result[0] += 0.0022753128555176336;
            result[1] += 0;
            result[2] += 0.9488054607508533;
            result[3] += 0.04550625711035267;
            result[4] += 0;
            result[5] += 0.0034129692832764505;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)47) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)45) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.004098360655737705;
            result[1] += 0.004098360655737705;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9795081967213115;
            result[5] += 0.012295081967213115;
          }
        }
      } else {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)58) ) ) {
            result[0] += 0.8837209302325582;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.11627906976744186;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.009871668311944718;
            result[2] += 0.007897334649555774;
            result[3] += 0.0631786771964462;
            result[4] += 0.05725567620927937;
            result[5] += 0.861796643632774;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)56.5) ) ) {
            result[0] += 0.7906976744186046;
            result[1] += 0.06976744186046512;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.13953488372093023;
            result[5] += 0;
          } else {
            result[0] += 0.025454545454545455;
            result[1] += 0.007272727272727273;
            result[2] += 0.03272727272727273;
            result[3] += 0.4909090909090909;
            result[4] += 0.06909090909090909;
            result[5] += 0.37454545454545457;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)83) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)90.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9975961538461539;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002403846153846154;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)107.5) ) ) {
            result[0] += 0.8227124183006536;
            result[1] += 0.015522875816993464;
            result[2] += 0.016339869281045753;
            result[3] += 0.017973856209150325;
            result[4] += 0.10866013071895425;
            result[5] += 0.018790849673202614;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
            result[0] += 0.03333333333333333;
            result[1] += 0.03333333333333333;
            result[2] += 0.05555555555555555;
            result[3] += 0.26666666666666666;
            result[4] += 0.1388888888888889;
            result[5] += 0.4722222222222222;
          } else {
            result[0] += 0.3384615384615385;
            result[1] += 0;
            result[2] += 0.5076923076923077;
            result[3] += 0.09230769230769231;
            result[4] += 0.015384615384615385;
            result[5] += 0.046153846153846156;
          }
        }
      }
    }
  } else {
    if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)94.5) ) ) {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0.23529411764705882;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6470588235294118;
            result[5] += 0.11764705882352941;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0273972602739726;
            result[3] += 0.2054794520547945;
            result[4] += 0.0273972602739726;
            result[5] += 0.7397260273972602;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)105.5) ) ) {
            result[0] += 0.0576923076923077;
            result[1] += 0;
            result[2] += 0.6346153846153847;
            result[3] += 0.07692307692307694;
            result[4] += 0.07692307692307694;
            result[5] += 0.15384615384615388;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)82) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4166666666666667;
            result[3] += 0.08333333333333333;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09433962264150944;
            result[3] += 0.8490566037735849;
            result[4] += 0;
            result[5] += 0.05660377358490566;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)94) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8823529411764706;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.11764705882352941;
          } else {
            result[0] += 0.75;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)82) ) ) {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.23529411764705882;
            result[3] += 0.23529411764705882;
            result[4] += 0;
            result[5] += 0.5294117647058824;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5147058823529412;
            result[3] += 0.4558823529411765;
            result[4] += 0;
            result[5] += 0.029411764705882356;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0.2;
            result[4] += 0;
            result[5] += 0.4;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8837209302325582;
            result[3] += 0.11627906976744186;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)95) ) ) {
            result[0] += 0.004784688995215311;
            result[1] += 0;
            result[2] += 0.8229665071770335;
            result[3] += 0.15311004784688995;
            result[4] += 0;
            result[5] += 0.019138755980861243;
          } else {
            result[0] += 0.5294117647058824;
            result[1] += 0;
            result[2] += 0.47058823529411764;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)86.5) ) ) {
            result[0] += 0.125;
            result[1] += 0;
            result[2] += 0.375;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0.0017953321364452424;
            result[1] += 0;
            result[2] += 0.9748653500897666;
            result[3] += 0.02333931777378815;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)75.5) ) ) {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)60.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9230769230769231;
            result[5] += 0.07692307692307693;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9655172413793104;
            result[5] += 0.034482758620689655;
          } else {
            result[0] += 0.2222222222222222;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4444444444444444;
            result[5] += 0.3333333333333333;
          }
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)78) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8076923076923077;
            result[5] += 0.19230769230769232;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1111111111111111;
            result[5] += 0.5555555555555556;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)58.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7272727272727273;
            result[5] += 0.2727272727272727;
          } else {
            result[0] += 0.001392757660167131;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.036211699164345405;
            result[4] += 0.009749303621169917;
            result[5] += 0.9526462395543176;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)59.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)58) ) ) {
            result[0] += 0.9647058823529412;
            result[1] += 0.011764705882352941;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.023529411764705882;
            result[5] += 0;
          } else {
            result[0] += 0.09375;
            result[1] += 0.09375;
            result[2] += 0;
            result[3] += 0.0625;
            result[4] += 0.625;
            result[5] += 0.125;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)84.5) ) ) {
            result[0] += 0.0061162079510703364;
            result[1] += 0.024464831804281346;
            result[2] += 0.0030581039755351682;
            result[3] += 0.20489296636085627;
            result[4] += 0.07951070336391437;
            result[5] += 0.6819571865443425;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04779411764705882;
            result[3] += 0.6397058823529411;
            result[4] += 0.003676470588235294;
            result[5] += 0.3088235294117647;
          }
        }
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)77) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)48.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)54.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.7142857142857143;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2857142857142857;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)44) ) ) {
            result[0] += 0;
            result[1] += 0.8837209302325582;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.11627906976744186;
            result[5] += 0;
          } else {
            result[0] += 0.0125;
            result[1] += 0.1125;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8375;
            result[5] += 0.0375;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)70.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)49.5) ) ) {
            result[0] += 0.10810810810810811;
            result[1] += 0.7837837837837838;
            result[2] += 0;
            result[3] += 0.08108108108108109;
            result[4] += 0.02702702702702703;
            result[5] += 0;
          } else {
            result[0] += 0.11290322580645161;
            result[1] += 0.03225806451612903;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8225806451612904;
            result[5] += 0.03225806451612903;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)71.5) ) ) {
            result[0] += 0.932994923857868;
            result[1] += 0.007106598984771574;
            result[2] += 0.003045685279187817;
            result[3] += 0.008121827411167513;
            result[4] += 0.03756345177664975;
            result[5] += 0.01116751269035533;
          } else {
            result[0] += 0.41379310344827586;
            result[1] += 0.05747126436781609;
            result[2] += 0.09770114942528736;
            result[3] += 0.20689655172413793;
            result[4] += 0.10919540229885058;
            result[5] += 0.11494252873563218;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)99) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0625;
            result[3] += 0.640625;
            result[4] += 0;
            result[5] += 0.296875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.061946902654867256;
            result[1] += 0.09734513274336283;
            result[2] += 0.25663716814159293;
            result[3] += 0.25663716814159293;
            result[4] += 0.07079646017699115;
            result[5] += 0.25663716814159293;
          } else {
            result[0] += 0.11111111111111112;
            result[1] += 0;
            result[2] += 0.7777777777777779;
            result[3] += 0.0925925925925926;
            result[4] += 0;
            result[5] += 0.01851851851851852;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.46551724137931033;
            result[3] += 0.3620689655172414;
            result[4] += 0;
            result[5] += 0.1724137931034483;
          } else {
            result[0] += 0.01818181818181818;
            result[1] += 0;
            result[2] += 0.8424242424242424;
            result[3] += 0.13333333333333333;
            result[4] += 0;
            result[5] += 0.006060606060606061;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)82.5) ) ) {
            result[0] += 0.12765957446808512;
            result[1] += 0;
            result[2] += 0.7021276595744682;
            result[3] += 0.12765957446808512;
            result[4] += 0;
            result[5] += 0.04255319148936171;
          } else {
            result[0] += 0.0027739251040221915;
            result[1] += 0;
            result[2] += 0.9625520110957004;
            result[3] += 0.026352288488210817;
            result[4] += 0;
            result[5] += 0.008321775312066574;
          }
        }
      }
    }
  }
  if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)81) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)64.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9899497487437185;
            result[5] += 0.010050251256281407;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
            result[0] += 0.9565217391304348;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.043478260869565216;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6956521739130435;
            result[5] += 0.30434782608695654;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5918367346938775;
            result[5] += 0.40816326530612246;
          } else {
            result[0] += 0.0012345679012345679;
            result[1] += 0.0012345679012345679;
            result[2] += 0;
            result[3] += 0.05308641975308642;
            result[4] += 0.037037037037037035;
            result[5] += 0.9074074074074074;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)84.5) ) ) {
            result[0] += 0.044642857142857144;
            result[1] += 0.05357142857142857;
            result[2] += 0.008928571428571428;
            result[3] += 0.26339285714285715;
            result[4] += 0.044642857142857144;
            result[5] += 0.5848214285714286;
          } else {
            result[0] += 0.013422818791946308;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7516778523489933;
            result[4] += 0.006711409395973154;
            result[5] += 0.22818791946308725;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)50) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)84) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)41) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0.2857142857142857;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)68.5) ) ) {
            result[0] += 0.17687074829931976;
            result[1] += 0.108843537414966;
            result[2] += 0;
            result[3] += 0.0272108843537415;
            result[4] += 0.6190476190476192;
            result[5] += 0.06802721088435376;
          } else {
            result[0] += 0.8394758394758395;
            result[1] += 0.030303030303030304;
            result[2] += 0.012285012285012284;
            result[3] += 0.03357903357903358;
            result[4] += 0.062244062244062245;
            result[5] += 0.022113022113022112;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
            result[0] += 0.05416666666666667;
            result[1] += 0;
            result[2] += 0.025;
            result[3] += 0.3458333333333333;
            result[4] += 0.17083333333333334;
            result[5] += 0.4041666666666667;
          } else {
            result[0] += 0.27428571428571435;
            result[1] += 0;
            result[2] += 0.5714285714285715;
            result[3] += 0.09142857142857144;
            result[4] += 0.01142857142857143;
            result[5] += 0.051428571428571435;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)103) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)91.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)110.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)118.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.21153846153846154;
            result[4] += 0.057692307692307696;
            result[5] += 0.7307692307692307;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0816326530612245;
            result[3] += 0.7551020408163266;
            result[4] += 0;
            result[5] += 0.163265306122449;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8235294117647058;
            result[3] += 0.11764705882352941;
            result[4] += 0;
            result[5] += 0.058823529411764705;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7558139534883721;
            result[3] += 0.18604651162790697;
            result[4] += 0;
            result[5] += 0.05813953488372093;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)77) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)112) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)90) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3333333333333333;
            result[4] += 0.3333333333333333;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0.1111111111111111;
            result[1] += 0;
            result[2] += 0.8888888888888888;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.017142857142857144;
            result[1] += 0;
            result[2] += 0.7771428571428571;
            result[3] += 0.18857142857142858;
            result[4] += 0;
            result[5] += 0.017142857142857144;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9166666666666666;
            result[3] += 0.08333333333333333;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.002777777777777778;
            result[1] += 0;
            result[2] += 0.9944444444444445;
            result[3] += 0.002777777777777778;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)90.5) ) ) {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)100) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.02074688796680498;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.970954356846473;
            result[5] += 0.008298755186721992;
          } else {
            result[0] += 0.07446808510638298;
            result[1] += 0.010638297872340425;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7021276595744681;
            result[5] += 0.2127659574468085;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0.9545454545454546;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.045454545454545456;
            result[5] += 0;
          } else {
            result[0] += 0.023809523809523808;
            result[1] += 0.11904761904761904;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8571428571428571;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)57.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.76;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.24;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)73.5) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)58) ) ) {
            result[0] += 0.8095238095238095;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.19047619047619047;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0012468827930174563;
            result[3] += 0.04987531172069826;
            result[4] += 0.034912718204488775;
            result[5] += 0.9139650872817955;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)87.5) ) ) {
            result[0] += 0.03463203463203463;
            result[1] += 0.025974025974025976;
            result[2] += 0.017316017316017316;
            result[3] += 0.18181818181818182;
            result[4] += 0.06926406926406926;
            result[5] += 0.670995670995671;
          } else {
            result[0] += 0.07420494699646643;
            result[1] += 0.014134275618374558;
            result[2] += 0.06713780918727916;
            result[3] += 0.5759717314487632;
            result[4] += 0.0176678445229682;
            result[5] += 0.2508833922261484;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)65) ) ) {
            result[0] += 0.8883720930232558;
            result[1] += 0.037209302325581395;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.046511627906976744;
            result[5] += 0.027906976744186046;
          } else {
            result[0] += 0.0851063829787234;
            result[1] += 0.014184397163120567;
            result[2] += 0.11702127659574468;
            result[3] += 0.42907801418439717;
            result[4] += 0.1099290780141844;
            result[5] += 0.24468085106382978;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)75.5) ) ) {
            result[0] += 0.8940809968847353;
            result[1] += 0.01038421599169263;
            result[2] += 0;
            result[3] += 0.013499480789200416;
            result[4] += 0.06542056074766356;
            result[5] += 0.016614745586708207;
          } else {
            result[0] += 0.3076923076923077;
            result[1] += 0.05128205128205128;
            result[2] += 0.4017094017094017;
            result[3] += 0.11965811965811966;
            result[4] += 0.042735042735042736;
            result[5] += 0.07692307692307693;
          }
        }
      }
    }
  } else {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)99.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.019230769230769232;
            result[3] += 0.46153846153846156;
            result[4] += 0.057692307692307696;
            result[5] += 0.46153846153846156;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2903225806451613;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.7096774193548387;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)107) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.29268292682926833;
            result[3] += 0.5853658536585367;
            result[4] += 0;
            result[5] += 0.12195121951219513;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)103.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)91) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09523809523809523;
            result[3] += 0.8571428571428571;
            result[4] += 0;
            result[5] += 0.047619047619047616;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)115.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7727272727272727;
            result[3] += 0.18181818181818182;
            result[4] += 0;
            result[5] += 0.045454545454545456;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)122) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)84) ) ) {
            result[0] += 0.005050505050505051;
            result[1] += 0;
            result[2] += 0.8484848484848485;
            result[3] += 0.1414141414141414;
            result[4] += 0;
            result[5] += 0.005050505050505051;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9563025210084034;
            result[3] += 0.03361344537815126;
            result[4] += 0.0033613445378151263;
            result[5] += 0.0067226890756302525;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)60.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0.0056179775280898875;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9943820224719101;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)56.5) ) ) {
            result[0] += 0.9285714285714286;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07142857142857142;
            result[5] += 0;
          } else {
            result[0] += 0.1016949152542373;
            result[1] += 0;
            result[2] += 0.01694915254237288;
            result[3] += 0;
            result[4] += 0.5423728813559322;
            result[5] += 0.3389830508474576;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0.025;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.875;
            result[5] += 0.1;
          } else {
            result[0] += 0;
            result[1] += 0.006904487917146145;
            result[2] += 0.0034522439585730723;
            result[3] += 0.04948216340621404;
            result[4] += 0.02531645569620253;
            result[5] += 0.9148446490218642;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.09929078014184398;
            result[2] += 0.014184397163120567;
            result[3] += 0.12056737588652482;
            result[4] += 0.11347517730496454;
            result[5] += 0.6524822695035462;
          } else {
            result[0] += 0.004048582995951417;
            result[1] += 0;
            result[2] += 0.09716599190283401;
            result[3] += 0.6477732793522267;
            result[4] += 0.004048582995951417;
            result[5] += 0.24696356275303644;
          }
        }
      }
    } else {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)54.5) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)90.5) ) ) {
            result[0] += 0;
            result[1] += 0.11538461538461539;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8076923076923077;
            result[5] += 0.07692307692307693;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)91.5) ) ) {
            result[0] += 0.12903225806451613;
            result[1] += 0.6451612903225806;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.22580645161290322;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)73) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)70.5) ) ) {
            result[0] += 0.18;
            result[1] += 0.15333333333333332;
            result[2] += 0;
            result[3] += 0.006666666666666667;
            result[4] += 0.66;
            result[5] += 0;
          } else {
            result[0] += 0.8829588014981273;
            result[1] += 0.011235955056179775;
            result[2] += 0;
            result[3] += 0.03745318352059925;
            result[4] += 0.05337078651685393;
            result[5] += 0.0149812734082397;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)84) ) ) {
            result[0] += 0.02040816326530612;
            result[1] += 0.0663265306122449;
            result[2] += 0.11224489795918367;
            result[3] += 0.37755102040816324;
            result[4] += 0.10204081632653061;
            result[5] += 0.32142857142857145;
          } else {
            result[0] += 0.48344370860927155;
            result[1] += 0.052980132450331126;
            result[2] += 0.3443708609271523;
            result[3] += 0.033112582781456956;
            result[4] += 0.052980132450331126;
            result[5] += 0.033112582781456956;
          }
        }
      }
    }
  } else {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03389830508474576;
            result[4] += 0.1694915254237288;
            result[5] += 0.7966101694915254;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3409090909090909;
            result[4] += 0.022727272727272728;
            result[5] += 0.6363636363636364;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1886792452830189;
            result[3] += 0.6509433962264152;
            result[4] += 0;
            result[5] += 0.16037735849056606;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)75.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5714285714285714;
            result[3] += 0.2857142857142857;
            result[4] += 0;
            result[5] += 0.14285714285714285;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9183673469387755;
            result[3] += 0.061224489795918366;
            result[4] += 0;
            result[5] += 0.02040816326530612;
          }
        }
      }
    } else {
      if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)82) ) ) {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)107) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7076923076923077;
            result[3] += 0.2153846153846154;
            result[4] += 0;
            result[5] += 0.07692307692307693;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.43137254901960786;
            result[3] += 0.49019607843137253;
            result[4] += 0;
            result[5] += 0.0784313725490196;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)93) ) ) {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9354838709677419;
            result[3] += 0.06451612903225806;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)124.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0.5714285714285714;
            result[4] += 0;
            result[5] += 0.14285714285714285;
          } else {
            result[0] += 0.008053691275167786;
            result[1] += 0;
            result[2] += 0.9328859060402684;
            result[3] += 0.05100671140939597;
            result[4] += 0.004026845637583893;
            result[5] += 0.004026845637583893;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)82) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)96) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0.005025125628140704;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9798994974874372;
            result[5] += 0.01507537688442211;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)54.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6730769230769231;
            result[5] += 0.3269230769230769;
          }
        }
      } else {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)57.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)57.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.11111111111111112;
            result[4] += 0.6666666666666667;
            result[5] += 0.22222222222222224;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)63.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.0022624434389140274;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03959276018099547;
            result[4] += 0.030542986425339366;
            result[5] += 0.9276018099547512;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)57.5) ) ) {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0.037037037037037035;
            result[2] += 0.12962962962962962;
            result[3] += 0.18518518518518517;
            result[4] += 0.05555555555555555;
            result[5] += 0.5925925925925926;
          } else {
            result[0] += 0.008620689655172415;
            result[1] += 0.004310344827586208;
            result[2] += 0.04741379310344828;
            result[3] += 0.6034482758620691;
            result[4] += 0.03448275862068966;
            result[5] += 0.30172413793103453;
          }
        }
      }
    }
  } else {
    if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)77) ) ) {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)67.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)83) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          } else {
            result[0] += 0.0024390243902439024;
            result[1] += 0.9853658536585366;
            result[2] += 0.004878048780487805;
            result[3] += 0.007317073170731708;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)67.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)99.5) ) ) {
            result[0] += 0.07627118644067797;
            result[1] += 0.025423728813559324;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8898305084745762;
            result[5] += 0.00847457627118644;
          } else {
            result[0] += 0.07142857142857142;
            result[1] += 0.8809523809523809;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.047619047619047616;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
            result[0] += 0.9043103448275862;
            result[1] += 0.00603448275862069;
            result[2] += 0.0008620689655172414;
            result[3] += 0.017241379310344827;
            result[4] += 0.05517241379310345;
            result[5] += 0.016379310344827588;
          } else {
            result[0] += 0.14057507987220447;
            result[1] += 0.025559105431309903;
            result[2] += 0.1853035143769968;
            result[3] += 0.22364217252396165;
            result[4] += 0.06070287539936102;
            result[5] += 0.36421725239616615;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.16;
            result[4] += 0.16;
            result[5] += 0.68;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.15789473684210525;
            result[3] += 0.7017543859649122;
            result[4] += 0;
            result[5] += 0.14035087719298245;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.20689655172413793;
            result[3] += 0.5172413793103449;
            result[4] += 0;
            result[5] += 0.27586206896551724;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7258064516129032;
            result[3] += 0.25806451612903225;
            result[4] += 0;
            result[5] += 0.016129032258064516;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.6451612903225806;
            result[1] += 0.03225806451612903;
            result[2] += 0.22580645161290322;
            result[3] += 0;
            result[4] += 0.06451612903225806;
            result[5] += 0.03225806451612903;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0.010169491525423728;
            result[1] += 0;
            result[2] += 0.9378531073446328;
            result[3] += 0.02937853107344633;
            result[4] += 0;
            result[5] += 0.022598870056497175;
          }
        }
      }
    }
  }
  if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)72.5) ) ) {
    if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9900990099009901;
            result[5] += 0.009900990099009901;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)54.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)72.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8666666666666667;
          } else {
            result[0] += 0.16216216216216217;
            result[1] += 0.02702702702702703;
            result[2] += 0;
            result[3] += 0.02702702702702703;
            result[4] += 0.4594594594594595;
            result[5] += 0.32432432432432434;
          }
        }
      }
    } else {
      if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)67.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)59) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)63.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8;
            result[5] += 0.2;
          } else {
            result[0] += 0.002635046113306983;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.04216073781291173;
            result[4] += 0.017127799736495388;
            result[5] += 0.9380764163372859;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)56.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)65.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.09826589595375723;
            result[2] += 0;
            result[3] += 0.09826589595375723;
            result[4] += 0.09248554913294797;
            result[5] += 0.7109826589595376;
          } else {
            result[0] += 0;
            result[1] += 0.003663003663003663;
            result[2] += 0.08058608058608059;
            result[3] += 0.5457875457875457;
            result[4] += 0.01098901098901099;
            result[5] += 0.358974358974359;
          }
        }
      }
    }
  } else {
    if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)77) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)88) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)51.5) ) ) {
            result[0] += 0;
            result[1] += 0.9976689976689976;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002331002331002331;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)68.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0.05405405405405406;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9459459459459459;
            result[5] += 0;
          } else {
            result[0] += 0.25263157894736843;
            result[1] += 0.28421052631578947;
            result[2] += 0.031578947368421054;
            result[3] += 0.15789473684210525;
            result[4] += 0.24210526315789474;
            result[5] += 0.031578947368421054;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
            result[0] += 0.8676081200353045;
            result[1] += 0.00706090026478376;
            result[2] += 0.00088261253309797;
            result[3] += 0.02206531332744925;
            result[4] += 0.07149161518093557;
            result[5] += 0.03089143865842895;
          } else {
            result[0] += 0.3793103448275862;
            result[1] += 0;
            result[2] += 0.13793103448275862;
            result[3] += 0.2561576354679803;
            result[4] += 0.04926108374384237;
            result[5] += 0.17733990147783252;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)64) ) ) {
            result[0] += 0;
            result[1] += 0.25;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.75;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.04054054054054054;
            result[2] += 0;
            result[3] += 0.12162162162162163;
            result[4] += 0.0945945945945946;
            result[5] += 0.7432432432432432;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1956521739130435;
            result[3] += 0.5869565217391305;
            result[4] += 0.028985507246376812;
            result[5] += 0.18840579710144928;
          } else {
            result[0] += 0.021897810218978107;
            result[1] += 0;
            result[2] += 0.613138686131387;
            result[3] += 0.3284671532846716;
            result[4] += 0.007299270072992701;
            result[5] += 0.029197080291970805;
          }
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0.1111111111111111;
            result[2] += 0.05555555555555555;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8333333333333334;
          } else {
            result[0] += 0.022222222222222223;
            result[1] += 0.06666666666666667;
            result[2] += 0.5333333333333333;
            result[3] += 0.1111111111111111;
            result[4] += 0.1111111111111111;
            result[5] += 0.15555555555555556;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
            result[0] += 0.29629629629629634;
            result[1] += 0;
            result[2] += 0.3703703703703704;
            result[3] += 0.11111111111111113;
            result[4] += 0.03703703703703704;
            result[5] += 0.1851851851851852;
          } else {
            result[0] += 0.0059101654846335705;
            result[1] += 0;
            result[2] += 0.9326241134751774;
            result[3] += 0.056737588652482275;
            result[4] += 0;
            result[5] += 0.0047281323877068565;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)67.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)99) ) ) {
            result[0] += 0.005291005291005291;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9523809523809523;
            result[5] += 0.042328042328042326;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)54) ) ) {
            result[0] += 0.9428571428571428;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.05714285714285714;
            result[5] += 0;
          } else {
            result[0] += 0.0821917808219178;
            result[1] += 0;
            result[2] += 0.0410958904109589;
            result[3] += 0.1506849315068493;
            result[4] += 0.5068493150684932;
            result[5] += 0.2191780821917808;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.24358974358974358;
            result[2] += 0;
            result[3] += 0.02564102564102564;
            result[4] += 0.5;
            result[5] += 0.23076923076923078;
          } else {
            result[0] += 0;
            result[1] += 0.0034762456546929316;
            result[2] += 0;
            result[3] += 0.047508690614136734;
            result[4] += 0.03476245654692932;
            result[5] += 0.9142526071842411;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
            result[0] += 0.01639344262295082;
            result[1] += 0;
            result[2] += 0.01639344262295082;
            result[3] += 0.09836065573770492;
            result[4] += 0.16393442622950818;
            result[5] += 0.7049180327868853;
          } else {
            result[0] += 0.01556420233463035;
            result[1] += 0;
            result[2] += 0.08171206225680934;
            result[3] += 0.5252918287937743;
            result[4] += 0.007782101167315175;
            result[5] += 0.36964980544747084;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)55.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)82.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)114) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.043478260869565216;
            result[4] += 0.9565217391304348;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0.7959183673469388;
            result[2] += 0;
            result[3] += 0.061224489795918366;
            result[4] += 0.14285714285714285;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)68.5) ) ) {
            result[0] += 0.15384615384615385;
            result[1] += 0.30177514792899407;
            result[2] += 0;
            result[3] += 0.01775147928994083;
            result[4] += 0.48520710059171596;
            result[5] += 0.04142011834319527;
          } else {
            result[0] += 0.8642732049036778;
            result[1] += 0.0035026269702276708;
            result[2] += 0.0008756567425569177;
            result[3] += 0.028021015761821366;
            result[4] += 0.063922942206655;
            result[5] += 0.03940455341506129;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89.5) ) ) {
            result[0] += 0.018867924528301886;
            result[1] += 0.04716981132075472;
            result[2] += 0.2641509433962264;
            result[3] += 0.3867924528301887;
            result[4] += 0.02830188679245283;
            result[5] += 0.25471698113207547;
          } else {
            result[0] += 0.8076923076923077;
            result[1] += 0.019230769230769232;
            result[2] += 0.125;
            result[3] += 0;
            result[4] += 0.04807692307692308;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)74.5) ) ) {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.19444444444444445;
            result[4] += 0;
            result[5] += 0.8055555555555556;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.875;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0.125;
            result[2] += 0.25;
            result[3] += 0.0625;
            result[4] += 0.25;
            result[5] += 0.3125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6764705882352942;
            result[3] += 0.29411764705882354;
            result[4] += 0;
            result[5] += 0.029411764705882353;
          }
        }
      }
    } else {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)84) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7377049180327869;
            result[3] += 0.25136612021857924;
            result[4] += 0;
            result[5] += 0.01092896174863388;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)74) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9577464788732395;
            result[3] += 0.028169014084507046;
            result[4] += 0;
            result[5] += 0.014084507042253523;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)76) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8760330578512396;
            result[3] += 0.11570247933884298;
            result[4] += 0;
            result[5] += 0.008264462809917356;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8533333333333334;
            result[3] += 0.13333333333333333;
            result[4] += 0;
            result[5] += 0.013333333333333334;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9833333333333333;
            result[3] += 0.016666666666666666;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)81) ) ) {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)45.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)81.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0.9953917050691244;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004608294930875576;
            result[5] += 0;
          } else {
            result[0] += 0.5714285714285714;
            result[1] += 0.42857142857142855;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.013513513513513514;
            result[3] += 0;
            result[4] += 0.9797297297297297;
            result[5] += 0.006756756756756757;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.4444444444444444;
            result[2] += 0;
            result[3] += 0.2222222222222222;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5238095238095238;
            result[5] += 0.47619047619047616;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.023776223776223775;
            result[4] += 0.011188811188811189;
            result[5] += 0.965034965034965;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)86) ) ) {
            result[0] += 0.13157894736842105;
            result[1] += 0.043859649122807015;
            result[2] += 0;
            result[3] += 0.11695906432748537;
            result[4] += 0.15789473684210525;
            result[5] += 0.5497076023391813;
          } else {
            result[0] += 0.012145748987854251;
            result[1] += 0.024291497975708502;
            result[2] += 0.03643724696356275;
            result[3] += 0.5951417004048583;
            result[4] += 0.016194331983805668;
            result[5] += 0.3157894736842105;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)71.5) ) ) {
            result[0] += 0.8604206500956024;
            result[1] += 0.013384321223709372;
            result[2] += 0.0019120458891013386;
            result[3] += 0.01816443594646272;
            result[4] += 0.08795411089866158;
            result[5] += 0.01816443594646272;
          } else {
            result[0] += 0.20540540540540542;
            result[1] += 0.016216216216216217;
            result[2] += 0.0972972972972973;
            result[3] += 0.12972972972972974;
            result[4] += 0.1837837837837838;
            result[5] += 0.3675675675675676;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)109.5) ) ) {
            result[0] += 0.020477815699658702;
            result[1] += 0.023890784982935155;
            result[2] += 0.3037542662116041;
            result[3] += 0.35494880546075086;
            result[4] += 0.017064846416382253;
            result[5] += 0.27986348122866894;
          } else {
            result[0] += 0.7777777777777778;
            result[1] += 0;
            result[2] += 0.08080808080808081;
            result[3] += 0;
            result[4] += 0.1414141414141414;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)75.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)91) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.027777777777777776;
            result[3] += 0;
            result[4] += 0.027777777777777776;
            result[5] += 0.9444444444444444;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.33333333333333337;
            result[3] += 0.5000000000000001;
            result[4] += 0;
            result[5] += 0.16666666666666669;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        }
      }
    } else {
      if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)83.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)91) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0.0072992700729927005;
            result[2] += 0.7226277372262774;
            result[3] += 0.24087591240875914;
            result[4] += 0;
            result[5] += 0.029197080291970802;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)60) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)108.5) ) ) {
            result[0] += 0.005802707930367505;
            result[1] += 0;
            result[2] += 0.9013539651837524;
            result[3] += 0.08704061895551257;
            result[4] += 0;
            result[5] += 0.005802707930367505;
          } else {
            result[0] += 0.003424657534246575;
            result[1] += 0;
            result[2] += 0.9931506849315068;
            result[3] += 0.003424657534246575;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)47) ) ) {
      if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)71) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0.9902200488997555;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.009779951100244499;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)65) ) ) {
            result[0] += 0.02531645569620253;
            result[1] += 0.008438818565400843;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9578059071729957;
            result[5] += 0.008438818565400843;
          } else {
            result[0] += 0.4;
            result[1] += 0.041025641025641026;
            result[2] += 0;
            result[3] += 0.05128205128205128;
            result[4] += 0.2512820512820513;
            result[5] += 0.2564102564102564;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)84.5) ) ) {
            result[0] += 0.005089058524173028;
            result[1] += 0;
            result[2] += 0.003816793893129771;
            result[3] += 0.03307888040712468;
            result[4] += 0.06361323155216285;
            result[5] += 0.8944020356234097;
          } else {
            result[0] += 0.017241379310344827;
            result[1] += 0.008620689655172414;
            result[2] += 0.01206896551724138;
            result[3] += 0.38620689655172413;
            result[4] += 0.05344827586206897;
            result[5] += 0.5224137931034483;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)67.5) ) ) {
            result[0] += 0.10975609756097561;
            result[1] += 0.4146341463414634;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4451219512195122;
            result[5] += 0.03048780487804878;
          } else {
            result[0] += 0.8609625668449198;
            result[1] += 0.008912655971479501;
            result[2] += 0.012477718360071301;
            result[3] += 0.03208556149732621;
            result[4] += 0.0570409982174688;
            result[5] += 0.0285204991087344;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0.06451612903225806;
            result[2] += 0.17204301075268819;
            result[3] += 0.5161290322580645;
            result[4] += 0.010752688172043012;
            result[5] += 0.23655913978494625;
          } else {
            result[0] += 0.16176470588235295;
            result[1] += 0.04411764705882353;
            result[2] += 0.5294117647058824;
            result[3] += 0.04411764705882353;
            result[4] += 0.10294117647058823;
            result[5] += 0.11764705882352941;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)102.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)113.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.6;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0196078431372549;
            result[3] += 0.8823529411764706;
            result[4] += 0;
            result[5] += 0.09803921568627451;
          } else {
            result[0] += 0.14285714285714285;
            result[1] += 0.14285714285714285;
            result[2] += 0.5714285714285714;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.14285714285714285;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0.034482758620689655;
            result[2] += 0.3793103448275862;
            result[3] += 0.41379310344827586;
            result[4] += 0;
            result[5] += 0.1724137931034483;
          } else {
            result[0] += 0;
            result[1] += 0.14285714285714285;
            result[2] += 0.8571428571428571;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)84) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)117.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7394366197183099;
            result[3] += 0.21830985915492956;
            result[4] += 0;
            result[5] += 0.04225352112676056;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.375;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)112.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9420289855072465;
            result[3] += 0.014492753623188408;
            result[4] += 0.014492753623188408;
            result[5] += 0.028985507246376815;
          } else {
            result[0] += 0.21428571428571427;
            result[1] += 0;
            result[2] += 0.5714285714285714;
            result[3] += 0.21428571428571427;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)77) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.8461538461538461;
            result[1] += 0;
            result[2] += 0.15384615384615385;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)63.5) ) ) {
            result[0] += 0.6666666666666666;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.00425531914893617;
            result[1] += 0;
            result[2] += 0.9744680851063829;
            result[3] += 0.019858156028368795;
            result[4] += 0;
            result[5] += 0.0014184397163120568;
          }
        }
      }
    }
  }
  if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)94.5) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)98.5) ) ) {
      if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)68.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)102) ) ) {
            result[0] += 0.07086614173228346;
            result[1] += 0.03937007874015748;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8398950131233596;
            result[5] += 0.049868766404199474;
          } else {
            result[0] += 0;
            result[1] += 0.8723404255319149;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1276595744680851;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)66.5) ) ) {
            result[0] += 0.24691358024691357;
            result[1] += 0.12345679012345678;
            result[2] += 0.012345679012345678;
            result[3] += 0.16049382716049382;
            result[4] += 0.345679012345679;
            result[5] += 0.1111111111111111;
          } else {
            result[0] += 0.9258160237388725;
            result[1] += 0.0029673590504451044;
            result[2] += 0.0029673590504451044;
            result[3] += 0.011869436201780418;
            result[4] += 0.014836795252225522;
            result[5] += 0.04154302670623146;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7096774193548387;
            result[5] += 0.2903225806451613;
          } else {
            result[0] += 0.0010471204188481676;
            result[1] += 0.0010471204188481676;
            result[2] += 0.0031413612565445027;
            result[3] += 0.05445026178010471;
            result[4] += 0.07329842931937172;
            result[5] += 0.8670157068062827;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)65) ) ) {
            result[0] += 0.6229508196721312;
            result[1] += 0.1557377049180328;
            result[2] += 0.02459016393442623;
            result[3] += 0.00819672131147541;
            result[4] += 0.1885245901639344;
            result[5] += 0;
          } else {
            result[0] += 0.04401408450704225;
            result[1] += 0.0017605633802816902;
            result[2] += 0.06338028169014084;
            result[3] += 0.4278169014084507;
            result[4] += 0.08450704225352113;
            result[5] += 0.3785211267605634;
          }
        }
      }
    } else {
      if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)95) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)106) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)63.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.05555555555555555;
            result[1] += 0.9444444444444444;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)75.5) ) ) {
      if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)75) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)81) ) ) {
            result[0] += 0.9824046920821115;
            result[1] += 0;
            result[2] += 0.005865102639296188;
            result[3] += 0.001466275659824047;
            result[4] += 0.010263929618768328;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3333333333333333;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)127.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0625;
            result[3] += 0.8333333333333334;
            result[4] += 0;
            result[5] += 0.10416666666666667;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)99.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.42424242424242425;
            result[3] += 0.45454545454545453;
            result[4] += 0;
            result[5] += 0.12121212121212122;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8620689655172414;
            result[3] += 0.10344827586206898;
            result[4] += 0;
            result[5] += 0.03448275862068966;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)90) ) ) {
            result[0] += 0.022222222222222223;
            result[1] += 0;
            result[2] += 0.5111111111111111;
            result[3] += 0.3111111111111111;
            result[4] += 0;
            result[5] += 0.15555555555555556;
          } else {
            result[0] += 0.9411764705882353;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.058823529411764705;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7736625514403292;
            result[3] += 0.1934156378600823;
            result[4] += 0;
            result[5] += 0.03292181069958848;
          } else {
            result[0] += 0.009358288770053475;
            result[1] += 0;
            result[2] += 0.9518716577540107;
            result[3] += 0.03074866310160428;
            result[4] += 0;
            result[5] += 0.008021390374331552;
          }
        }
      }
    }
  }
  if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)91.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
            result[0] += 0.005934718100890208;
            result[1] += 0.017804154302670624;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.913946587537092;
            result[5] += 0.06231454005934718;
          } else {
            result[0] += 0.014084507042253521;
            result[1] += 0.7183098591549296;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2676056338028169;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)66) ) ) {
            result[0] += 0.045454545454545456;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.13636363636363635;
            result[5] += 0.8181818181818182;
          } else {
            result[0] += 0.9026315789473685;
            result[1] += 0.002631578947368421;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0868421052631579;
            result[5] += 0.007894736842105263;
          }
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.02040816326530612;
            result[2] += 0.04081632653061224;
            result[3] += 0.04081632653061224;
            result[4] += 0.673469387755102;
            result[5] += 0.22448979591836735;
          } else {
            result[0] += 0.00261437908496732;
            result[1] += 0;
            result[2] += 0.00130718954248366;
            result[3] += 0.03790849673202614;
            result[4] += 0.04183006535947712;
            result[5] += 0.9163398692810457;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)78) ) ) {
            result[0] += 0.03508771929824562;
            result[1] += 0.10087719298245615;
            result[2] += 0.03508771929824562;
            result[3] += 0.07017543859649124;
            result[4] += 0.2368421052631579;
            result[5] += 0.5219298245614036;
          } else {
            result[0] += 0.10456273764258556;
            result[1] += 0.022813688212927757;
            result[2] += 0.0076045627376425855;
            result[3] += 0.45627376425855515;
            result[4] += 0.057034220532319393;
            result[5] += 0.3517110266159696;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)91) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)35.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)70.5) ) ) {
            result[0] += 0.2;
            result[1] += 0.2;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.14285714285714285;
            result[2] += 0;
            result[3] += 0.8571428571428571;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)57) ) ) {
            result[0] += 0.11764705882352941;
            result[1] += 0.8823529411764706;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)75.5) ) ) {
      if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)77) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)99) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
            result[0] += 0.9840579710144928;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.002898550724637681;
            result[4] += 0.013043478260869565;
            result[5] += 0;
          } else {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0.3;
            result[3] += 0.1;
            result[4] += 0.1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)108.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8666666666666667;
            result[4] += 0;
            result[5] += 0.13333333333333333;
          } else {
            result[0] += 0.06666666666666667;
            result[1] += 0;
            result[2] += 0.6444444444444445;
            result[3] += 0.06666666666666667;
            result[4] += 0;
            result[5] += 0.2222222222222222;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.05555555555555555;
            result[4] += 0;
            result[5] += 0.9444444444444444;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.725;
            result[4] += 0;
            result[5] += 0.175;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.631578947368421;
            result[4] += 0;
            result[5] += 0.3684210526315789;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7164179104477612;
            result[3] += 0.208955223880597;
            result[4] += 0;
            result[5] += 0.07462686567164178;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)82.5) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)114.5) ) ) {
            result[0] += 0.017341040462427744;
            result[1] += 0;
            result[2] += 0.6994219653179191;
            result[3] += 0.18497109826589594;
            result[4] += 0.011560693641618497;
            result[5] += 0.08670520231213873;
          } else {
            result[0] += 0.7209302325581395;
            result[1] += 0;
            result[2] += 0.18604651162790697;
            result[3] += 0.023255813953488372;
            result[4] += 0.06976744186046512;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
            result[0] += 0.019801980198019802;
            result[1] += 0;
            result[2] += 0.7871287128712872;
            result[3] += 0.16831683168316833;
            result[4] += 0;
            result[5] += 0.024752475247524754;
          } else {
            result[0] += 0.005943536404160475;
            result[1] += 0;
            result[2] += 0.9450222882615156;
            result[3] += 0.04903417533432392;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)81) ) ) {
    if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)105) ) ) {
            result[0] += 0;
            result[1] += 0.00749063670411985;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9850187265917603;
            result[5] += 0.00749063670411985;
          } else {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)79) ) ) {
            result[0] += 0.02040816326530612;
            result[1] += 0;
            result[2] += 0.061224489795918366;
            result[3] += 0;
            result[4] += 0.2653061224489796;
            result[5] += 0.6530612244897959;
          } else {
            result[0] += 0.68;
            result[1] += 0.11;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.15;
            result[5] += 0.06;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0.0014705882352941176;
            result[2] += 0;
            result[3] += 0.011764705882352941;
            result[4] += 0.030882352941176472;
            result[5] += 0.9558823529411765;
          } else {
            result[0] += 0;
            result[1] += 0.009345794392523364;
            result[2] += 0.006230529595015576;
            result[3] += 0.17133956386292834;
            result[4] += 0.1838006230529595;
            result[5] += 0.6292834890965732;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.032;
            result[3] += 0.692;
            result[4] += 0;
            result[5] += 0.276;
          } else {
            result[0] += 0.22916666666666669;
            result[1] += 0;
            result[2] += 0.6041666666666667;
            result[3] += 0.10416666666666669;
            result[4] += 0;
            result[5] += 0.06250000000000001;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)48.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)100.5) ) ) {
            result[0] += 0.03614457831325301;
            result[1] += 0.1566265060240964;
            result[2] += 0;
            result[3] += 0.012048192771084338;
            result[4] += 0.7710843373493976;
            result[5] += 0.024096385542168676;
          } else {
            result[0] += 0;
            result[1] += 0.84375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.15625;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)72.5) ) ) {
            result[0] += 0.2072072072072072;
            result[1] += 0.08108108108108109;
            result[2] += 0;
            result[3] += 0.21621621621621623;
            result[4] += 0.12612612612612611;
            result[5] += 0.36936936936936937;
          } else {
            result[0] += 0.918111753371869;
            result[1] += 0.016377649325626204;
            result[2] += 0.0009633911368015414;
            result[3] += 0.01348747591522158;
            result[4] += 0.046242774566473986;
            result[5] += 0.004816955684007707;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)89.5) ) ) {
            result[0] += 0.022140221402214024;
            result[1] += 0.04428044280442805;
            result[2] += 0.32103321033210336;
            result[3] += 0.3394833948339484;
            result[4] += 0.040590405904059046;
            result[5] += 0.23247232472324725;
          } else {
            result[0] += 0.6494845360824743;
            result[1] += 0;
            result[2] += 0.24742268041237114;
            result[3] += 0.010309278350515464;
            result[4] += 0.09278350515463918;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)82) ) ) {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)90.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)71) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0.4;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.043478260869565216;
            result[3] += 0.043478260869565216;
            result[4] += 0.043478260869565216;
            result[5] += 0.8695652173913043;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05660377358490566;
            result[3] += 0.6226415094339622;
            result[4] += 0;
            result[5] += 0.32075471698113206;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6363636363636364;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.36363636363636365;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)78) ) ) {
            result[0] += 0;
            result[1] += 0.4;
            result[2] += 0.2;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.010638297872340427;
            result[2] += 0.7446808510638299;
            result[3] += 0.1914893617021277;
            result[4] += 0;
            result[5] += 0.053191489361702135;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)103) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)113.5) ) ) {
            result[0] += 0;
            result[1] += 0.125;
            result[2] += 0.25;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6341463414634146;
            result[3] += 0.34146341463414637;
            result[4] += 0;
            result[5] += 0.024390243902439025;
          } else {
            result[0] += 0.022123893805309734;
            result[1] += 0;
            result[2] += 0.9070796460176991;
            result[3] += 0.05752212389380531;
            result[4] += 0;
            result[5] += 0.01327433628318584;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0018281535648994515;
            result[1] += 0;
            result[2] += 0.9853747714808044;
            result[3] += 0.012797074954296161;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)59.5) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)70.5) ) ) {
            result[0] += 0.00558659217877095;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9832402234636871;
            result[5] += 0.0111731843575419;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0.6666666666666666;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)57.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.25;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4166666666666667;
            result[5] += 0.3333333333333333;
          }
        }
      } else {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)64.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 0.7142857142857143;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2857142857142857;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)81.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.023809523809523808;
            result[4] += 0.7857142857142857;
            result[5] += 0.19047619047619047;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)41.5) ) ) {
            result[0] += 0;
            result[1] += 0.9411764705882353;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.058823529411764705;
          } else {
            result[0] += 0.0011148272017837235;
            result[1] += 0.002229654403567447;
            result[2] += 0.002229654403567447;
            result[3] += 0.05239687848383501;
            result[4] += 0.04013377926421405;
            result[5] += 0.9018952062430323;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)80) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)50) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.1111111111111111;
            result[4] += 0.037037037037037035;
            result[5] += 0.8518518518518519;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.012195121951219513;
            result[3] += 0.6788617886178862;
            result[4] += 0.008130081300813009;
            result[5] += 0.3008130081300813;
          } else {
            result[0] += 0.20689655172413793;
            result[1] += 0.13793103448275862;
            result[2] += 0.4482758620689655;
            result[3] += 0.20689655172413793;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)77) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)95) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)50) ) ) {
            result[0] += 0;
            result[1] += 0.6923076923076923;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3076923076923077;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.037383177570093455;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9626168224299065;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)57) ) ) {
            result[0] += 0;
            result[1] += 0.9955357142857143;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004464285714285714;
            result[5] += 0;
          } else {
            result[0] += 0.22222222222222224;
            result[1] += 0.4444444444444445;
            result[2] += 0;
            result[3] += 0.11111111111111112;
            result[4] += 0.22222222222222224;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)80) ) ) {
            result[0] += 0.467391304347826;
            result[1] += 0.03804347826086956;
            result[2] += 0.005434782608695651;
            result[3] += 0.03804347826086956;
            result[4] += 0.3043478260869565;
            result[5] += 0.1467391304347826;
          } else {
            result[0] += 0.09523809523809523;
            result[1] += 0;
            result[2] += 0.1746031746031746;
            result[3] += 0.30158730158730157;
            result[4] += 0.023809523809523808;
            result[5] += 0.40476190476190477;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)77) ) ) {
            result[0] += 0.9218303145853194;
            result[1] += 0.010486177311725452;
            result[2] += 0;
            result[3] += 0.017159199237368923;
            result[4] += 0.0438512869399428;
            result[5] += 0.00667302192564347;
          } else {
            result[0] += 0.3048780487804878;
            result[1] += 0.14634146341463414;
            result[2] += 0.3048780487804878;
            result[3] += 0.0975609756097561;
            result[4] += 0.04878048780487805;
            result[5] += 0.0975609756097561;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0.021505376344086023;
            result[2] += 0.03225806451612903;
            result[3] += 0.7634408602150538;
            result[4] += 0;
            result[5] += 0.1827956989247312;
          } else {
            result[0] += 0;
            result[1] += 0.046875;
            result[2] += 0.359375;
            result[3] += 0.328125;
            result[4] += 0;
            result[5] += 0.265625;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)53.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.05813953488372093;
            result[1] += 0;
            result[2] += 0.5232558139534884;
            result[3] += 0.20930232558139536;
            result[4] += 0.06976744186046512;
            result[5] += 0.13953488372093023;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)94) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.9459459459459459;
            result[1] += 0;
            result[2] += 0.05405405405405406;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)87.5) ) ) {
            result[0] += 0.0103359173126615;
            result[1] += 0;
            result[2] += 0.8217054263565892;
            result[3] += 0.14728682170542637;
            result[4] += 0;
            result[5] += 0.020671834625323;
          } else {
            result[0] += 0.001763668430335097;
            result[1] += 0;
            result[2] += 0.9788359788359788;
            result[3] += 0.01763668430335097;
            result[4] += 0;
            result[5] += 0.001763668430335097;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)83.5) ) ) {
            result[0] += 0.018518518518518517;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9768518518518519;
            result[5] += 0.004629629629629629;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)57) ) ) {
            result[0] += 0;
            result[1] += 0.9411764705882353;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.058823529411764705;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.11538461538461539;
            result[4] += 0.46153846153846156;
            result[5] += 0.4230769230769231;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.04878048780487805;
            result[2] += 0;
            result[3] += 0.024390243902439025;
            result[4] += 0.7317073170731707;
            result[5] += 0.1951219512195122;
          } else {
            result[0] += 0.029978586723768737;
            result[1] += 0.0021413276231263384;
            result[2] += 0.0010706638115631692;
            result[3] += 0.035331905781584586;
            result[4] += 0.057815845824411134;
            result[5] += 0.8736616702355461;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)86.5) ) ) {
            result[0] += 0.0071428571428571435;
            result[1] += 0;
            result[2] += 0.042857142857142864;
            result[3] += 0.2571428571428572;
            result[4] += 0.021428571428571432;
            result[5] += 0.6714285714285715;
          } else {
            result[0] += 0.005291005291005291;
            result[1] += 0;
            result[2] += 0.08465608465608465;
            result[3] += 0.6825396825396826;
            result[4] += 0;
            result[5] += 0.2275132275132275;
          }
        }
      }
    } else {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)86.5) ) ) {
            result[0] += 0.041666666666666664;
            result[1] += 0.5833333333333334;
            result[2] += 0;
            result[3] += 0.08333333333333333;
            result[4] += 0.2916666666666667;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)70.5) ) ) {
            result[0] += 0.14847161572052403;
            result[1] += 0.2838427947598253;
            result[2] += 0.013100436681222707;
            result[3] += 0.017467248908296942;
            result[4] += 0.4672489082969432;
            result[5] += 0.06986899563318777;
          } else {
            result[0] += 0.8715753424657534;
            result[1] += 0.004280821917808219;
            result[2] += 0.017123287671232876;
            result[3] += 0.028253424657534245;
            result[4] += 0.045376712328767124;
            result[5] += 0.03339041095890411;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0.023076923076923078;
            result[1] += 0.046153846153846156;
            result[2] += 0.06923076923076923;
            result[3] += 0.36538461538461536;
            result[4] += 0.09615384615384616;
            result[5] += 0.4;
          } else {
            result[0] += 0.39316239316239304;
            result[1] += 0.008547008547008546;
            result[2] += 0.3846153846153845;
            result[3] += 0.08547008547008546;
            result[4] += 0.07692307692307691;
            result[5] += 0.051282051282051266;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
      if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16;
            result[3] += 0.04;
            result[4] += 0.08;
            result[5] += 0.72;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.65;
            result[4] += 0;
            result[5] += 0.225;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)93) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)104) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.525;
            result[3] += 0.2;
            result[4] += 0;
            result[5] += 0.275;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)91.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3181818181818182;
            result[4] += 0;
            result[5] += 0.6818181818181818;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)67.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.16666666666666666;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)88.5) ) ) {
            result[0] += 0.008368200836820083;
            result[1] += 0;
            result[2] += 0.7824267782426778;
            result[3] += 0.19246861924686193;
            result[4] += 0;
            result[5] += 0.016736401673640166;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9705882352941176;
            result[3] += 0.029411764705882353;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)111.5) ) ) {
            result[0] += 0.00881057268722467;
            result[1] += 0;
            result[2] += 0.9295154185022027;
            result[3] += 0.06167400881057269;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9804469273743017;
            result[3] += 0.019553072625698324;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)81) ) ) {
    if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)79.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)68.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)99) ) ) {
            result[0] += 0.0594059405940594;
            result[1] += 0.006600660066006601;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8778877887788779;
            result[5] += 0.056105610561056105;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)69.5) ) ) {
            result[0] += 0.047619047619047616;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.23809523809523808;
            result[5] += 0.7142857142857143;
          } else {
            result[0] += 0.8947368421052632;
            result[1] += 0.005847953216374269;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.08187134502923976;
            result[5] += 0.017543859649122806;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62) ) ) {
            result[0] += 0;
            result[1] += 0.06666666666666667;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9333333333333333;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.06997742663656885;
            result[4] += 0.045146726862302484;
            result[5] += 0.8848758465011287;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)69) ) ) {
            result[0] += 0.17647058823529413;
            result[1] += 0.03619909502262444;
            result[2] += 0.03167420814479638;
            result[3] += 0.18099547511312217;
            result[4] += 0.2895927601809955;
            result[5] += 0.2850678733031674;
          } else {
            result[0] += 0.005390835579514826;
            result[1] += 0.013477088948787064;
            result[2] += 0.04851752021563343;
            result[3] += 0.5525606469002696;
            result[4] += 0.02964959568733154;
            result[5] += 0.35040431266846367;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)99) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.12;
            result[4] += 0.88;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.8571428571428571;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.09523809523809523;
            result[5] += 0.047619047619047616;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)99.5) ) ) {
            result[0] += 0.08823529411764706;
            result[1] += 0.6470588235294118;
            result[2] += 0.058823529411764705;
            result[3] += 0;
            result[4] += 0.20588235294117646;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)69.5) ) ) {
            result[0] += 0.16393442622950818;
            result[1] += 0.36065573770491804;
            result[2] += 0;
            result[3] += 0.01639344262295082;
            result[4] += 0.3770491803278688;
            result[5] += 0.08196721311475409;
          } else {
            result[0] += 0.9102167182662538;
            result[1] += 0.007223942208462332;
            result[2] += 0.010319917440660475;
            result[3] += 0.007223942208462332;
            result[4] += 0.04540763673890609;
            result[5] += 0.0196078431372549;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.29411764705882354;
            result[3] += 0.27941176470588236;
            result[4] += 0;
            result[5] += 0.4264705882352941;
          } else {
            result[0] += 0.20437956204379562;
            result[1] += 0;
            result[2] += 0.6423357664233577;
            result[3] += 0.014598540145985401;
            result[4] += 0.06569343065693431;
            result[5] += 0.072992700729927;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)95) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.041666666666666664;
            result[3] += 0.041666666666666664;
            result[4] += 0;
            result[5] += 0.9166666666666666;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)64) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.08;
            result[4] += 0;
            result[5] += 0.72;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3;
            result[3] += 0.6125;
            result[4] += 0.0125;
            result[5] += 0.075;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8235294117647058;
            result[3] += 0.17647058823529413;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)94.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5714285714285714;
            result[3] += 0.42857142857142855;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9;
            result[3] += 0.1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7692307692307693;
            result[3] += 0;
            result[4] += 0.07692307692307693;
            result[5] += 0.15384615384615385;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)91) ) ) {
            result[0] += 0.001798561151079137;
            result[1] += 0;
            result[2] += 0.9370503597122303;
            result[3] += 0.059352517985611516;
            result[4] += 0;
            result[5] += 0.001798561151079137;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)45.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0.004149377593360996;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9875518672199171;
            result[5] += 0.008298755186721992;
          } else {
            result[0] += 0.04918032786885246;
            result[1] += 0.01639344262295082;
            result[2] += 0;
            result[3] += 0.06557377049180328;
            result[4] += 0.6229508196721312;
            result[5] += 0.2459016393442623;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)69) ) ) {
            result[0] += 0.2857142857142857;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0;
          } else {
            result[0] += 0.9743589743589743;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.02564102564102564;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)85.5) ) ) {
            result[0] += 0.0011682242990654205;
            result[1] += 0.009345794392523364;
            result[2] += 0.015186915887850467;
            result[3] += 0.0455607476635514;
            result[4] += 0.06658878504672897;
            result[5] += 0.8621495327102804;
          } else {
            result[0] += 0.06306306306306306;
            result[1] += 0.018018018018018018;
            result[2] += 0.032432432432432434;
            result[3] += 0.3945945945945946;
            result[4] += 0.07567567567567568;
            result[5] += 0.41621621621621624;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)69.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)51.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.006944444444444445;
            result[1] += 0.9907407407407408;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002314814814814815;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)48.5) ) ) {
            result[0] += 0;
            result[1] += 0.7142857142857143;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2857142857142857;
            result[5] += 0;
          } else {
            result[0] += 0.10256410256410256;
            result[1] += 0.0641025641025641;
            result[2] += 0;
            result[3] += 0.01282051282051282;
            result[4] += 0.8076923076923077;
            result[5] += 0.01282051282051282;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)77) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)74) ) ) {
            result[0] += 0.2280701754385965;
            result[1] += 0.3859649122807018;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3508771929824562;
            result[5] += 0.03508771929824562;
          } else {
            result[0] += 0.8950495049504951;
            result[1] += 0.0029702970297029703;
            result[2] += 0.005940594059405941;
            result[3] += 0.031683168316831684;
            result[4] += 0.03663366336633663;
            result[5] += 0.027722772277227723;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
            result[0] += 0.0819672131147541;
            result[1] += 0.016393442622950824;
            result[2] += 0.049180327868852465;
            result[3] += 0.2295081967213115;
            result[4] += 0.1475409836065574;
            result[5] += 0.4754098360655738;
          } else {
            result[0] += 0.19047619047619047;
            result[1] += 0;
            result[2] += 0.6190476190476191;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0.07936507936507936;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)95) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)65) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)90) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.1780821917808219;
            result[4] += 0;
            result[5] += 0.821917808219178;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0.010869565217391304;
            result[2] += 0.14130434782608695;
            result[3] += 0.7065217391304348;
            result[4] += 0.010869565217391304;
            result[5] += 0.13043478260869565;
          } else {
            result[0] += 0;
            result[1] += 0.1;
            result[2] += 0.1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)98) ) ) {
            result[0] += 0;
            result[1] += 0.07692307692307693;
            result[2] += 0.7538461538461538;
            result[3] += 0.03076923076923077;
            result[4] += 0;
            result[5] += 0.13846153846153847;
          } else {
            result[0] += 0;
            result[1] += 0.42857142857142855;
            result[2] += 0;
            result[3] += 0.14285714285714285;
            result[4] += 0.42857142857142855;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.16666666666666666;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)85.5) ) ) {
            result[0] += 0.016483516483516484;
            result[1] += 0;
            result[2] += 0.6923076923076923;
            result[3] += 0.27472527472527475;
            result[4] += 0;
            result[5] += 0.016483516483516484;
          } else {
            result[0] += 0.008064516129032258;
            result[1] += 0;
            result[2] += 0.8790322580645161;
            result[3] += 0.11290322580645161;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8125;
            result[3] += 0.15625;
            result[4] += 0;
            result[5] += 0.03125;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.006968641114982578;
            result[1] += 0;
            result[2] += 0.9634146341463414;
            result[3] += 0.029616724738675958;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)99) ) ) {
            result[0] += 0.006993006993006993;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9790209790209791;
            result[5] += 0.013986013986013986;
          } else {
            result[0] += 0;
            result[1] += 0.625;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.375;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0.05555555555555555;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2222222222222222;
            result[5] += 0.7222222222222222;
          } else {
            result[0] += 0;
            result[1] += 0.057692307692307696;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8653846153846154;
            result[5] += 0.07692307692307693;
          }
        }
      } else {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)70.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9285714285714286;
            result[5] += 0.07142857142857142;
          } else {
            result[0] += 0.007972665148063782;
            result[1] += 0.0011389521640091116;
            result[2] += 0.0011389521640091116;
            result[3] += 0.06378132118451026;
            result[4] += 0.030751708428246014;
            result[5] += 0.8952164009111617;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)69.5) ) ) {
            result[0] += 0.023809523809523808;
            result[1] += 0.023809523809523808;
            result[2] += 0;
            result[3] += 0.2619047619047619;
            result[4] += 0.17857142857142858;
            result[5] += 0.5119047619047619;
          } else {
            result[0] += 0.5081081081081081;
            result[1] += 0.005405405405405406;
            result[2] += 0.005405405405405406;
            result[3] += 0.13513513513513514;
            result[4] += 0.13513513513513514;
            result[5] += 0.21081081081081082;
          }
        }
      }
    } else {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)55.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)69.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)90.5) ) ) {
            result[0] += 0;
            result[1] += 0.45454545454545453;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5454545454545454;
            result[5] += 0;
          } else {
            result[0] += 0.004366812227074236;
            result[1] += 0.9934497816593887;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002183406113537118;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)100.5) ) ) {
            result[0] += 0.14285714285714285;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8571428571428571;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)58) ) ) {
            result[0] += 0.9795918367346939;
            result[1] += 0.02040816326530612;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.034129692832764506;
            result[1] += 0.034129692832764506;
            result[2] += 0.027303754266211604;
            result[3] += 0.5017064846416383;
            result[4] += 0.027303754266211604;
            result[5] += 0.37542662116040953;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)68.5) ) ) {
            result[0] += 0.11428571428571428;
            result[1] += 0.41904761904761906;
            result[2] += 0;
            result[3] += 0.01904761904761905;
            result[4] += 0.4095238095238095;
            result[5] += 0.0380952380952381;
          } else {
            result[0] += 0.8292902066486972;
            result[1] += 0.020664869721473494;
            result[2] += 0.04672057502246182;
            result[3] += 0.03593890386343217;
            result[4] += 0.04672057502246182;
            result[5] += 0.020664869721473494;
          }
        }
      }
    }
  } else {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.0975609756097561;
            result[4] += 0.024390243902439025;
            result[5] += 0.8780487804878049;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.625;
            result[5] += 0.375;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8125;
            result[3] += 0.0625;
            result[4] += 0;
            result[5] += 0.125;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)77) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.06818181818181818;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0.18181818181818182;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0975609756097561;
            result[3] += 0.6585365853658537;
            result[4] += 0;
            result[5] += 0.24390243902439024;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6111111111111112;
            result[3] += 0.3333333333333333;
            result[4] += 0;
            result[5] += 0.05555555555555555;
          }
        }
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)99) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)93) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)76.5) ) ) {
            result[0] += 0;
            result[1] += 0.04;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2;
            result[5] += 0.76;
          } else {
            result[0] += 0;
            result[1] += 0.2222222222222222;
            result[2] += 0.1111111111111111;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7142857142857143;
            result[3] += 0.10714285714285714;
            result[4] += 0;
            result[5] += 0.17857142857142858;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)84) ) ) {
            result[0] += 0.17647058823529413;
            result[1] += 0;
            result[2] += 0.35294117647058826;
            result[3] += 0.17647058823529413;
            result[4] += 0.29411764705882354;
            result[5] += 0;
          } else {
            result[0] += 0.9230769230769231;
            result[1] += 0.07692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)84.5) ) ) {
            result[0] += 0.014545454545454547;
            result[1] += 0;
            result[2] += 0.8218181818181819;
            result[3] += 0.13454545454545458;
            result[4] += 0;
            result[5] += 0.029090909090909094;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9575113808801214;
            result[3] += 0.03945371775417299;
            result[4] += 0;
            result[5] += 0.0030349013657056147;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)93.5) ) ) {
            result[0] += 0.004608294930875577;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9861751152073733;
            result[5] += 0.009216589861751154;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)90) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.03225806451612903;
            result[3] += 0;
            result[4] += 0.6774193548387096;
            result[5] += 0.2903225806451613;
          } else {
            result[0] += 0;
            result[1] += 0.6875;
            result[2] += 0;
            result[3] += 0.3125;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.75;
            result[5] += 0.25;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0014903129657228018;
            result[3] += 0.037257824143070044;
            result[4] += 0.00894187779433681;
            result[5] += 0.9523099850968704;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
            result[0] += 0.323943661971831;
            result[1] += 0.2676056338028169;
            result[2] += 0;
            result[3] += 0.028169014084507043;
            result[4] += 0.22535211267605634;
            result[5] += 0.15492957746478872;
          } else {
            result[0] += 0.002645502645502646;
            result[1] += 0.010582010582010583;
            result[2] += 0.005291005291005292;
            result[3] += 0.3280423280423281;
            result[4] += 0.06084656084656085;
            result[5] += 0.5925925925925927;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)46.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.09090909090909091;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9090909090909091;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)83) ) ) {
            result[0] += 0;
            result[1] += 0.9973958333333334;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0026041666666666665;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)65) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)41) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.07692307692307693;
            result[1] += 0.047337278106508875;
            result[2] += 0.01775147928994083;
            result[3] += 0.05325443786982249;
            result[4] += 0.7692307692307693;
            result[5] += 0.03550295857988166;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)73.5) ) ) {
            result[0] += 0.15471698113207547;
            result[1] += 0.06792452830188679;
            result[2] += 0.018867924528301886;
            result[3] += 0.2339622641509434;
            result[4] += 0.1660377358490566;
            result[5] += 0.3584905660377358;
          } else {
            result[0] += 0.8093058733791;
            result[1] += 0.006864988558352403;
            result[2] += 0.03127383676582762;
            result[3] += 0.04881769641495043;
            result[4] += 0.06407322654462244;
            result[5] += 0.03966437833714722;
          }
        }
      }
    }
  } else {
    if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)103) ) ) {
      if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)90.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0.75;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.15833333333333333;
            result[3] += 0.6416666666666667;
            result[4] += 0;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4782608695652174;
            result[3] += 0.4782608695652174;
            result[4] += 0;
            result[5] += 0.043478260869565216;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.84375;
            result[3] += 0.09375;
            result[4] += 0;
            result[5] += 0.0625;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0.75;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5454545454545454;
            result[3] += 0.45454545454545453;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.012820512820512822;
            result[1] += 0;
            result[2] += 0.9487179487179488;
            result[3] += 0.012820512820512822;
            result[4] += 0;
            result[5] += 0.025641025641025644;
          }
        }
      }
    } else {
      if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)103.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8125;
            result[4] += 0;
            result[5] += 0.1875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.65;
            result[3] += 0.225;
            result[4] += 0;
            result[5] += 0.125;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8636363636363634;
            result[3] += 0.12587412587412586;
            result[4] += 0.0034965034965034956;
            result[5] += 0.006993006993006991;
          } else {
            result[0] += 0.009191176470588236;
            result[1] += 0;
            result[2] += 0.9705882352941176;
            result[3] += 0.02022058823529412;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)80) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.10714285714285714;
            result[1] += 0;
            result[2] += 0.8928571428571429;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)91.5) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)97.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)41) ) ) {
            result[0] += 0.02985074626865672;
            result[1] += 0.8656716417910448;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.10447761194029852;
            result[5] += 0;
          } else {
            result[0] += 0.03715170278637771;
            result[1] += 0.009287925696594427;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.913312693498452;
            result[5] += 0.04024767801857585;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)69.5) ) ) {
            result[0] += 0.07017543859649124;
            result[1] += 0.01754385964912281;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.456140350877193;
            result[5] += 0.456140350877193;
          } else {
            result[0] += 0.9033149171270718;
            result[1] += 0.008287292817679558;
            result[2] += 0.0055248618784530384;
            result[3] += 0;
            result[4] += 0.04696132596685083;
            result[5] += 0.03591160220994475;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.01935483870967742;
            result[4] += 0.05161290322580645;
            result[5] += 0.9290322580645162;
          } else {
            result[0] += 0.03125;
            result[1] += 0.00625;
            result[2] += 0;
            result[3] += 0.16875;
            result[4] += 0.16875;
            result[5] += 0.625;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0.02564102564102564;
            result[2] += 0.016317016317016316;
            result[3] += 0.48717948717948717;
            result[4] += 0.07925407925407925;
            result[5] += 0.3916083916083916;
          } else {
            result[0] += 0.4129353233830846;
            result[1] += 0.08955223880597014;
            result[2] += 0.004975124378109453;
            result[3] += 0.13930348258706468;
            result[4] += 0.21890547263681592;
            result[5] += 0.13432835820895522;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)54.5) ) ) {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      } else {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)108.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.15789473684210528;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.736842105263158;
            result[5] += 0.10526315789473685;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.5000000000000001;
            result[1] += 0.16666666666666669;
            result[2] += 0.16666666666666669;
            result[3] += 0;
            result[4] += 0.16666666666666669;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)77.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)74.5) ) ) {
            result[0] += 0.9889064976228209;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.011093502377179081;
            result[5] += 0;
          } else {
            result[0] += 0.68;
            result[1] += 0;
            result[2] += 0.04;
            result[3] += 0.04;
            result[4] += 0.24;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.4;
            result[4] += 0.1;
            result[5] += 0.5;
          } else {
            result[0] += 0.20833333333333334;
            result[1] += 0;
            result[2] += 0.7083333333333334;
            result[3] += 0.041666666666666664;
            result[4] += 0.041666666666666664;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)103) ) ) {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)100.5) ) ) {
            result[0] += 0.013605442176870748;
            result[1] += 0;
            result[2] += 0.23809523809523808;
            result[3] += 0.6190476190476191;
            result[4] += 0;
            result[5] += 0.1292517006802721;
          } else {
            result[0] += 0.008064516129032258;
            result[1] += 0;
            result[2] += 0.5645161290322581;
            result[3] += 0.25806451612903225;
            result[4] += 0;
            result[5] += 0.1693548387096774;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9295774647887324;
            result[3] += 0.07042253521126761;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)81) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)97.5) ) ) {
            result[0] += 0.15;
            result[1] += 0;
            result[2] += 0.7125;
            result[3] += 0.0375;
            result[4] += 0.0375;
            result[5] += 0.0625;
          } else {
            result[0] += 0.95;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.05;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)100.5) ) ) {
            result[0] += 0.012048192771084338;
            result[1] += 0;
            result[2] += 0.6144578313253012;
            result[3] += 0.26506024096385544;
            result[4] += 0;
            result[5] += 0.10843373493975904;
          } else {
            result[0] += 0.0078125;
            result[1] += 0;
            result[2] += 0.9479166666666666;
            result[3] += 0.04296875;
            result[4] += 0;
            result[5] += 0.0013020833333333333;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)82) ) ) {
            result[0] += 0.010948905109489052;
            result[1] += 0.010948905109489052;
            result[2] += 0;
            result[3] += 0.010948905109489052;
            result[4] += 0.9051094890510949;
            result[5] += 0.06204379562043796;
          } else {
            result[0] += 0;
            result[1] += 0.9333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.06666666666666667;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)69.5) ) ) {
            result[0] += 0.05;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.35;
            result[5] += 0.6;
          } else {
            result[0] += 0.9242424242424242;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07575757575757576;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0.02564102564102564;
            result[2] += 0;
            result[3] += 0.05128205128205128;
            result[4] += 0.6410256410256411;
            result[5] += 0.28205128205128205;
          } else {
            result[0] += 0.001221001221001221;
            result[1] += 0;
            result[2] += 0.002442002442002442;
            result[3] += 0.03663003663003663;
            result[4] += 0.03785103785103785;
            result[5] += 0.9218559218559218;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)83.5) ) ) {
            result[0] += 0.00823045267489712;
            result[1] += 0.01646090534979424;
            result[2] += 0.00823045267489712;
            result[3] += 0.20987654320987653;
            result[4] += 0.06172839506172839;
            result[5] += 0.6954732510288066;
          } else {
            result[0] += 0.017341040462427744;
            result[1] += 0.017341040462427744;
            result[2] += 0;
            result[3] += 0.7572254335260116;
            result[4] += 0;
            result[5] += 0.20809248554913296;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)93) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0.109375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.859375;
            result[5] += 0.03125;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0.8846153846153846;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.11538461538461539;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)70.5) ) ) {
            result[0] += 0.20224719101123595;
            result[1] += 0.12359550561797752;
            result[2] += 0.056179775280898875;
            result[3] += 0.06741573033707865;
            result[4] += 0.5056179775280899;
            result[5] += 0.0449438202247191;
          } else {
            result[0] += 0.8837638376383764;
            result[1] += 0.011992619926199263;
            result[2] += 0.008302583025830259;
            result[3] += 0.027675276752767528;
            result[4] += 0.04797047970479705;
            result[5] += 0.02029520295202952;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)107.5) ) ) {
            result[0] += 0.07;
            result[1] += 0.06333333333333334;
            result[2] += 0.3233333333333333;
            result[3] += 0.30666666666666664;
            result[4] += 0.043333333333333335;
            result[5] += 0.19333333333333333;
          } else {
            result[0] += 0.8421052631578948;
            result[1] += 0;
            result[2] += 0.12280701754385966;
            result[3] += 0.03508771929824562;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)103) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)90.5) ) ) {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.0425531914893617;
            result[2] += 0.06382978723404255;
            result[3] += 0.06382978723404255;
            result[4] += 0.0425531914893617;
            result[5] += 0.7872340425531915;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)77) ) ) {
            result[0] += 0.2;
            result[1] += 0.2;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14545454545454545;
            result[3] += 0.7818181818181819;
            result[4] += 0;
            result[5] += 0.07272727272727272;
          } else {
            result[0] += 0;
            result[1] += 0.0821917808219178;
            result[2] += 0.5342465753424658;
            result[3] += 0.3287671232876712;
            result[4] += 0;
            result[5] += 0.0547945205479452;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8787878787878788;
            result[3] += 0.06060606060606061;
            result[4] += 0;
            result[5] += 0.06060606060606061;
          }
        }
      }
    } else {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)118.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.875;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.022813688212927757;
            result[1] += 0;
            result[2] += 0.844106463878327;
            result[3] += 0.12167300380228137;
            result[4] += 0;
            result[5] += 0.011406844106463879;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0.3333333333333333;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7884615384615384;
            result[3] += 0.21153846153846154;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.98;
            result[3] += 0.02;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)74.5) ) ) {
            result[0] += 0.6666666666666666;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9900596421471173;
            result[3] += 0.009940357852882704;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)58) ) ) {
            result[0] += 0.04;
            result[1] += 0.01;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.95;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5444444444444444;
            result[5] += 0.45555555555555555;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)54) ) ) {
            result[0] += 0.7887323943661972;
            result[1] += 0.12676056338028172;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.05633802816901409;
            result[5] += 0.028169014084507046;
          } else {
            result[0] += 0.11290322580645161;
            result[1] += 0.04838709677419355;
            result[2] += 0.04838709677419355;
            result[3] += 0;
            result[4] += 0.7258064516129032;
            result[5] += 0.06451612903225806;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5348837209302325;
            result[5] += 0.46511627906976744;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.051597051597051594;
            result[4] += 0.02457002457002457;
            result[5] += 0.9238329238329238;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)82) ) ) {
            result[0] += 0.034334763948497854;
            result[1] += 0.004291845493562232;
            result[2] += 0.04721030042918455;
            result[3] += 0.18454935622317598;
            result[4] += 0.14163090128755365;
            result[5] += 0.5879828326180258;
          } else {
            result[0] += 0.014423076923076924;
            result[1] += 0.014423076923076924;
            result[2] += 0.03365384615384615;
            result[3] += 0.6538461538461539;
            result[4] += 0.014423076923076924;
            result[5] += 0.2692307692307692;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)49.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.3333333333333333;
            result[1] += 0.1111111111111111;
            result[2] += 0.2222222222222222;
            result[3] += 0.3333333333333333;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)104.5) ) ) {
            result[0] += 0.8079800498753118;
            result[1] += 0.016625103906899422;
            result[2] += 0.008312551953449711;
            result[3] += 0.019118869492934336;
            result[4] += 0.11055694098088115;
            result[5] += 0.0374064837905237;
          } else {
            result[0] += 0.1016949152542373;
            result[1] += 0.7796610169491526;
            result[2] += 0;
            result[3] += 0.01694915254237288;
            result[4] += 0.1016949152542373;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)105) ) ) {
            result[0] += 0.04460966542750929;
            result[1] += 0.007434944237918215;
            result[2] += 0.14869888475836432;
            result[3] += 0.31970260223048325;
            result[4] += 0.07806691449814127;
            result[5] += 0.40148698884758366;
          } else {
            result[0] += 0.8072289156626506;
            result[1] += 0;
            result[2] += 0.0963855421686747;
            result[3] += 0.04819277108433735;
            result[4] += 0;
            result[5] += 0.04819277108433735;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)82) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.09803921568627451;
            result[3] += 0.6862745098039216;
            result[4] += 0;
            result[5] += 0.21568627450980393;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7000000000000001;
            result[3] += 0.20000000000000004;
            result[4] += 0;
            result[5] += 0.10000000000000002;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8;
            result[3] += 0.1;
            result[4] += 0;
            result[5] += 0.1;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9705882352941176;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.029411764705882353;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)94) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0.014705882352941176;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.23529411764705882;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)114.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0.8333333333333334;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)118.5) ) ) {
            result[0] += 0.008097165991902834;
            result[1] += 0;
            result[2] += 0.8582995951417004;
            result[3] += 0.12955465587044535;
            result[4] += 0;
            result[5] += 0.004048582995951417;
          } else {
            result[0] += 0.3333333333333333;
            result[1] += 0;
            result[2] += 0.5555555555555556;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)89.5) ) ) {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0016891891891891893;
            result[1] += 0;
            result[2] += 0.972972972972973;
            result[3] += 0.02533783783783784;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9478260869565217;
            result[5] += 0.05217391304347826;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0;
            result[4] += 0.42857142857142855;
            result[5] += 0.42857142857142855;
          } else {
            result[0] += 0;
            result[1] += 0.07142857142857144;
            result[2] += 0;
            result[3] += 0.8571428571428572;
            result[4] += 0;
            result[5] += 0.07142857142857144;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)60.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.05;
            result[4] += 0.5;
            result[5] += 0.45;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.02403204272363151;
            result[4] += 0.017356475300400534;
            result[5] += 0.9586114819759679;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)63.5) ) ) {
            result[0] += 0.4225352112676056;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.014084507042253521;
            result[4] += 0.43661971830985913;
            result[5] += 0.1267605633802817;
          } else {
            result[0] += 0;
            result[1] += 0.0044444444444444444;
            result[2] += 0.006666666666666667;
            result[3] += 0.4222222222222222;
            result[4] += 0.04888888888888889;
            result[5] += 0.5177777777777778;
          }
        }
      }
    } else {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)52.5) ) ) {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)49.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8181818181818182;
            result[5] += 0.18181818181818182;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)84.5) ) ) {
            result[0] += 0.004705882352941176;
            result[1] += 0.9835294117647059;
            result[2] += 0;
            result[3] += 0.002352941176470588;
            result[4] += 0.009411764705882352;
            result[5] += 0;
          } else {
            result[0] += 0.375;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)70.5) ) ) {
            result[0] += 0.09248554913294797;
            result[1] += 0.24855491329479767;
            result[2] += 0;
            result[3] += 0.005780346820809248;
            result[4] += 0.6473988439306358;
            result[5] += 0.005780346820809248;
          } else {
            result[0] += 0.8800358102059087;
            result[1] += 0.011638316920322292;
            result[2] += 0.0035810205908683975;
            result[3] += 0.03043867502238138;
            result[4] += 0.048343777976723366;
            result[5] += 0.025962399283795883;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.011904761904761904;
            result[1] += 0.04365079365079365;
            result[2] += 0.07539682539682539;
            result[3] += 0.4166666666666667;
            result[4] += 0.06349206349206349;
            result[5] += 0.3888888888888889;
          } else {
            result[0] += 0.38125000000000003;
            result[1] += 0;
            result[2] += 0.45625000000000004;
            result[3] += 0.11250000000000002;
            result[4] += 0.006250000000000001;
            result[5] += 0.043750000000000004;
          }
        }
      }
    }
  } else {
    if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)101.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)90.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10714285714285714;
            result[3] += 0.8392857142857143;
            result[4] += 0;
            result[5] += 0.05357142857142857;
          }
        }
      } else {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)88) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0.08333333333333333;
            result[4] += 0;
            result[5] += 0.75;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6829268292682927;
            result[3] += 0.3170731707317073;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)87.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.96;
            result[3] += 0.04;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)74.5) ) ) {
            result[0] += 0.6;
            result[1] += 0.4;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.425;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0.325;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)108.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.71875;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0.03125;
          } else {
            result[0] += 0.045454545454545456;
            result[1] += 0;
            result[2] += 0.8787878787878788;
            result[3] += 0.07575757575757576;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)106) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.92;
            result[3] += 0.07;
            result[4] += 0;
            result[5] += 0.01;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9955156950672646;
            result[3] += 0.004484304932735426;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)62.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.03125;
            result[3] += 0;
            result[4] += 0.875;
            result[5] += 0.09375;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05263157894736842;
            result[3] += 0.05263157894736842;
            result[4] += 0.3684210526315789;
            result[5] += 0.5263157894736842;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)55.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9;
            result[5] += 0.1;
          } else {
            result[0] += 0.9811320754716981;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.018867924528301886;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)84.5) ) ) {
            result[0] += 0.002881844380403458;
            result[1] += 0.026897214217098942;
            result[2] += 0;
            result[3] += 0.06051873198847262;
            result[4] += 0.05571565802113353;
            result[5] += 0.8539865513928915;
          } else {
            result[0] += 0.014084507042253521;
            result[1] += 0.014084507042253521;
            result[2] += 0.10328638497652583;
            result[3] += 0.4507042253521127;
            result[4] += 0.018779342723004695;
            result[5] += 0.39906103286384975;
          }
        }
      }
    } else {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)52.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0.9930715935334873;
            result[2] += 0;
            result[3] += 0.0023094688221709007;
            result[4] += 0.004618937644341801;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)50) ) ) {
            result[0] += 0.07142857142857142;
            result[1] += 0.8928571428571429;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.03571428571428571;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.1;
            result[2] += 0.011111111111111112;
            result[3] += 0.022222222222222223;
            result[4] += 0.8444444444444444;
            result[5] += 0.022222222222222223;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)106) ) ) {
            result[0] += 0.8375594294770207;
            result[1] += 0.0007923930269413631;
            result[2] += 0.0063391442155309044;
            result[3] += 0.032488114104595886;
            result[4] += 0.08637083993660857;
            result[5] += 0.0364500792393027;
          } else {
            result[0] += 0.06666666666666667;
            result[1] += 0.7777777777777778;
            result[2] += 0.044444444444444446;
            result[3] += 0;
            result[4] += 0.1111111111111111;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)104.5) ) ) {
            result[0] += 0.06611570247933884;
            result[1] += 0.06611570247933884;
            result[2] += 0.19008264462809918;
            result[3] += 0.47107438016528924;
            result[4] += 0.01652892561983471;
            result[5] += 0.19008264462809918;
          } else {
            result[0] += 0.6161616161616161;
            result[1] += 0.04040404040404041;
            result[2] += 0.25252525252525254;
            result[3] += 0.010101010101010102;
            result[4] += 0.04040404040404041;
            result[5] += 0.04040404040404041;
          }
        }
      }
    }
  } else {
    if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)91.5) ) ) {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)63.5) ) ) {
        result[0] += 0;
        result[1] += 0;
        result[2] += 0;
        result[3] += 0;
        result[4] += 1;
        result[5] += 0;
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)101.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)67.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7666666666666667;
            result[4] += 0;
            result[5] += 0.23333333333333334;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04;
            result[3] += 0.1;
            result[4] += 0.08;
            result[5] += 0.78;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5454545454545454;
            result[3] += 0.2727272727272727;
            result[4] += 0;
            result[5] += 0.18181818181818182;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0.1;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2571428571428571;
            result[3] += 0.6571428571428571;
            result[4] += 0;
            result[5] += 0.08571428571428572;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0.4;
          } else {
            result[0] += 0.012820512820512824;
            result[1] += 0;
            result[2] += 0.7564102564102566;
            result[3] += 0.20512820512820518;
            result[4] += 0.012820512820512824;
            result[5] += 0.012820512820512824;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9166666666666666;
            result[3] += 0.08333333333333333;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7787610619469026;
            result[3] += 0.19469026548672566;
            result[4] += 0;
            result[5] += 0.02654867256637168;
          } else {
            result[0] += 0.0013698630136986301;
            result[1] += 0;
            result[2] += 0.9575342465753425;
            result[3] += 0.03561643835616438;
            result[4] += 0;
            result[5] += 0.005479452054794521;
          }
        }
      }
    }
  }
  if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)99.5) ) ) {
            result[0] += 0.009259259259259259;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9675925925925926;
            result[5] += 0.023148148148148147;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)54) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.12121212121212122;
            result[1] += 0.09090909090909091;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6363636363636364;
            result[5] += 0.15151515151515152;
          }
        }
      } else {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9032258064516129;
            result[5] += 0.0967741935483871;
          } else {
            result[0] += 0;
            result[1] += 0.005807200929152149;
            result[2] += 0;
            result[3] += 0.03484320557491289;
            result[4] += 0.0313588850174216;
            result[5] += 0.9279907084785134;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)87.5) ) ) {
            result[0] += 0.008733624454148471;
            result[1] += 0.004366812227074236;
            result[2] += 0.021834061135371178;
            result[3] += 0.2794759825327511;
            result[4] += 0.10480349344978165;
            result[5] += 0.5807860262008734;
          } else {
            result[0] += 0.0196078431372549;
            result[1] += 0.013071895424836602;
            result[2] += 0.0392156862745098;
            result[3] += 0.7058823529411765;
            result[4] += 0.013071895424836602;
            result[5] += 0.20915032679738563;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)38) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.0024096385542168677;
            result[1] += 0.9927710843373494;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004819277108433735;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)68.5) ) ) {
            result[0] += 0.26627218934911245;
            result[1] += 0.1242603550295858;
            result[2] += 0;
            result[3] += 0.01775147928994083;
            result[4] += 0.5680473372781065;
            result[5] += 0.023668639053254437;
          } else {
            result[0] += 0.8097269624573379;
            result[1] += 0.016211604095563138;
            result[2] += 0.0034129692832764505;
            result[3] += 0.03498293515358362;
            result[4] += 0.08276450511945392;
            result[5] += 0.052901023890784986;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0.0196078431372549;
            result[1] += 0.023529411764705882;
            result[2] += 0.12941176470588237;
            result[3] += 0.4823529411764706;
            result[4] += 0.043137254901960784;
            result[5] += 0.30196078431372547;
          } else {
            result[0] += 0.42105263157894735;
            result[1] += 0;
            result[2] += 0.46411483253588515;
            result[3] += 0.04784688995215311;
            result[4] += 0.03827751196172249;
            result[5] += 0.028708133971291867;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)94.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)75) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0.03508771929824561;
            result[2] += 0.017543859649122806;
            result[3] += 0.3333333333333333;
            result[4] += 0.07017543859649122;
            result[5] += 0.543859649122807;
          } else {
            result[0] += 0.29629629629629634;
            result[1] += 0.07407407407407408;
            result[2] += 0.4444444444444445;
            result[3] += 0.11111111111111112;
            result[4] += 0;
            result[5] += 0.07407407407407408;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)93) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.38461538461538464;
            result[4] += 0;
            result[5] += 0.6153846153846154;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7971014492753623;
            result[3] += 0.14492753623188406;
            result[4] += 0;
            result[5] += 0.057971014492753624;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)107) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666667;
            result[3] += 0.16666666666666669;
            result[4] += 0;
            result[5] += 0.16666666666666669;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0.7777777777777778;
            result[4] += 0;
            result[5] += 0.05555555555555555;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0.02857142857142857;
            result[1] += 0;
            result[2] += 0.9142857142857143;
            result[3] += 0.05714285714285714;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.84;
            result[3] += 0.11;
            result[4] += 0;
            result[5] += 0.05;
          } else {
            result[0] += 0.001488095238095238;
            result[1] += 0;
            result[2] += 0.9672619047619048;
            result[3] += 0.03125;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)71.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9904761904761905;
            result[5] += 0.009523809523809525;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)59.5) ) ) {
            result[0] += 0.25;
            result[1] += 0.013157894736842105;
            result[2] += 0;
            result[3] += 0.013157894736842105;
            result[4] += 0.4342105263157895;
            result[5] += 0.2894736842105263;
          } else {
            result[0] += 0.0009596928982725527;
            result[1] += 0.013435700575815739;
            result[2] += 0.0028790786948176585;
            result[3] += 0.06429942418426103;
            result[4] += 0.036468330134357005;
            result[5] += 0.8819577735124761;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.24;
            result[4] += 0.04;
            result[5] += 0.72;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.06896551724137931;
            result[3] += 0.7586206896551724;
            result[4] += 0;
            result[5] += 0.1724137931034483;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)83) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)110.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)83) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9900497512437811;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.009950248756218905;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
            result[0] += 0.029850746268656716;
            result[1] += 0.21641791044776118;
            result[2] += 0;
            result[3] += 0.022388059701492536;
            result[4] += 0.7313432835820896;
            result[5] += 0;
          } else {
            result[0] += 0.7993527508090617;
            result[1] += 0.026699029126213594;
            result[2] += 0.0032362459546925572;
            result[3] += 0.03236245954692557;
            result[4] += 0.0946601941747573;
            result[5] += 0.04368932038834952;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)90.5) ) ) {
            result[0] += 0.010025062656641603;
            result[1] += 0.005012531328320802;
            result[2] += 0.2681704260651629;
            result[3] += 0.43609022556390975;
            result[4] += 0.007518796992481203;
            result[5] += 0.2731829573934837;
          } else {
            result[0] += 0.6990291262135923;
            result[1] += 0;
            result[2] += 0.14563106796116504;
            result[3] += 0.019417475728155338;
            result[4] += 0.13592233009708737;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)103) ) ) {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)93.5) ) ) {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)87) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)71) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0196078431372549;
            result[3] += 0.13725490196078433;
            result[4] += 0;
            result[5] += 0.8431372549019608;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)74) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.25;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1891891891891892;
            result[3] += 0.21621621621621623;
            result[4] += 0.05405405405405406;
            result[5] += 0.5405405405405406;
          } else {
            result[0] += 0;
            result[1] += 0.7;
            result[2] += 0.3;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.30434782608695654;
            result[3] += 0.5652173913043478;
            result[4] += 0;
            result[5] += 0.13043478260869565;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7857142857142857;
            result[3] += 0.21428571428571427;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)69) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)108.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)56) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)79) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5714285714285714;
            result[3] += 0.42857142857142855;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)86.5) ) ) {
            result[0] += 0.009433962264150945;
            result[1] += 0;
            result[2] += 0.6603773584905661;
            result[3] += 0.30188679245283023;
            result[4] += 0;
            result[5] += 0.028301886792452834;
          } else {
            result[0] += 0.005988023952095809;
            result[1] += 0;
            result[2] += 0.9161676646706587;
            result[3] += 0.0718562874251497;
            result[4] += 0;
            result[5] += 0.005988023952095809;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
            result[0] += 0.22727272727272727;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.2727272727272727;
          } else {
            result[0] += 0.003448275862068966;
            result[1] += 0;
            result[2] += 0.9862068965517242;
            result[3] += 0.008620689655172415;
            result[4] += 0;
            result[5] += 0.001724137931034483;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)63.5) ) ) {
            result[0] += 0.00392156862745098;
            result[1] += 0.01568627450980392;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9647058823529412;
            result[5] += 0.01568627450980392;
          } else {
            result[0] += 0.0625;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0.4375;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)68.5) ) ) {
            result[0] += 0.03508771929824561;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.40350877192982454;
            result[5] += 0.5614035087719298;
          } else {
            result[0] += 0.8807339449541285;
            result[1] += 0.009174311926605505;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07339449541284404;
            result[5] += 0.03669724770642202;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)56) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.019704433497536946;
            result[4] += 0.019704433497536946;
            result[5] += 0.9605911330049262;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0.011904761904761904;
            result[2] += 0.011904761904761904;
            result[3] += 0.03571428571428571;
            result[4] += 0.125;
            result[5] += 0.8154761904761905;
          } else {
            result[0] += 0.006872852233676976;
            result[1] += 0.01718213058419244;
            result[2] += 0.006872852233676976;
            result[3] += 0.27491408934707906;
            result[4] += 0.12714776632302405;
            result[5] += 0.5670103092783505;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)55.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)90.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0.125;
            result[2] += 0;
            result[3] += 0.25;
            result[4] += 0.625;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9887133182844243;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.011286681715575621;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)69.5) ) ) {
            result[0] += 0.08620689655172414;
            result[1] += 0.2413793103448276;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6551724137931034;
            result[5] += 0.017241379310344827;
          } else {
            result[0] += 0.8835227272727273;
            result[1] += 0.011363636363636364;
            result[2] += 0.002840909090909091;
            result[3] += 0.023674242424242424;
            result[4] += 0.04640151515151515;
            result[5] += 0.032196969696969696;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)87.5) ) ) {
            result[0] += 0.026030368763557486;
            result[1] += 0.04989154013015185;
            result[2] += 0.11930585683297182;
            result[3] += 0.4577006507592191;
            result[4] += 0.05206073752711497;
            result[5] += 0.29501084598698485;
          } else {
            result[0] += 0.6776859504132231;
            result[1] += 0.049586776859504134;
            result[2] += 0.18181818181818182;
            result[3] += 0.03305785123966942;
            result[4] += 0.049586776859504134;
            result[5] += 0.008264462809917356;
          }
        }
      }
    }
  } else {
    if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)85) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          } else {
            result[0] += 0.01694915254237288;
            result[1] += 0;
            result[2] += 0.1864406779661017;
            result[3] += 0.7457627118644068;
            result[4] += 0;
            result[5] += 0.05084745762711865;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)81.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8823529411764706;
            result[3] += 0.08823529411764706;
            result[4] += 0;
            result[5] += 0.029411764705882353;
          }
        }
      }
    } else {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)99) ) ) {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7391304347826088;
            result[3] += 0.19565217391304351;
            result[4] += 0;
            result[5] += 0.06521739130434784;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)103) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.03333333333333333;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0.13333333333333333;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)87.5) ) ) {
            result[0] += 0.0035842293906810036;
            result[1] += 0;
            result[2] += 0.9247311827956989;
            result[3] += 0.07168458781362007;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0036968576709796672;
            result[1] += 0;
            result[2] += 0.9833641404805915;
            result[3] += 0.012939001848428836;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)65) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)45) ) ) {
            result[0] += 0;
            result[1] += 0.25;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.375;
            result[5] += 0.375;
          } else {
            result[0] += 0.017241379310344827;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9655172413793104;
            result[5] += 0.017241379310344827;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.01694915254237288;
            result[3] += 0.01694915254237288;
            result[4] += 0.6779661016949152;
            result[5] += 0.288135593220339;
          } else {
            result[0] += 0;
            result[1] += 0.23529411764705882;
            result[2] += 0;
            result[3] += 0.7647058823529411;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)58) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)57.5) ) ) {
            result[0] += 0.9625;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0375;
            result[5] += 0;
          } else {
            result[0] += 0.09090909090909091;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.13636363636363635;
            result[4] += 0.7272727272727273;
            result[5] += 0.045454545454545456;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03717472118959108;
            result[4] += 0.05204460966542751;
            result[5] += 0.9107806691449815;
          } else {
            result[0] += 0.006787330316742082;
            result[1] += 0.011312217194570135;
            result[2] += 0.011312217194570135;
            result[3] += 0.4298642533936652;
            result[4] += 0.01809954751131222;
            result[5] += 0.5226244343891403;
          }
        }
      }
    } else {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)52) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)56) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.85;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.15;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)70.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)48.5) ) ) {
            result[0] += 0;
            result[1] += 0.9259259259259259;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07407407407407407;
            result[5] += 0;
          } else {
            result[0] += 0.10810810810810811;
            result[1] += 0.04504504504504504;
            result[2] += 0;
            result[3] += 0.018018018018018018;
            result[4] += 0.8108108108108109;
            result[5] += 0.018018018018018018;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)74.5) ) ) {
            result[0] += 0.847210994341148;
            result[1] += 0.03233629749393695;
            result[2] += 0.02021018593371059;
            result[3] += 0.02910266774454325;
            result[4] += 0.054163298302344384;
            result[5] += 0.016976556184316895;
          } else {
            result[0] += 0.13;
            result[1] += 0.006666666666666667;
            result[2] += 0.3466666666666667;
            result[3] += 0.29;
            result[4] += 0.04666666666666667;
            result[5] += 0.18;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)95.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)107.5) ) ) {
            result[0] += 0;
            result[1] += 0.08571428571428572;
            result[2] += 0.05714285714285714;
            result[3] += 0;
            result[4] += 0.34285714285714286;
            result[5] += 0.5142857142857142;
          } else {
            result[0] += 0.125;
            result[1] += 0;
            result[2] += 0.875;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0851063829787234;
            result[3] += 0.7446808510638298;
            result[4] += 0.02127659574468085;
            result[5] += 0.14893617021276595;
          } else {
            result[0] += 0.09090909090909091;
            result[1] += 0;
            result[2] += 0.5227272727272727;
            result[3] += 0.22727272727272727;
            result[4] += 0;
            result[5] += 0.1590909090909091;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)75.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75.5) ) ) {
            result[0] += 0.75;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.25;
          } else {
            result[0] += 0.03703703703703704;
            result[1] += 0;
            result[2] += 0.7962962962962964;
            result[3] += 0.16666666666666669;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.17647058823529413;
            result[3] += 0.6470588235294118;
            result[4] += 0;
            result[5] += 0.17647058823529413;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)130) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8617511520737328;
            result[3] += 0.1382488479262673;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.865979381443299;
            result[3] += 0.13402061855670103;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9880952380952381;
            result[3] += 0.011904761904761904;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9736842105263158;
            result[3] += 0.02631578947368421;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)101) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.03571428571428571;
            result[1] += 0.017857142857142856;
            result[2] += 0;
            result[3] += 0.05357142857142857;
            result[4] += 0.6607142857142857;
            result[5] += 0.23214285714285715;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)49.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.2857142857142857;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
            result[0] += 0.00909090909090909;
            result[1] += 0.07272727272727272;
            result[2] += 0;
            result[3] += 0.01818181818181818;
            result[4] += 0.5454545454545454;
            result[5] += 0.35454545454545455;
          } else {
            result[0] += 0.055491329479768786;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03468208092485549;
            result[4] += 0.04393063583815029;
            result[5] += 0.8658959537572254;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.01694915254237288;
            result[3] += 0.22033898305084745;
            result[4] += 0;
            result[5] += 0.7627118644067796;
          } else {
            result[0] += 0.05185185185185185;
            result[1] += 0;
            result[2] += 0.04814814814814815;
            result[3] += 0.6222222222222222;
            result[4] += 0.003703703703703704;
            result[5] += 0.2740740740740741;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)102.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)48.5) ) ) {
            result[0] += 0.036585365853658534;
            result[1] += 0.8170731707317073;
            result[2] += 0;
            result[3] += 0.024390243902439025;
            result[4] += 0.12195121951219512;
            result[5] += 0;
          } else {
            result[0] += 0.008547008547008548;
            result[1] += 0.07692307692307693;
            result[2] += 0.008547008547008548;
            result[3] += 0.008547008547008548;
            result[4] += 0.8290598290598291;
            result[5] += 0.06837606837606838;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)73) ) ) {
            result[0] += 0.8012820512820513;
            result[1] += 0.008814102564102564;
            result[2] += 0.008012820512820512;
            result[3] += 0.05368589743589743;
            result[4] += 0.04887820512820513;
            result[5] += 0.07932692307692307;
          } else {
            result[0] += 0.18064516129032257;
            result[1] += 0.01935483870967742;
            result[2] += 0.2161290322580645;
            result[3] += 0.2645161290322581;
            result[4] += 0.06451612903225806;
            result[5] += 0.25483870967741934;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)65) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)47) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9090909090909091;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.09090909090909091;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)58.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)86) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)107.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10526315789473684;
            result[3] += 0.7192982456140351;
            result[4] += 0;
            result[5] += 0.17543859649122806;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.25;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)91.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)72) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0.75;
          } else {
            result[0] += 0.027777777777777776;
            result[1] += 0;
            result[2] += 0.7777777777777778;
            result[3] += 0.17592592592592593;
            result[4] += 0;
            result[5] += 0.018518518518518517;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)88.5) ) ) {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)74.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9090909090909091;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.09090909090909091;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)69) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)108.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
            result[0] += 0.0036496350364963502;
            result[1] += 0;
            result[2] += 0.8503649635036497;
            result[3] += 0.12773722627737227;
            result[4] += 0;
            result[5] += 0.01824817518248175;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9768270944741533;
            result[3] += 0.023172905525846704;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
            result[0] += 0.007380073800738007;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.981549815498155;
            result[5] += 0.01107011070110701;
          } else {
            result[0] += 0.017543859649122806;
            result[1] += 0.017543859649122806;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5614035087719298;
            result[5] += 0.40350877192982454;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)82.5) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)58) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)56.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.2;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)83.5) ) ) {
            result[0] += 0.007900677200902935;
            result[1] += 0.009029345372460496;
            result[2] += 0.001128668171557562;
            result[3] += 0.045146726862302484;
            result[4] += 0.04740406320541761;
            result[5] += 0.8893905191873589;
          } else {
            result[0] += 0.022099447513812154;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.36464088397790057;
            result[4] += 0.0718232044198895;
            result[5] += 0.5414364640883977;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)59) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)90.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)46.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05263157894736842;
            result[3] += 0;
            result[4] += 0.9473684210526315;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          } else {
            result[0] += 0.004819277108433735;
            result[1] += 0.9951807228915662;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)63.5) ) ) {
            result[0] += 0.8465608465608465;
            result[1] += 0.015873015873015872;
            result[2] += 0.010582010582010581;
            result[3] += 0;
            result[4] += 0.07936507936507936;
            result[5] += 0.047619047619047616;
          } else {
            result[0] += 0.01272264631043257;
            result[1] += 0.027989821882951654;
            result[2] += 0.06870229007633588;
            result[3] += 0.44274809160305345;
            result[4] += 0.09923664122137404;
            result[5] += 0.3486005089058524;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)71.5) ) ) {
            result[0] += 0.12307692307692308;
            result[1] += 0.24615384615384617;
            result[2] += 0.03076923076923077;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0;
          } else {
            result[0] += 0.8532934131736527;
            result[1] += 0.005988023952095809;
            result[2] += 0.028942115768463075;
            result[3] += 0.011976047904191617;
            result[4] += 0.05788423153692615;
            result[5] += 0.041916167664670656;
          }
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)97.5) ) ) {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)83.5) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)105) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7090909090909091;
            result[4] += 0;
            result[5] += 0.2909090909090909;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.11764705882352941;
            result[3] += 0.6470588235294118;
            result[4] += 0;
            result[5] += 0.23529411764705882;
          } else {
            result[0] += 0.056451612903225805;
            result[1] += 0;
            result[2] += 0.5645161290322581;
            result[3] += 0.27419354838709675;
            result[4] += 0;
            result[5] += 0.10483870967741936;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)79) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8181818181818182;
            result[3] += 0.18181818181818182;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)105) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9736842105263158;
            result[3] += 0.02631578947368421;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)94.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.25;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8141025641025641;
            result[3] += 0.1794871794871795;
            result[4] += 0;
            result[5] += 0.00641025641025641;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8125;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.1875;
          } else {
            result[0] += 0.8333333333333334;
            result[1] += 0;
            result[2] += 0.16666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)77) ) ) {
            result[0] += 0.75;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.971830985915493;
            result[3] += 0.02535211267605634;
            result[4] += 0;
            result[5] += 0.0028169014084507044;
          }
        }
      }
    }
  }
  if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)70.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)62) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)43.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)51.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0.008583690987124463;
            result[2] += 0;
            result[3] += 0.004291845493562232;
            result[4] += 0.9484978540772532;
            result[5] += 0.03862660944206009;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
            result[0] += 0.009313154831199068;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.02910360884749709;
            result[4] += 0.04074505238649592;
            result[5] += 0.9208381839348079;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.03225806451612903;
            result[3] += 0.4608294930875576;
            result[4] += 0;
            result[5] += 0.5069124423963134;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)50) ) ) {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)87) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)56.5) ) ) {
            result[0] += 0;
            result[1] += 0.2857142857142857;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.8571428571428571;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.14285714285714285;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)68.5) ) ) {
            result[0] += 0.11330049261083745;
            result[1] += 0.187192118226601;
            result[2] += 0;
            result[3] += 0.004926108374384237;
            result[4] += 0.6798029556650247;
            result[5] += 0.01477832512315271;
          } else {
            result[0] += 0.8505367464905038;
            result[1] += 0.015689512799339392;
            result[2] += 0.012386457473162677;
            result[3] += 0.030553261767134604;
            result[4] += 0.0652353426919901;
            result[5] += 0.025598678777869532;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)89.5) ) ) {
            result[0] += 0.00702576112412178;
            result[1] += 0;
            result[2] += 0.21311475409836064;
            result[3] += 0.4449648711943794;
            result[4] += 0.0468384074941452;
            result[5] += 0.28805620608899296;
          } else {
            result[0] += 0.5416666666666666;
            result[1] += 0;
            result[2] += 0.2708333333333333;
            result[3] += 0.020833333333333332;
            result[4] += 0.125;
            result[5] += 0.041666666666666664;
          }
        }
      }
    }
  } else {
    if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)104.5) ) ) {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1724137931034483;
            result[3] += 0.034482758620689655;
            result[4] += 0.034482758620689655;
            result[5] += 0.7586206896551724;
          } else {
            result[0] += 0;
            result[1] += 0.013888888888888888;
            result[2] += 0.2638888888888889;
            result[3] += 0.5347222222222222;
            result[4] += 0.006944444444444444;
            result[5] += 0.18055555555555555;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)95.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7368421052631579;
            result[3] += 0.2631578947368421;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)64) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)106.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8148148148148148;
            result[3] += 0.18518518518518517;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)99) ) ) {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3076923076923077;
            result[4] += 0;
            result[5] += 0.6923076923076923;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)91) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8518518518518519;
            result[3] += 0.14814814814814814;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.3;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.6;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)63.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)63.5) ) ) {
            result[0] += 0.8;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.005502063273727649;
            result[1] += 0;
            result[2] += 0.9436038514442917;
            result[3] += 0.045392022008253104;
            result[4] += 0;
            result[5] += 0.005502063273727649;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)96) ) ) {
            result[0] += 0.009345794392523364;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.985981308411215;
            result[5] += 0.004672897196261682;
          } else {
            result[0] += 0;
            result[1] += 0.8571428571428571;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.14285714285714285;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)56.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.14285714285714285;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.38961038961038963;
            result[5] += 0.4675324675324675;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0.02564102564102564;
            result[2] += 0;
            result[3] += 0.05128205128205128;
            result[4] += 0.41025641025641024;
            result[5] += 0.5128205128205128;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0029850746268656717;
            result[3] += 0.019402985074626865;
            result[4] += 0.016417910447761194;
            result[5] += 0.9611940298507463;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0.003389830508474576;
            result[2] += 0;
            result[3] += 0.17966101694915254;
            result[4] += 0.0711864406779661;
            result[5] += 0.7457627118644068;
          } else {
            result[0] += 0.00784313725490196;
            result[1] += 0.0196078431372549;
            result[2] += 0.00392156862745098;
            result[3] += 0.5843137254901961;
            result[4] += 0.023529411764705882;
            result[5] += 0.3607843137254902;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)69.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)47.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.1694915254237288;
            result[1] += 0.03389830508474576;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7457627118644068;
            result[5] += 0.05084745762711865;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)56.5) ) ) {
            result[0] += 0;
            result[1] += 0.9925742574257426;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.007425742574257425;
            result[5] += 0;
          } else {
            result[0] += 0.045454545454545456;
            result[1] += 0.42424242424242425;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5303030303030303;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)73) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)71.5) ) ) {
            result[0] += 0.4166666666666667;
            result[1] += 0.1388888888888889;
            result[2] += 0.020833333333333332;
            result[3] += 0.1111111111111111;
            result[4] += 0.25;
            result[5] += 0.0625;
          } else {
            result[0] += 0.9198841698841699;
            result[1] += 0.003861003861003861;
            result[2] += 0.006756756756756757;
            result[3] += 0.019305019305019305;
            result[4] += 0.03088803088803089;
            result[5] += 0.019305019305019305;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)88.5) ) ) {
            result[0] += 0.009615384615384616;
            result[1] += 0.009615384615384616;
            result[2] += 0.27884615384615385;
            result[3] += 0.3701923076923077;
            result[4] += 0.038461538461538464;
            result[5] += 0.2932692307692308;
          } else {
            result[0] += 0.6794871794871795;
            result[1] += 0.01282051282051282;
            result[2] += 0.11538461538461539;
            result[3] += 0;
            result[4] += 0.19230769230769232;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)82.5) ) ) {
      if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)82) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)108) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.11111111111111112;
            result[3] += 0.7373737373737375;
            result[4] += 0;
            result[5] += 0.15151515151515155;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)108.5) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)76.5) ) ) {
            result[0] += 0.6666666666666666;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.013513513513513514;
            result[1] += 0;
            result[2] += 0.918918918918919;
            result[3] += 0.06756756756756757;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)81) ) ) {
            result[0] += 0.20000000000000004;
            result[1] += 0;
            result[2] += 0.7000000000000001;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.10000000000000002;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.75;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)93.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)97) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8;
            result[3] += 0.2;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)82) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5882352941176471;
            result[3] += 0.27941176470588236;
            result[4] += 0;
            result[5] += 0.1323529411764706;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9107142857142857;
            result[3] += 0.08928571428571429;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)86.5) ) ) {
            result[0] += 0.012145748987854251;
            result[1] += 0;
            result[2] += 0.8825910931174089;
            result[3] += 0.09716599190283401;
            result[4] += 0;
            result[5] += 0.008097165991902834;
          } else {
            result[0] += 0.001845018450184502;
            result[1] += 0;
            result[2] += 0.9778597785977861;
            result[3] += 0.020295202952029523;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)115) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9831223628691983;
            result[5] += 0.016877637130801686;
          } else {
            result[0] += 0.06666666666666667;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0.26666666666666666;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0.6666666666666667;
            result[1] += 0.04444444444444445;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.15555555555555559;
            result[5] += 0.13333333333333336;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.002232142857142857;
            result[3] += 0.041294642857142856;
            result[4] += 0.0625;
            result[5] += 0.8939732142857143;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)68.5) ) ) {
            result[0] += 0;
            result[1] += 0.014084507042253521;
            result[2] += 0.007042253521126761;
            result[3] += 0.2605633802816901;
            result[4] += 0.04225352112676056;
            result[5] += 0.676056338028169;
          } else {
            result[0] += 0.05855855855855856;
            result[1] += 0.0045045045045045045;
            result[2] += 0.02252252252252252;
            result[3] += 0.5990990990990991;
            result[4] += 0.018018018018018018;
            result[5] += 0.2972972972972973;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)70.5) ) ) {
            result[0] += 0.08444444444444446;
            result[1] += 0.3911111111111112;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5155555555555557;
            result[5] += 0.00888888888888889;
          } else {
            result[0] += 0.89492119089317;
            result[1] += 0.007880910683012261;
            result[2] += 0;
            result[3] += 0.027145359019264452;
            result[4] += 0.05341506129597198;
            result[5] += 0.01663747810858144;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
            result[0] += 0.009900990099009901;
            result[1] += 0;
            result[2] += 0.2508250825082508;
            result[3] += 0.46204620462046203;
            result[4] += 0.009900990099009901;
            result[5] += 0.26732673267326734;
          } else {
            result[0] += 0.5797101449275363;
            result[1] += 0.043478260869565216;
            result[2] += 0.2898550724637681;
            result[3] += 0.028985507246376812;
            result[4] += 0.050724637681159424;
            result[5] += 0.007246376811594203;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)55) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)31.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)83.5) ) ) {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)99) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)102.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.06382978723404255;
            result[3] += 0.5957446808510638;
            result[4] += 0.02127659574468085;
            result[5] += 0.3191489361702128;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.1;
            result[4] += 0;
            result[5] += 0.8;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3;
            result[3] += 0.4;
            result[4] += 0;
            result[5] += 0.3;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9090909090909091;
            result[3] += 0.09090909090909091;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)101) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0.2;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5294117647058824;
            result[3] += 0.36470588235294116;
            result[4] += 0.047058823529411764;
            result[5] += 0.058823529411764705;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)99) ) ) {
            result[0] += 0.02702702702702703;
            result[1] += 0.013513513513513514;
            result[2] += 0.918918918918919;
            result[3] += 0.04054054054054054;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.6;
            result[1] += 0.4;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)103) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6818181818181818;
            result[3] += 0.22727272727272727;
            result[4] += 0;
            result[5] += 0.09090909090909091;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8513513513513514;
            result[3] += 0.13513513513513517;
            result[4] += 0;
            result[5] += 0.013513513513513516;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.16666666666666666;
            result[5] += 0.8333333333333334;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)105.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8823529411764706;
            result[3] += 0.11764705882352941;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9758064516129032;
            result[3] += 0.024193548387096774;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9617021276595744;
            result[5] += 0.03829787234042553;
          } else {
            result[0] += 0;
            result[1] += 0.9;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.3;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.2;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)82) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)58) ) ) {
            result[0] += 0.5060240963855421;
            result[1] += 0.012048192771084338;
            result[2] += 0;
            result[3] += 0.024096385542168676;
            result[4] += 0.3493975903614458;
            result[5] += 0.10843373493975904;
          } else {
            result[0] += 0.010044642857142858;
            result[1] += 0;
            result[2] += 0.002232142857142857;
            result[3] += 0.04575892857142857;
            result[4] += 0.06584821428571429;
            result[5] += 0.8761160714285714;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.12195121951219512;
            result[4] += 0.024390243902439025;
            result[5] += 0.8536585365853658;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5955056179775281;
            result[4] += 0.011235955056179775;
            result[5] += 0.39325842696629215;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)55.5) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)63.5) ) ) {
            result[0] += 0;
            result[1] += 0.9952267303102625;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.00477326968973747;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.4444444444444445;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.33333333333333337;
            result[5] += 0.22222222222222224;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)70.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)101.5) ) ) {
            result[0] += 0.1111111111111111;
            result[1] += 0.022222222222222223;
            result[2] += 0.02962962962962963;
            result[3] += 0.022222222222222223;
            result[4] += 0.7777777777777778;
            result[5] += 0.037037037037037035;
          } else {
            result[0] += 0.03225806451612903;
            result[1] += 0.967741935483871;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
            result[0] += 0.885317115551694;
            result[1] += 0.024326672458731533;
            result[2] += 0;
            result[3] += 0.023457862728062547;
            result[4] += 0.05299739357080798;
            result[5] += 0.013900955690703732;
          } else {
            result[0] += 0.1756756756756757;
            result[1] += 0.00900900900900901;
            result[2] += 0.29729729729729737;
            result[3] += 0.2612612612612613;
            result[4] += 0.027027027027027032;
            result[5] += 0.22972972972972977;
          }
        }
      }
    }
  } else {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)88.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)91.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)76) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)102.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)104) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.9642857142857143;
            result[4] += 0;
            result[5] += 0.03571428571428571;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0.0625;
            result[4] += 0;
            result[5] += 0.1875;
          }
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)63.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0;
            result[4] += 0.8571428571428571;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.38028169014084506;
            result[3] += 0.4647887323943662;
            result[4] += 0;
            result[5] += 0.15492957746478872;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)117) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8541666666666667;
            result[3] += 0.04166666666666667;
            result[4] += 0;
            result[5] += 0.10416666666666669;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6410256410256411;
            result[3] += 0.34615384615384615;
            result[4] += 0.01282051282051282;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9552238805970149;
            result[3] += 0.04477611940298507;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
            result[0] += 0.21428571428571427;
            result[1] += 0;
            result[2] += 0.6428571428571429;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.14285714285714285;
          } else {
            result[0] += 0.0029455081001472753;
            result[1] += 0;
            result[2] += 0.9587628865979382;
            result[3] += 0.036818851251840944;
            result[4] += 0;
            result[5] += 0.0014727540500736377;
          }
        }
      }
    }
  }
  if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)93.5) ) ) {
    if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)99) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)47.5) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          } else {
            result[0] += 0;
            result[1] += 0.004;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.996;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)73.5) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)70) ) ) {
            result[0] += 0.06896551724137931;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7931034482758621;
            result[5] += 0.13793103448275862;
          } else {
            result[0] += 0;
            result[1] += 0.06666666666666667;
            result[2] += 0;
            result[3] += 0.03333333333333333;
            result[4] += 0.3333333333333333;
            result[5] += 0.5666666666666667;
          }
        }
      }
    } else {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)60.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)61) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8095238095238095;
            result[5] += 0.19047619047619047;
          } else {
            result[0] += 0.0029154518950437317;
            result[1] += 0;
            result[2] += 0.0029154518950437317;
            result[3] += 0.03498542274052478;
            result[4] += 0.011661807580174927;
            result[5] += 0.9475218658892128;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
            result[0] += 0.017482517482517484;
            result[1] += 0.0034965034965034965;
            result[2] += 0.013986013986013986;
            result[3] += 0.15034965034965034;
            result[4] += 0.08741258741258741;
            result[5] += 0.7272727272727273;
          } else {
            result[0] += 0.0903954802259887;
            result[1] += 0.01977401129943503;
            result[2] += 0.002824858757062147;
            result[3] += 0.4067796610169492;
            result[4] += 0.1271186440677966;
            result[5] += 0.3531073446327684;
          }
        }
      }
    }
  } else {
    if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)59) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)55.5) ) ) {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)76) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)34.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)49.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)76) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      }
    } else {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)75.5) ) ) {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)66.5) ) ) {
            result[0] += 0.10526315789473684;
            result[1] += 0.47368421052631576;
            result[2] += 0.017543859649122806;
            result[3] += 0;
            result[4] += 0.38596491228070173;
            result[5] += 0.017543859649122806;
          } else {
            result[0] += 0.9157782515991472;
            result[1] += 0.009594882729211088;
            result[2] += 0.006396588486140725;
            result[3] += 0.01812366737739872;
            result[4] += 0.039445628997867806;
            result[5] += 0.010660980810234541;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0.04938271604938271;
            result[2] += 0.15432098765432098;
            result[3] += 0.37037037037037035;
            result[4] += 0.05555555555555555;
            result[5] += 0.37037037037037035;
          } else {
            result[0] += 0.6184971098265896;
            result[1] += 0.09826589595375723;
            result[2] += 0.1791907514450867;
            result[3] += 0.04046242774566474;
            result[4] += 0.06358381502890173;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.23958333333333334;
            result[3] += 0.5;
            result[4] += 0.036458333333333336;
            result[5] += 0.22395833333333334;
          } else {
            result[0] += 0.03539823008849558;
            result[1] += 0;
            result[2] += 0.6106194690265487;
            result[3] += 0.2035398230088496;
            result[4] += 0.026548672566371685;
            result[5] += 0.12389380530973453;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)121) ) ) {
            result[0] += 0.015098722415795587;
            result[1] += 0;
            result[2] += 0.8908246225319396;
            result[3] += 0.07781649245063879;
            result[4] += 0;
            result[5] += 0.016260162601626018;
          } else {
            result[0] += 0.3076923076923077;
            result[1] += 0;
            result[2] += 0.6923076923076923;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)120) ) ) {
            result[0] += 0;
            result[1] += 0.011049723756906077;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.988950276243094;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)91) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.18181818181818182;
            result[4] += 0.45454545454545453;
            result[5] += 0.36363636363636365;
          } else {
            result[0] += 0;
            result[1] += 0.8571428571428571;
            result[2] += 0;
            result[3] += 0.14285714285714285;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)84.5) ) ) {
            result[0] += 0.005641748942172073;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.018335684062059238;
            result[4] += 0.04936530324400564;
            result[5] += 0.926657263751763;
          } else {
            result[0] += 0.13777777777777778;
            result[1] += 0.0044444444444444444;
            result[2] += 0;
            result[3] += 0.12;
            result[4] += 0.07111111111111111;
            result[5] += 0.6666666666666666;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)83.5) ) ) {
            result[0] += 0.02617801047120419;
            result[1] += 0.06806282722513089;
            result[2] += 0.020942408376963352;
            result[3] += 0.21465968586387435;
            result[4] += 0.07853403141361257;
            result[5] += 0.5916230366492147;
          } else {
            result[0] += 0.03007518796992481;
            result[1] += 0;
            result[2] += 0.007518796992481203;
            result[3] += 0.7518796992481203;
            result[4] += 0.007518796992481203;
            result[5] += 0.20300751879699247;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)50) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0.625;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.375;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.995260663507109;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004739336492890996;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)68.5) ) ) {
            result[0] += 0.1329479768786127;
            result[1] += 0.2254335260115607;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6184971098265896;
            result[5] += 0.023121387283236993;
          } else {
            result[0] += 0.8840206185567011;
            result[1] += 0.010309278350515465;
            result[2] += 0.0025773195876288664;
            result[3] += 0.030927835051546396;
            result[4] += 0.04467353951890035;
            result[5] += 0.027491408934707907;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.02830188679245283;
            result[1] += 0.050314465408805034;
            result[2] += 0.09748427672955975;
            result[3] += 0.3490566037735849;
            result[4] += 0.09748427672955975;
            result[5] += 0.37735849056603776;
          } else {
            result[0] += 0.3155080213903743;
            result[1] += 0;
            result[2] += 0.5614973262032086;
            result[3] += 0.09090909090909091;
            result[4] += 0.0106951871657754;
            result[5] += 0.0213903743315508;
          }
        }
      }
    }
  } else {
    if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
      if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)86) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13725490196078433;
            result[3] += 0.7843137254901961;
            result[4] += 0;
            result[5] += 0.0784313725490196;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5454545454545454;
            result[3] += 0.18181818181818182;
            result[4] += 0;
            result[5] += 0.2727272727272727;
          }
        }
      } else {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9166666666666666;
            result[3] += 0.08333333333333333;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)91) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0.8888888888888888;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)88.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)98) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8888888888888888;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)88.5) ) ) {
            result[0] += 0.03225806451612904;
            result[1] += 0;
            result[2] += 0.7741935483870969;
            result[3] += 0;
            result[4] += 0.06451612903225808;
            result[5] += 0.12903225806451615;
          } else {
            result[0] += 0.2;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8165137614678899;
            result[3] += 0.14220183486238533;
            result[4] += 0;
            result[5] += 0.04128440366972477;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)85.5) ) ) {
            result[0] += 0.010101010101010104;
            result[1] += 0;
            result[2] += 0.8383838383838385;
            result[3] += 0.14141414141414144;
            result[4] += 0.010101010101010104;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9754098360655737;
            result[3] += 0.02459016393442623;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0.016574585635359115;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9613259668508287;
            result[5] += 0.022099447513812154;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)69.5) ) ) {
            result[0] += 0.06172839506172839;
            result[1] += 0;
            result[2] += 0.024691358024691357;
            result[3] += 0.07407407407407407;
            result[4] += 0.6419753086419753;
            result[5] += 0.19753086419753085;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0.7777777777777778;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)88.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)56.5) ) ) {
            result[0] += 0.6304347826086957;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.34782608695652173;
            result[5] += 0.021739130434782608;
          } else {
            result[0] += 0;
            result[1] += 0.002028397565922921;
            result[2] += 0.002028397565922921;
            result[3] += 0.056795131845841784;
            result[4] += 0.059837728194726165;
            result[5] += 0.8793103448275862;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
            result[0] += 0.4634146341463415;
            result[1] += 0.2926829268292683;
            result[2] += 0.17073170731707318;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.07317073170731707;
          } else {
            result[0] += 0;
            result[1] += 0.008695652173913045;
            result[2] += 0.07391304347826089;
            result[3] += 0.5086956521739131;
            result[4] += 0.026086956521739132;
            result[5] += 0.382608695652174;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)91) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.8611111111111112;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1388888888888889;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)107.5) ) ) {
            result[0] += 0.8202943454686288;
            result[1] += 0.017815646785437643;
            result[2] += 0.017041053446940353;
            result[3] += 0.01859024012393493;
            result[4] += 0.09217660728117737;
            result[5] += 0.034082106893880706;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.043478260869565216;
            result[1] += 0.019762845849802372;
            result[2] += 0.039525691699604744;
            result[3] += 0.4150197628458498;
            result[4] += 0.13438735177865613;
            result[5] += 0.34782608695652173;
          } else {
            result[0] += 0.37606837606837606;
            result[1] += 0;
            result[2] += 0.4444444444444444;
            result[3] += 0.11965811965811966;
            result[4] += 0.02564102564102564;
            result[5] += 0.03418803418803419;
          }
        }
      }
    }
  } else {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)97.5) ) ) {
      if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)92.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)109.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.038461538461538464;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.9615384615384616;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)105) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.22580645161290322;
            result[3] += 0.7419354838709677;
            result[4] += 0;
            result[5] += 0.03225806451612903;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1935483870967742;
            result[3] += 0.2903225806451613;
            result[4] += 0;
            result[5] += 0.5161290322580645;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)101) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13043478260869565;
            result[3] += 0.5217391304347826;
            result[4] += 0;
            result[5] += 0.34782608695652173;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.765;
            result[3] += 0.225;
            result[4] += 0;
            result[5] += 0.01;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)119) ) ) {
            result[0] += 0.014492753623188408;
            result[1] += 0;
            result[2] += 0.8405797101449276;
            result[3] += 0.1449275362318841;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)76) ) ) {
            result[0] += 0.12903225806451613;
            result[1] += 0;
            result[2] += 0.7419354838709677;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.12903225806451613;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9630818619582665;
            result[3] += 0.03691813804173355;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)45) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.009174311926605505;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9770642201834863;
            result[5] += 0.013761467889908258;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)56.5) ) ) {
            result[0] += 0.9534883720930233;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.046511627906976744;
            result[5] += 0;
          } else {
            result[0] += 0.046511627906976744;
            result[1] += 0;
            result[2] += 0.09302325581395349;
            result[3] += 0.2558139534883721;
            result[4] += 0.3488372093023256;
            result[5] += 0.2558139534883721;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)82) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 0.08695652173913043;
            result[2] += 0;
            result[3] += 0.043478260869565216;
            result[4] += 0.5434782608695652;
            result[5] += 0.32608695652173914;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0022396416573348264;
            result[3] += 0.05375139977603583;
            result[4] += 0.05823068309070549;
            result[5] += 0.8857782754759238;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0.02631578947368421;
            result[2] += 0;
            result[3] += 0.15789473684210525;
            result[4] += 0.03508771929824561;
            result[5] += 0.7807017543859649;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.02054794520547945;
            result[3] += 0.6712328767123288;
            result[4] += 0.0273972602739726;
            result[5] += 0.2808219178082192;
          }
        }
      }
    } else {
      if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)47.5) ) ) {
            result[0] += 0.11428571428571428;
            result[1] += 0.8857142857142857;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.7829754601226994;
            result[1] += 0.019938650306748466;
            result[2] += 0.006901840490797546;
            result[3] += 0.03834355828220859;
            result[4] += 0.12960122699386503;
            result[5] += 0.02223926380368098;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
            result[0] += 0.017094017094017096;
            result[1] += 0.008547008547008548;
            result[2] += 0.08547008547008547;
            result[3] += 0.37606837606837606;
            result[4] += 0.008547008547008548;
            result[5] += 0.5042735042735043;
          } else {
            result[0] += 0.32450331125827814;
            result[1] += 0.026490066225165563;
            result[2] += 0.45695364238410596;
            result[3] += 0.09271523178807947;
            result[4] += 0.0728476821192053;
            result[5] += 0.026490066225165563;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.2692307692307693;
            result[1] += 0.576923076923077;
            result[2] += 0.15384615384615388;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)83.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)58.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1111111111111111;
            result[5] += 0.8888888888888888;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)97.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.029850746268656716;
            result[3] += 0.8507462686567164;
            result[4] += 0;
            result[5] += 0.11940298507462686;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)99.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2786885245901639;
            result[3] += 0.2786885245901639;
            result[4] += 0;
            result[5] += 0.4426229508196721;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9090909090909091;
            result[3] += 0.09090909090909091;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)81) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)103) ) ) {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.07692307692307693;
            result[3] += 0.15384615384615385;
            result[4] += 0.038461538461538464;
            result[5] += 0.7307692307692307;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2857142857142857;
            result[3] += 0.5238095238095238;
            result[4] += 0;
            result[5] += 0.19047619047619047;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.10569105691056911;
            result[1] += 0.032520325203252036;
            result[2] += 0.6829268292682927;
            result[3] += 0.07317073170731707;
            result[4] += 0.024390243902439025;
            result[5] += 0.08130081300813008;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6779661016949152;
            result[3] += 0.3050847457627119;
            result[4] += 0;
            result[5] += 0.01694915254237288;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9066666666666666;
            result[3] += 0.08666666666666667;
            result[4] += 0;
            result[5] += 0.006666666666666667;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9325153374233128;
            result[3] += 0.06748466257668712;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9879518072289156;
            result[3] += 0.012048192771084338;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9451219512195121;
            result[5] += 0.054878048780487805;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3939393939393939;
            result[5] += 0.6060606060606061;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)68.5) ) ) {
            result[0] += 0.05660377358490566;
            result[1] += 0.1320754716981132;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7358490566037735;
            result[5] += 0.07547169811320754;
          } else {
            result[0] += 0.717391304347826;
            result[1] += 0.10869565217391304;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.10869565217391304;
            result[5] += 0.06521739130434782;
          }
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6956521739130435;
            result[5] += 0.30434782608695654;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0024783147459727386;
            result[3] += 0.05328376703841388;
            result[4] += 0.027261462205700124;
            result[5] += 0.9169764560099133;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0.06748466257668712;
            result[2] += 0.012269938650306749;
            result[3] += 0.26993865030674846;
            result[4] += 0.018404907975460124;
            result[5] += 0.6319018404907976;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.9382716049382716;
            result[4] += 0;
            result[5] += 0.06172839506172839;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)45.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)88) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)44) ) ) {
            result[0] += 0;
            result[1] += 0.9534883720930233;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.046511627906976744;
            result[5] += 0;
          } else {
            result[0] += 0.03597122302158273;
            result[1] += 0.05755395683453238;
            result[2] += 0.02158273381294964;
            result[3] += 0.02158273381294964;
            result[4] += 0.8345323741007195;
            result[5] += 0.02877697841726619;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)104.5) ) ) {
            result[0] += 0.8709677419354839;
            result[1] += 0.003395585738539898;
            result[2] += 0.001697792869269949;
            result[3] += 0.02801358234295416;
            result[4] += 0.07640067911714771;
            result[5] += 0.019524617996604415;
          } else {
            result[0] += 0.10344827586206896;
            result[1] += 0.896551724137931;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)91.5) ) ) {
            result[0] += 0.015105740181268883;
            result[1] += 0.030211480362537766;
            result[2] += 0.256797583081571;
            result[3] += 0.27794561933534745;
            result[4] += 0.09667673716012085;
            result[5] += 0.32326283987915405;
          } else {
            result[0] += 0.7317073170731707;
            result[1] += 0.036585365853658534;
            result[2] += 0.10975609756097561;
            result[3] += 0;
            result[4] += 0.12195121951219512;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)90.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.015384615384615385;
            result[3] += 0.07692307692307693;
            result[4] += 0.06153846153846154;
            result[5] += 0.8461538461538461;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)69.5) ) ) {
            result[0] += 0.16129032258064516;
            result[1] += 0.3548387096774194;
            result[2] += 0.22580645161290322;
            result[3] += 0;
            result[4] += 0.16129032258064516;
            result[5] += 0.0967741935483871;
          } else {
            result[0] += 0.0064516129032258064;
            result[1] += 0;
            result[2] += 0.16774193548387098;
            result[3] += 0.6645161290322581;
            result[4] += 0;
            result[5] += 0.16129032258064516;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)104.5) ) ) {
            result[0] += 0.05714285714285714;
            result[1] += 0;
            result[2] += 0.5571428571428572;
            result[3] += 0.34285714285714286;
            result[4] += 0.014285714285714285;
            result[5] += 0.02857142857142857;
          } else {
            result[0] += 0.01282051282051282;
            result[1] += 0;
            result[2] += 0.9230769230769231;
            result[3] += 0.02564102564102564;
            result[4] += 0;
            result[5] += 0.038461538461538464;
          }
        }
      }
    } else {
      if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)98.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.46153846153846156;
            result[3] += 0.5384615384615384;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0967741935483871;
            result[1] += 0;
            result[2] += 0.8064516129032258;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.0967741935483871;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.892018779342723;
            result[3] += 0.107981220657277;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)121.5) ) ) {
            result[0] += 0.875;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0075046904315197;
            result[1] += 0;
            result[2] += 0.9718574108818011;
            result[3] += 0.020637898686679174;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)70.5) ) ) {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)60.5) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)67.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)41) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6;
            result[5] += 0.4;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.125;
            result[4] += 0.875;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7941176470588235;
            result[5] += 0.20588235294117646;
          } else {
            result[0] += 0.005470459518599562;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03282275711159737;
            result[4] += 0.036105032822757115;
            result[5] += 0.925601750547046;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04;
            result[3] += 0.12;
            result[4] += 0.14;
            result[5] += 0.7;
          } else {
            result[0] += 0.013071895424836602;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.6535947712418301;
            result[4] += 0.032679738562091505;
            result[5] += 0.3006535947712418;
          }
        }
      }
    } else {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)93) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)93) ) ) {
            result[0] += 0.027972027972027972;
            result[1] += 0.11188811188811189;
            result[2] += 0;
            result[3] += 0.04895104895104895;
            result[4] += 0.7692307692307693;
            result[5] += 0.04195804195804196;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)72) ) ) {
            result[0] += 0;
            result[1] += 0.375;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0.125;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9950980392156863;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004901960784313725;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)73) ) ) {
            result[0] += 0.8571428571428571;
            result[1] += 0.028735632183908046;
            result[2] += 0.005747126436781609;
            result[3] += 0.022167487684729065;
            result[4] += 0.0665024630541872;
            result[5] += 0.019704433497536946;
          } else {
            result[0] += 0.19444444444444445;
            result[1] += 0.09722222222222222;
            result[2] += 0.11805555555555555;
            result[3] += 0.2638888888888889;
            result[4] += 0.06944444444444445;
            result[5] += 0.2569444444444444;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0.04912280701754386;
            result[1] += 0.010526315789473684;
            result[2] += 0.05964912280701754;
            result[3] += 0.4456140350877193;
            result[4] += 0.10175438596491228;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0.28323699421965315;
            result[1] += 0;
            result[2] += 0.560693641618497;
            result[3] += 0.11560693641618494;
            result[4] += 0.005780346820809247;
            result[5] += 0.03468208092485548;
          }
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)95.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)86) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0.7272727272727273;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2727272727272727;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9565217391304348;
            result[3] += 0;
            result[4] += 0.043478260869565216;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0.041666666666666664;
            result[2] += 0;
            result[3] += 0.08333333333333333;
            result[4] += 0.0625;
            result[5] += 0.8125;
          } else {
            result[0] += 0.3333333333333333;
            result[1] += 0;
            result[2] += 0.5555555555555556;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.1111111111111111;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)77) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0.2222222222222222;
            result[4] += 0;
            result[5] += 0.1111111111111111;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0.6071428571428571;
            result[4] += 0;
            result[5] += 0.25;
          } else {
            result[0] += 0.048387096774193554;
            result[1] += 0;
            result[2] += 0.6612903225806452;
            result[3] += 0.11290322580645162;
            result[4] += 0.01612903225806452;
            result[5] += 0.16129032258064518;
          }
        }
      }
    } else {
      if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)84) ) ) {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)90) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.36363636363636365;
            result[3] += 0.6363636363636364;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)97.5) ) ) {
            result[0] += 0.004545454545454546;
            result[1] += 0;
            result[2] += 0.8454545454545456;
            result[3] += 0.14545454545454548;
            result[4] += 0;
            result[5] += 0.004545454545454546;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.4;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.003367003367003367;
            result[1] += 0;
            result[2] += 0.9696969696969697;
            result[3] += 0.025252525252525252;
            result[4] += 0;
            result[5] += 0.0016835016835016834;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0.009900990099009901;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9801980198019802;
            result[5] += 0.009900990099009901;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)92) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05;
            result[3] += 0;
            result[4] += 0.45;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0.26666666666666666;
            result[2] += 0;
            result[3] += 0.7333333333333333;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)58) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7777777777777778;
            result[5] += 0.2222222222222222;
          } else {
            result[0] += 0.9803921568627451;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0196078431372549;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
            result[0] += 0.0010604453870625664;
            result[1] += 0.003181336161187699;
            result[2] += 0;
            result[3] += 0.05938494167550371;
            result[4] += 0.05726405090137858;
            result[5] += 0.8791092258748674;
          } else {
            result[0] += 0;
            result[1] += 0.00398406374501992;
            result[2] += 0.043824701195219126;
            result[3] += 0.450199203187251;
            result[4] += 0.03187250996015936;
            result[5] += 0.4701195219123506;
          }
        }
      }
    } else {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)56.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 0.08;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.92;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)71) ) ) {
            result[0] += 0.004545454545454545;
            result[1] += 0.975;
            result[2] += 0.0022727272727272726;
            result[3] += 0.004545454545454545;
            result[4] += 0.013636363636363636;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)66.5) ) ) {
            result[0] += 0.09154929577464789;
            result[1] += 0.4647887323943662;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4014084507042254;
            result[5] += 0.04225352112676056;
          } else {
            result[0] += 0.8786231884057971;
            result[1] += 0.010869565217391304;
            result[2] += 0.008152173913043478;
            result[3] += 0.020833333333333332;
            result[4] += 0.058876811594202896;
            result[5] += 0.022644927536231884;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)93) ) ) {
            result[0] += 0.05172413793103449;
            result[1] += 0.004310344827586208;
            result[2] += 0.25000000000000006;
            result[3] += 0.35775862068965525;
            result[4] += 0.060344827586206906;
            result[5] += 0.2758620689655173;
          } else {
            result[0] += 0.732824427480916;
            result[1] += 0;
            result[2] += 0.16030534351145037;
            result[3] += 0.015267175572519083;
            result[4] += 0.08396946564885496;
            result[5] += 0.007633587786259542;
          }
        }
      }
    }
  } else {
    if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7941176470588235;
            result[4] += 0.058823529411764705;
            result[5] += 0.14705882352941177;
          } else {
            result[0] += 0;
            result[1] += 0.04;
            result[2] += 0.06;
            result[3] += 0.32;
            result[4] += 0.08;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0625;
            result[3] += 0;
            result[4] += 0.9375;
            result[5] += 0;
          } else {
            result[0] += 0.058823529411764705;
            result[1] += 0.014705882352941176;
            result[2] += 0;
            result[3] += 0.014705882352941176;
            result[4] += 0.11764705882352941;
            result[5] += 0.7941176470588235;
          }
        }
      } else {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)93) ) ) {
          if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4117647058823529;
            result[3] += 0.23529411764705882;
            result[4] += 0;
            result[5] += 0.35294117647058826;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.875;
            result[3] += 0.041666666666666664;
            result[4] += 0.08333333333333333;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)77.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)77) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04347826086956522;
            result[3] += 0.9130434782608696;
            result[4] += 0;
            result[5] += 0.04347826086956522;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.44660194174757284;
            result[3] += 0.39805825242718446;
            result[4] += 0;
            result[5] += 0.1553398058252427;
          } else {
            result[0] += 0.005208333333333333;
            result[1] += 0;
            result[2] += 0.8541666666666666;
            result[3] += 0.13541666666666666;
            result[4] += 0;
            result[5] += 0.005208333333333333;
          }
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)82.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.625;
            result[4] += 0;
            result[5] += 0.125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.875;
            result[3] += 0.10416666666666667;
            result[4] += 0;
            result[5] += 0.020833333333333332;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8333333333333334;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.003305785123966942;
            result[1] += 0;
            result[2] += 0.9834710743801653;
            result[3] += 0.013223140495867768;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
            result[0] += 0.013953488372093025;
            result[1] += 0.004651162790697675;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9767441860465117;
            result[5] += 0.004651162790697675;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7058823529411765;
            result[5] += 0.29411764705882354;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)59.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.014492753623188406;
            result[1] += 0.014492753623188406;
            result[2] += 0;
            result[3] += 0.10144927536231885;
            result[4] += 0.391304347826087;
            result[5] += 0.4782608695652174;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.15625;
            result[3] += 0;
            result[4] += 0.8125;
            result[5] += 0.03125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.029069767441860465;
            result[4] += 0.023255813953488372;
            result[5] += 0.9476744186046512;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)87.5) ) ) {
            result[0] += 0.004784688995215311;
            result[1] += 0;
            result[2] += 0.014354066985645933;
            result[3] += 0.15311004784688995;
            result[4] += 0.17703349282296652;
            result[5] += 0.6507177033492823;
          } else {
            result[0] += 0.008547008547008548;
            result[1] += 0.029914529914529916;
            result[2] += 0.008547008547008548;
            result[3] += 0.5641025641025641;
            result[4] += 0.017094017094017096;
            result[5] += 0.3717948717948718;
          }
        }
      }
    } else {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)50) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9975;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0025;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)99.5) ) ) {
            result[0] += 0.02127659574468085;
            result[1] += 0.010638297872340425;
            result[2] += 0.031914893617021274;
            result[3] += 0.031914893617021274;
            result[4] += 0.9042553191489362;
            result[5] += 0;
          } else {
            result[0] += 0.04545454545454546;
            result[1] += 0.9090909090909092;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.04545454545454546;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)69.5) ) ) {
            result[0] += 0.05454545454545454;
            result[1] += 0;
            result[2] += 0.10909090909090909;
            result[3] += 0.07272727272727272;
            result[4] += 0.2727272727272727;
            result[5] += 0.4909090909090909;
          } else {
            result[0] += 0.8887884267631103;
            result[1] += 0.022603978300180832;
            result[2] += 0.006329113924050633;
            result[3] += 0.018083182640144666;
            result[4] += 0.059674502712477394;
            result[5] += 0.0045207956600361665;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)89) ) ) {
            result[0] += 0.03870967741935484;
            result[1] += 0.04516129032258064;
            result[2] += 0.1870967741935484;
            result[3] += 0.3870967741935484;
            result[4] += 0.05806451612903226;
            result[5] += 0.2838709677419355;
          } else {
            result[0] += 0.819672131147541;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.18032786885245902;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)82.5) ) ) {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)74) ) ) {
            result[0] += 0;
            result[1] += 0.15384615384615385;
            result[2] += 0;
            result[3] += 0.07692307692307693;
            result[4] += 0.46153846153846156;
            result[5] += 0.3076923076923077;
          } else {
            result[0] += 0;
            result[1] += 0.024390243902439025;
            result[2] += 0;
            result[3] += 0.07317073170731707;
            result[4] += 0;
            result[5] += 0.9024390243902439;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3333333333333333;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          } else {
            result[0] += 0;
            result[1] += 0.012269938650306749;
            result[2] += 0.19631901840490798;
            result[3] += 0.7055214723926381;
            result[4] += 0;
            result[5] += 0.08588957055214724;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.375;
            result[2] += 0.375;
            result[3] += 0.125;
            result[4] += 0.125;
            result[5] += 0;
          } else {
            result[0] += 0.9375;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0625;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)94.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.06666666666666667;
            result[3] += 0.4666666666666667;
            result[4] += 0;
            result[5] += 0.4666666666666667;
          } else {
            result[0] += 0.0787878787878788;
            result[1] += 0;
            result[2] += 0.7515151515151516;
            result[3] += 0.1575757575757576;
            result[4] += 0;
            result[5] += 0.012121212121212123;
          }
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0.1111111111111111;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6842105263157895;
            result[3] += 0.15789473684210525;
            result[4] += 0;
            result[5] += 0.15789473684210525;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0.08571428571428572;
            result[2] += 0.6285714285714286;
            result[3] += 0.02857142857142857;
            result[4] += 0;
            result[5] += 0.2571428571428571;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.872;
            result[3] += 0.104;
            result[4] += 0;
            result[5] += 0.024;
          } else {
            result[0] += 0.003669724770642202;
            result[1] += 0;
            result[2] += 0.9798165137614679;
            result[3] += 0.01651376146788991;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)88.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0.0045871559633027525;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9954128440366973;
            result[5] += 0;
          } else {
            result[0] += 0.09523809523809523;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7142857142857143;
            result[5] += 0.19047619047619047;
          }
        } else {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)68.5) ) ) {
            result[0] += 0.022988505747126436;
            result[1] += 0.034482758620689655;
            result[2] += 0;
            result[3] += 0.05747126436781609;
            result[4] += 0.4827586206896552;
            result[5] += 0.40229885057471265;
          } else {
            result[0] += 0.8571428571428571;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.02040816326530612;
            result[4] += 0.07142857142857142;
            result[5] += 0.05102040816326531;
          }
        }
      } else {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)60.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0.022727272727272728;
            result[2] += 0.022727272727272728;
            result[3] += 0.06818181818181818;
            result[4] += 0.7727272727272727;
            result[5] += 0.11363636363636363;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0.0038022813688212928;
            result[2] += 0;
            result[3] += 0.050697084917617236;
            result[4] += 0.02788339670468948;
            result[5] += 0.917617237008872;
          } else {
            result[0] += 0.03205128205128205;
            result[1] += 0;
            result[2] += 0.00641025641025641;
            result[3] += 0.1987179487179487;
            result[4] += 0.1794871794871795;
            result[5] += 0.5833333333333334;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)95) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)104.5) ) ) {
            result[0] += 0.045454545454545456;
            result[1] += 0.045454545454545456;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8863636363636364;
            result[5] += 0.022727272727272728;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)85.5) ) ) {
            result[0] += 0.0023809523809523816;
            result[1] += 0.9833333333333334;
            result[2] += 0;
            result[3] += 0.004761904761904763;
            result[4] += 0.009523809523809526;
            result[5] += 0;
          } else {
            result[0] += 0.75;
            result[1] += 0.25;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)74.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
            result[0] += 0.8125;
            result[1] += 0.09375;
            result[2] += 0.046875;
            result[3] += 0;
            result[4] += 0.046875;
            result[5] += 0;
          } else {
            result[0] += 0.00872093023255814;
            result[1] += 0.0436046511627907;
            result[2] += 0.14244186046511628;
            result[3] += 0.43023255813953487;
            result[4] += 0.020348837209302327;
            result[5] += 0.3546511627906977;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)69.5) ) ) {
            result[0] += 0.109375;
            result[1] += 0.40625;
            result[2] += 0.03125;
            result[3] += 0.03125;
            result[4] += 0.375;
            result[5] += 0.046875;
          } else {
            result[0] += 0.8307291666666667;
            result[1] += 0.002604166666666667;
            result[2] += 0.04166666666666667;
            result[3] += 0.05208333333333334;
            result[4] += 0.045138888888888895;
            result[5] += 0.02777777777777778;
          }
        }
      }
    }
  } else {
    if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.07692307692307693;
            result[4] += 0.38461538461538464;
            result[5] += 0.5384615384615384;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.12903225806451613;
            result[3] += 0.18548387096774194;
            result[4] += 0.04838709677419355;
            result[5] += 0.6370967741935484;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.45454545454545453;
            result[3] += 0.5454545454545454;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)99) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0.5777777777777777;
            result[4] += 0.044444444444444446;
            result[5] += 0.26666666666666666;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6440677966101694;
            result[3] += 0.2542372881355932;
            result[4] += 0;
            result[5] += 0.1016949152542373;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)69) ) ) {
            result[0] += 0.3888888888888889;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0.2777777777777778;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8208955223880597;
            result[3] += 0;
            result[4] += 0.029850746268656716;
            result[5] += 0.14925373134328357;
          }
        }
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)93) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.18181818181818182;
            result[3] += 0.8181818181818182;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.680672268907563;
            result[3] += 0.3025210084033613;
            result[4] += 0;
            result[5] += 0.01680672268907563;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.96;
            result[3] += 0.04;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)69) ) ) {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)77) ) ) {
            result[0] += 0.3333333333333333;
            result[1] += 0;
            result[2] += 0.5555555555555556;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0014044943820224719;
            result[1] += 0;
            result[2] += 0.9620786516853933;
            result[3] += 0.03651685393258427;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)81) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4444444444444444;
            result[5] += 0.5555555555555556;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)76.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0.12121212121212122;
            result[4] += 0.18181818181818182;
            result[5] += 0.030303030303030304;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.46808510638297873;
            result[5] += 0.5319148936170213;
          } else {
            result[0] += 0.004862236628849271;
            result[1] += 0;
            result[2] += 0.0016207455429497568;
            result[3] += 0.017828200972447326;
            result[4] += 0.0016207455429497568;
            result[5] += 0.9740680713128039;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)55.5) ) ) {
            result[0] += 0.8461538461538461;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07692307692307693;
            result[5] += 0.07692307692307693;
          } else {
            result[0] += 0.017204301075268817;
            result[1] += 0.017204301075268817;
            result[2] += 0.021505376344086023;
            result[3] += 0.3161290322580645;
            result[4] += 0.08602150537634409;
            result[5] += 0.5419354838709678;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0.07142857142857142;
            result[2] += 0;
            result[3] += 0.14285714285714285;
            result[4] += 0.7857142857142857;
            result[5] += 0;
          } else {
            result[0] += 0.08571428571428572;
            result[1] += 0.8571428571428571;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.05714285714285714;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)90.5) ) ) {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)73) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)99) ) ) {
            result[0] += 0.5667447306791569;
            result[1] += 0.03044496487119438;
            result[2] += 0;
            result[3] += 0.06791569086651054;
            result[4] += 0.2927400468384075;
            result[5] += 0.04215456674473068;
          } else {
            result[0] += 0.910377358490566;
            result[1] += 0.020047169811320754;
            result[2] += 0;
            result[3] += 0.01179245283018868;
            result[4] += 0.05188679245283019;
            result[5] += 0.00589622641509434;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)109.5) ) ) {
            result[0] += 0.038461538461538464;
            result[1] += 0.07371794871794872;
            result[2] += 0.1794871794871795;
            result[3] += 0.2564102564102564;
            result[4] += 0.14102564102564102;
            result[5] += 0.3108974358974359;
          } else {
            result[0] += 0.6702127659574468;
            result[1] += 0;
            result[2] += 0.2765957446808511;
            result[3] += 0.010638297872340425;
            result[4] += 0.0425531914893617;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)99) ) ) {
      if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)80.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)66) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9090909090909091;
            result[5] += 0.09090909090909091;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.08943089430894309;
            result[3] += 0.5203252032520326;
            result[4] += 0;
            result[5] += 0.3902439024390244;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)98) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)61) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.09090909090909091;
            result[4] += 0.9090909090909091;
            result[5] += 0;
          } else {
            result[0] += 0.05263157894736842;
            result[1] += 0;
            result[2] += 0.10526315789473684;
            result[3] += 0.05263157894736842;
            result[4] += 0.05263157894736842;
            result[5] += 0.7368421052631579;
          }
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8666666666666667;
            result[3] += 0.06666666666666667;
            result[4] += 0;
            result[5] += 0.06666666666666667;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          }
        }
      }
    } else {
      if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)103) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)94.5) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.375;
            result[4] += 0;
            result[5] += 0.625;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.15;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0.05;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.49056603773584906;
            result[3] += 0.39622641509433965;
            result[4] += 0;
            result[5] += 0.11320754716981132;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9838709677419355;
            result[3] += 0.016129032258064516;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)92) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8666666666666667;
            result[3] += 0;
            result[4] += 0.13333333333333333;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)87.5) ) ) {
            result[0] += 0.006329113924050633;
            result[1] += 0;
            result[2] += 0.8322784810126582;
            result[3] += 0.1518987341772152;
            result[4] += 0;
            result[5] += 0.00949367088607595;
          } else {
            result[0] += 0.0020325203252032522;
            result[1] += 0;
            result[2] += 0.967479674796748;
            result[3] += 0.03048780487804878;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 0.0047169811320754715;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9716981132075472;
            result[5] += 0.02358490566037736;
          } else {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)54) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.08860759493670886;
            result[1] += 0.02531645569620253;
            result[2] += 0;
            result[3] += 0.1518987341772152;
            result[4] += 0.3924050632911392;
            result[5] += 0.34177215189873417;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)57.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8823529411764706;
            result[5] += 0.11764705882352941;
          } else {
            result[0] += 0;
            result[1] += 0.005263157894736842;
            result[2] += 0.003947368421052632;
            result[3] += 0.021052631578947368;
            result[4] += 0.02763157894736842;
            result[5] += 0.9421052631578948;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)83.5) ) ) {
            result[0] += 0.007246376811594203;
            result[1] += 0.018115942028985508;
            result[2] += 0;
            result[3] += 0.20652173913043478;
            result[4] += 0.09420289855072464;
            result[5] += 0.6739130434782609;
          } else {
            result[0] += 0.015228426395939087;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.6548223350253807;
            result[4] += 0;
            result[5] += 0.3299492385786802;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)93.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
            result[0] += 0.02702702702702703;
            result[1] += 0.08108108108108109;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8783783783783784;
            result[5] += 0.013513513513513514;
          } else {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0.2222222222222222;
            result[4] += 0.1111111111111111;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)55.5) ) ) {
            result[0] += 0;
            result[1] += 0.9954337899543378;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0045662100456621;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.4;
            result[2] += 0.06666666666666667;
            result[3] += 0;
            result[4] += 0.5333333333333333;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)68.5) ) ) {
            result[0] += 0.18032786885245902;
            result[1] += 0.3442622950819672;
            result[2] += 0.03278688524590164;
            result[3] += 0.01639344262295082;
            result[4] += 0.39344262295081966;
            result[5] += 0.03278688524590164;
          } else {
            result[0] += 0.8799328295549956;
            result[1] += 0.023509655751469346;
            result[2] += 0.0050377833753148605;
            result[3] += 0.02854743912678421;
            result[4] += 0.03694374475230898;
            result[5] += 0.026028547439126776;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0.02617801047120419;
            result[1] += 0.005235602094240838;
            result[2] += 0.08900523560209424;
            result[3] += 0.47643979057591623;
            result[4] += 0.08376963350785341;
            result[5] += 0.3193717277486911;
          } else {
            result[0] += 0.2373737373737374;
            result[1] += 0;
            result[2] += 0.47979797979797983;
            result[3] += 0.08080808080808083;
            result[4] += 0.12121212121212123;
            result[5] += 0.08080808080808083;
          }
        }
      }
    }
  } else {
    if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)97.5) ) ) {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)77) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.038461538461538464;
            result[4] += 0.15384615384615385;
            result[5] += 0.8076923076923077;
          } else {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.9333333333333333;
            result[4] += 0;
            result[5] += 0.06666666666666667;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0.2222222222222222;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          }
        }
      } else {
        if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)82) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0.04;
            result[2] += 0.04;
            result[3] += 0.88;
            result[4] += 0;
            result[5] += 0.04;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)100) ) ) {
            result[0] += 0.05263157894736842;
            result[1] += 0;
            result[2] += 0.6052631578947368;
            result[3] += 0.2631578947368421;
            result[4] += 0.02631578947368421;
            result[5] += 0.05263157894736842;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)97.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)84) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)82.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5555555555555556;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.18181818181818182;
            result[3] += 0.8181818181818182;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)77) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0.07142857142857142;
            result[2] += 0.9285714285714286;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.875;
            result[1] += 0.125;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7666666666666667;
            result[3] += 0.20555555555555555;
            result[4] += 0;
            result[5] += 0.027777777777777776;
          } else {
            result[0] += 0.007215007215007215;
            result[1] += 0;
            result[2] += 0.950937950937951;
            result[3] += 0.04184704184704185;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)48.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)52) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)41) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0038022813688212928;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9657794676806084;
            result[5] += 0.030418250950570342;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)56) ) ) {
            result[0] += 0.8181818181818182;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.18181818181818182;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.002190580503833516;
            result[2] += 0.001095290251916758;
            result[3] += 0.04271631982475356;
            result[4] += 0.038335158817086525;
            result[5] += 0.9156626506024096;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)82) ) ) {
            result[0] += 0.3273809523809524;
            result[1] += 0.041666666666666664;
            result[2] += 0;
            result[3] += 0.07738095238095238;
            result[4] += 0.125;
            result[5] += 0.42857142857142855;
          } else {
            result[0] += 0.04240282685512368;
            result[1] += 0.007067137809187279;
            result[2] += 0.0706713780918728;
            result[3] += 0.5300353356890459;
            result[4] += 0.0035335689045936395;
            result[5] += 0.3462897526501767;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)93.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0.375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4375;
            result[5] += 0.1875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9866666666666667;
            result[5] += 0.013333333333333334;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)55) ) ) {
            result[0] += 0;
            result[1] += 0.9957173447537473;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.004282655246252677;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.5909090909090909;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4090909090909091;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
            result[0] += 0.9057971014492754;
            result[1] += 0.014492753623188406;
            result[2] += 0.021739130434782608;
            result[3] += 0;
            result[4] += 0.057971014492753624;
            result[5] += 0;
          } else {
            result[0] += 0.08374384236453201;
            result[1] += 0.06896551724137931;
            result[2] += 0.014778325123152709;
            result[3] += 0.3448275862068966;
            result[4] += 0.19704433497536947;
            result[5] += 0.29064039408866993;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75.5) ) ) {
            result[0] += 0.9229885057471265;
            result[1] += 0.01609195402298851;
            result[2] += 0.001149425287356322;
            result[3] += 0.006896551724137932;
            result[4] += 0.05287356321839081;
            result[5] += 0;
          } else {
            result[0] += 0.28431372549019607;
            result[1] += 0;
            result[2] += 0.5882352941176471;
            result[3] += 0.049019607843137254;
            result[4] += 0.0196078431372549;
            result[5] += 0.058823529411764705;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)86) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)73.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 1;
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)46) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7916666666666666;
            result[5] += 0.20833333333333334;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)64) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.42857142857142855;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5714285714285714;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.12408759124087591;
            result[3] += 0.6934306569343066;
            result[4] += 0;
            result[5] += 0.18248175182481752;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)89) ) ) {
            result[0] += 0.047619047619047616;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8095238095238095;
          } else {
            result[0] += 0.020000000000000004;
            result[1] += 0.04000000000000001;
            result[2] += 0.6400000000000001;
            result[3] += 0.16000000000000003;
            result[4] += 0.060000000000000005;
            result[5] += 0.08000000000000002;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)103) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.8;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.00819672131147541;
            result[1] += 0;
            result[2] += 0.6967213114754098;
            result[3] += 0.23770491803278687;
            result[4] += 0;
            result[5] += 0.05737704918032787;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)77) ) ) {
            result[0] += 0.5909090909090909;
            result[1] += 0;
            result[2] += 0.4090909090909091;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8910256410256411;
            result[3] += 0.09935897435897435;
            result[4] += 0;
            result[5] += 0.009615384615384616;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)81) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0.25;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9223300970873787;
            result[3] += 0.06796116504854369;
            result[4] += 0;
            result[5] += 0.009708737864077669;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9974683544303797;
            result[3] += 0.002531645569620253;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)89.5) ) ) {
      if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)45.5) ) ) {
            result[0] += 0;
            result[1] += 0.7368421052631579;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2631578947368421;
            result[5] += 0;
          } else {
            result[0] += 0.019083969465648856;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9541984732824428;
            result[5] += 0.026717557251908396;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
            result[0] += 0.9101123595505618;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0449438202247191;
            result[5] += 0.0449438202247191;
          } else {
            result[0] += 0.125;
            result[1] += 0.046875;
            result[2] += 0;
            result[3] += 0.03125;
            result[4] += 0.265625;
            result[5] += 0.53125;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)65) ) ) {
            result[0] += 0.0196078431372549;
            result[1] += 0.0196078431372549;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9411764705882353;
            result[5] += 0.0196078431372549;
          } else {
            result[0] += 0;
            result[1] += 0.08333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.4166666666666667;
            result[5] += 0.5;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0.0016260162601626016;
            result[2] += 0;
            result[3] += 0.02113821138211382;
            result[4] += 0.01951219512195122;
            result[5] += 0.9577235772357724;
          } else {
            result[0] += 0.010498687664041995;
            result[1] += 0.0026246719160104987;
            result[2] += 0;
            result[3] += 0.2230971128608924;
            result[4] += 0.09973753280839895;
            result[5] += 0.6640419947506562;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)54.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)74) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)73) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)71.5) ) ) {
            result[0] += 0.16541353383458646;
            result[1] += 0.20300751879699247;
            result[2] += 0;
            result[3] += 0.015037593984962405;
            result[4] += 0.6165413533834586;
            result[5] += 0;
          } else {
            result[0] += 0.8542056074766357;
            result[1] += 0.006542056074766356;
            result[2] += 0.009345794392523366;
            result[3] += 0.042990654205607486;
            result[4] += 0.042056074766355145;
            result[5] += 0.04485981308411215;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)87.5) ) ) {
            result[0] += 0.00881057268722467;
            result[1] += 0.02643171806167401;
            result[2] += 0.16299559471365638;
            result[3] += 0.4889867841409692;
            result[4] += 0.048458149779735685;
            result[5] += 0.2643171806167401;
          } else {
            result[0] += 0.6011904761904763;
            result[1] += 0;
            result[2] += 0.23809523809523814;
            result[3] += 0.04166666666666667;
            result[4] += 0.07142857142857144;
            result[5] += 0.04761904761904763;
          }
        }
      }
    }
  } else {
    if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)103) ) ) {
      if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)77) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1111111111111111;
            result[5] += 0.8888888888888888;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.9411764705882353;
            result[4] += 0;
            result[5] += 0.058823529411764705;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4666666666666667;
            result[3] += 0.43333333333333335;
            result[4] += 0;
            result[5] += 0.1;
          }
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8877551020408163;
            result[3] += 0.08163265306122448;
            result[4] += 0;
            result[5] += 0.030612244897959183;
          }
        }
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)93) ) ) {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)90) ) ) {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)95) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.42857142857142855;
            result[3] += 0.5714285714285714;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)88) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.42857142857142855;
            result[3] += 0.5714285714285714;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0025412960609911056;
            result[1] += 0;
            result[2] += 0.9415501905972046;
            result[3] += 0.048284625158831;
            result[4] += 0;
            result[5] += 0.007623888182973317;
          }
        }
      }
    }
  }
  if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)43.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9955357142857143;
            result[5] += 0.004464285714285714;
          } else {
            result[0] += 0.125;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.625;
            result[5] += 0.25;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6829268292682927;
            result[5] += 0.3170731707317073;
          } else {
            result[0] += 0.0028694404591104736;
            result[1] += 0.0014347202295552368;
            result[2] += 0.00860832137733142;
            result[3] += 0.03443328550932568;
            result[4] += 0.027259684361549498;
            result[5] += 0.9253945480631277;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)58) ) ) {
            result[0] += 0.9583333333333334;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.041666666666666664;
            result[5] += 0;
          } else {
            result[0] += 0.006109979633401222;
            result[1] += 0.002036659877800407;
            result[2] += 0.002036659877800407;
            result[3] += 0.3543788187372709;
            result[4] += 0.08961303462321792;
            result[5] += 0.5458248472505092;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)70) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)41) ) ) {
            result[0] += 0;
            result[1] += 0.9850746268656716;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.014925373134328358;
            result[5] += 0;
          } else {
            result[0] += 0.13071895424836602;
            result[1] += 0.1503267973856209;
            result[2] += 0;
            result[3] += 0.006535947712418301;
            result[4] += 0.7058823529411765;
            result[5] += 0.006535947712418301;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
            result[0] += 0.850253807106599;
            result[1] += 0.015228426395939087;
            result[2] += 0.03553299492385787;
            result[3] += 0.02961082910321489;
            result[4] += 0.04060913705583756;
            result[5] += 0.028764805414551606;
          } else {
            result[0] += 0.19742489270386265;
            result[1] += 0.012875536480686695;
            result[2] += 0.20600858369098712;
            result[3] += 0.3218884120171674;
            result[4] += 0.06866952789699571;
            result[5] += 0.19313304721030042;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)126.5) ) ) {
            result[0] += 0;
            result[1] += 0.967741935483871;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.03225806451612903;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  } else {
    if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)98.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)84.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)69) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0.027777777777777776;
            result[2] += 0;
            result[3] += 0.027777777777777776;
            result[4] += 0.75;
            result[5] += 0.19444444444444445;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)110.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1165644171779141;
            result[3] += 0.6012269938650306;
            result[4] += 0.006134969325153374;
            result[5] += 0.27607361963190186;
          } else {
            result[0] += 0.14285714285714285;
            result[1] += 0;
            result[2] += 0.32142857142857145;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5357142857142857;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)98) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)99.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.625;
            result[3] += 0;
            result[4] += 0.125;
            result[5] += 0.25;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)102.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9655172413793104;
            result[3] += 0.034482758620689655;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0.3333333333333333;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)85.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)92.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.75;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4807692307692308;
            result[3] += 0.34615384615384615;
            result[4] += 0;
            result[5] += 0.17307692307692307;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8106995884773662;
            result[3] += 0.18930041152263374;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)103.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.8571428571428571;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)74.5) ) ) {
            result[0] += 0.23076923076923078;
            result[1] += 0;
            result[2] += 0.6153846153846154;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.15384615384615385;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9612141652613828;
            result[3] += 0.0387858347386172;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)62.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9951923076923077;
            result[5] += 0.004807692307692308;
          } else {
            result[0] += 0;
            result[1] += 0.047619047619047616;
            result[2] += 0.09523809523809523;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0.19047619047619047;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)57.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.09090909090909091;
            result[4] += 0.9090909090909091;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.16666666666666666;
            result[5] += 0.8333333333333334;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)65.5) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)81.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.014957264957264958;
            result[4] += 0.014957264957264958;
            result[5] += 0.9700854700854701;
          } else {
            result[0] += 0;
            result[1] += 0.007017543859649123;
            result[2] += 0;
            result[3] += 0.10175438596491228;
            result[4] += 0.08421052631578947;
            result[5] += 0.8070175438596491;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)58) ) ) {
            result[0] += 0.891304347826087;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.08695652173913045;
            result[5] += 0.02173913043478261;
          } else {
            result[0] += 0.0022123893805309734;
            result[1] += 0.008849557522123894;
            result[2] += 0;
            result[3] += 0.38495575221238937;
            result[4] += 0.10176991150442478;
            result[5] += 0.5022123893805309;
          }
        }
      }
    } else {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)75) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0.16666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8333333333333334;
            result[5] += 0;
          } else {
            result[0] += 0.006423982869379015;
            result[1] += 0.9850107066381156;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.008565310492505354;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)50.5) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)70.5) ) ) {
            result[0] += 0.18260869565217394;
            result[1] += 0.2521739130434783;
            result[2] += 0.013043478260869566;
            result[3] += 0.013043478260869566;
            result[4] += 0.5260869565217392;
            result[5] += 0.013043478260869566;
          } else {
            result[0] += 0.8927875243664717;
            result[1] += 0.011695906432748537;
            result[2] += 0.003898635477582846;
            result[3] += 0.024366471734892786;
            result[4] += 0.05165692007797271;
            result[5] += 0.015594541910331383;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)111.5) ) ) {
            result[0] += 0.06666666666666668;
            result[1] += 0.005128205128205129;
            result[2] += 0.34871794871794876;
            result[3] += 0.3025641025641026;
            result[4] += 0.03846153846153847;
            result[5] += 0.2384615384615385;
          } else {
            result[0] += 0.7872340425531915;
            result[1] += 0.031914893617021274;
            result[2] += 0.10638297872340426;
            result[3] += 0;
            result[4] += 0.05319148936170213;
            result[5] += 0.02127659574468085;
          }
        }
      }
    }
  } else {
    if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0.2631578947368421;
            result[2] += 0.15789473684210525;
            result[3] += 0;
            result[4] += 0.5789473684210527;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1388888888888889;
            result[3] += 0;
            result[4] += 0.1388888888888889;
            result[5] += 0.7222222222222222;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)70) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.08333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.9166666666666666;
          } else {
            result[0] += 0.011627906976744186;
            result[1] += 0;
            result[2] += 0.23255813953488372;
            result[3] += 0.6511627906976745;
            result[4] += 0.023255813953488372;
            result[5] += 0.08139534883720931;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)80) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.45454545454545453;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.5454545454545454;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.23913043478260873;
            result[3] += 0.6956521739130436;
            result[4] += 0;
            result[5] += 0.06521739130434784;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)91.5) ) ) {
            result[0] += 0.16666666666666666;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0.5;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0425531914893617;
            result[1] += 0.010638297872340425;
            result[2] += 0.7978723404255319;
            result[3] += 0.10638297872340426;
            result[4] += 0;
            result[5] += 0.0425531914893617;
          }
        }
      } else {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)105.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.6666666666666666;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8394160583941606;
            result[3] += 0.15328467153284672;
            result[4] += 0;
            result[5] += 0.0072992700729927005;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)86.5) ) ) {
            result[0] += 0.2;
            result[1] += 0;
            result[2] += 0.8;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9851485148514851;
            result[3] += 0.01485148514851485;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)91.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)65) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)97.5) ) ) {
            result[0] += 0;
            result[1] += 0.015625;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.95703125;
            result[5] += 0.02734375;
          } else {
            result[0] += 0.05263157894736842;
            result[1] += 0.6842105263157895;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.2631578947368421;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0.75;
          } else {
            result[0] += 0;
            result[1] += 0.09375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.71875;
            result[5] += 0.1875;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)58) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.9338842975206614;
            result[1] += 0.016528925619834715;
            result[2] += 0;
            result[3] += 0.016528925619834715;
            result[4] += 0.016528925619834715;
            result[5] += 0.016528925619834715;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
            result[0] += 0.0013440860215053765;
            result[1] += 0;
            result[2] += 0.005376344086021506;
            result[3] += 0.03494623655913978;
            result[4] += 0.036290322580645164;
            result[5] += 0.9220430107526881;
          } else {
            result[0] += 0.07722772277227723;
            result[1] += 0.011881188118811881;
            result[2] += 0.0019801980198019802;
            result[3] += 0.25544554455445545;
            result[4] += 0.12673267326732673;
            result[5] += 0.5267326732673268;
          }
        }
      }
    } else {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)73) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)72.5) ) ) {
            result[0] += 0.007751937984496124;
            result[1] += 0.4573643410852713;
            result[2] += 0.031007751937984496;
            result[3] += 0.007751937984496124;
            result[4] += 0.4883720930232558;
            result[5] += 0.007751937984496124;
          } else {
            result[0] += 0.8722772277227723;
            result[1] += 0.015841584158415842;
            result[2] += 0.02574257425742574;
            result[3] += 0.03861386138613861;
            result[4] += 0.030693069306930693;
            result[5] += 0.016831683168316833;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80.5) ) ) {
            result[0] += 0.007462686567164179;
            result[1] += 0.018656716417910446;
            result[2] += 0.08582089552238806;
            result[3] += 0.5261194029850746;
            result[4] += 0.04477611940298507;
            result[5] += 0.31716417910447764;
          } else {
            result[0] += 0.33796296296296297;
            result[1] += 0.037037037037037035;
            result[2] += 0.44907407407407407;
            result[3] += 0.06481481481481481;
            result[4] += 0.05555555555555555;
            result[5] += 0.05555555555555555;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)87.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)54.5) ) ) {
            result[0] += 0;
            result[1] += 0.9974874371859297;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002512562814070352;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)88.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)64.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3;
            result[3] += 0;
            result[4] += 0.7;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.06451612903225806;
            result[4] += 0;
            result[5] += 0.9354838709677419;
          }
        } else {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        }
      } else {
        if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)101.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.10416666666666667;
            result[3] += 0.7708333333333334;
            result[4] += 0;
            result[5] += 0.125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5116279069767442;
            result[3] += 0.3023255813953488;
            result[4] += 0;
            result[5] += 0.18604651162790697;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7906976744186047;
            result[3] += 0.16279069767441864;
            result[4] += 0;
            result[5] += 0.04651162790697675;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)108.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)100) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7398373983739838;
            result[3] += 0.23577235772357724;
            result[4] += 0;
            result[5] += 0.024390243902439025;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)130.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8698224852071006;
            result[3] += 0.1301775147928994;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)84.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9259259259259259;
            result[3] += 0.07407407407407407;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9936974789915967;
            result[3] += 0.0063025210084033615;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)65) ) ) {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)99.5) ) ) {
            result[0] += 0.004132231404958678;
            result[1] += 0.008264462809917356;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9710743801652892;
            result[5] += 0.01652892561983471;
          } else {
            result[0] += 0;
            result[1] += 0.9;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)56.5) ) ) {
            result[0] += 0.825;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.175;
            result[5] += 0;
          } else {
            result[0] += 0.02564102564102564;
            result[1] += 0.28205128205128205;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.20512820512820512;
            result[5] += 0.48717948717948717;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)82) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0.002551020408163265;
            result[2] += 0;
            result[3] += 0.036989795918367346;
            result[4] += 0.03826530612244898;
            result[5] += 0.9221938775510204;
          } else {
            result[0] += 0;
            result[1] += 0.5625;
            result[2] += 0;
            result[3] += 0.375;
            result[4] += 0.0625;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)87.5) ) ) {
            result[0] += 0.009900990099009901;
            result[1] += 0;
            result[2] += 0.009900990099009901;
            result[3] += 0.16831683168316833;
            result[4] += 0.07425742574257425;
            result[5] += 0.7376237623762376;
          } else {
            result[0] += 0.0056179775280898875;
            result[1] += 0.011235955056179775;
            result[2] += 0.0056179775280898875;
            result[3] += 0.6966292134831461;
            result[4] += 0.011235955056179775;
            result[5] += 0.2696629213483146;
          }
        }
      }
    } else {
      if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
            result[0] += 0.021164021164021163;
            result[1] += 0.4021164021164021;
            result[2] += 0;
            result[3] += 0.010582010582010581;
            result[4] += 0.5608465608465608;
            result[5] += 0.005291005291005291;
          } else {
            result[0] += 0.8281758957654723;
            result[1] += 0.012214983713355049;
            result[2] += 0.003257328990228013;
            result[3] += 0.026058631921824105;
            result[4] += 0.08631921824104234;
            result[5] += 0.043973941368078175;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)85.5) ) ) {
            result[0] += 0.016611295681063124;
            result[1] += 0.009966777408637873;
            result[2] += 0.23588039867109634;
            result[3] += 0.4584717607973422;
            result[4] += 0.03986710963455149;
            result[5] += 0.23920265780730898;
          } else {
            result[0] += 0.45714285714285713;
            result[1] += 0.007142857142857143;
            result[2] += 0.3357142857142857;
            result[3] += 0.08571428571428572;
            result[4] += 0.09285714285714286;
            result[5] += 0.02142857142857143;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)58.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)112) ) ) {
            result[0] += 0;
            result[1] += 0.3;
            result[2] += 0;
            result[3] += 0.3;
            result[4] += 0.4;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)75.5) ) ) {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)89) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.5714285714285714;
            result[4] += 0;
            result[5] += 0.42857142857142855;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.043478260869565216;
            result[5] += 0.9565217391304348;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)103.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)101.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.11764705882352941;
            result[3] += 0.8529411764705882;
            result[4] += 0;
            result[5] += 0.029411764705882353;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0.3055555555555556;
            result[4] += 0;
            result[5] += 0.027777777777777776;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.84;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.16;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)105.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)113) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7197452229299363;
            result[3] += 0.2611464968152866;
            result[4] += 0;
            result[5] += 0.01910828025477707;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5384615384615384;
            result[3] += 0.46153846153846156;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9565217391304348;
            result[3] += 0.043478260869565216;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.003246753246753247;
            result[1] += 0;
            result[2] += 0.9756493506493507;
            result[3] += 0.017857142857142856;
            result[4] += 0;
            result[5] += 0.003246753246753247;
          }
        }
      }
    }
  }
  if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)81) ) ) {
    if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)74.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)99) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
            result[0] += 0.003968253968253968;
            result[1] += 0.01984126984126984;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9603174603174603;
            result[5] += 0.015873015873015872;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.043478260869565216;
            result[3] += 0;
            result[4] += 0.4927536231884058;
            result[5] += 0.463768115942029;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)74) ) ) {
            result[0] += 0;
            result[1] += 0.88;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.12;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
            result[0] += 0.6938775510204082;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.30612244897959184;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0012091898428053204;
            result[3] += 0.049576783555018135;
            result[4] += 0.0592503022974607;
            result[5] += 0.8899637243047158;
          }
        } else {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)65) ) ) {
            result[0] += 0.638888888888889;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3194444444444445;
            result[5] += 0.04166666666666667;
          } else {
            result[0] += 0.003134796238244514;
            result[1] += 0.006269592476489028;
            result[2] += 0.04075235109717868;
            result[3] += 0.5297805642633229;
            result[4] += 0.034482758620689655;
            result[5] += 0.38557993730407525;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)77) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9973118279569892;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.002688172043010753;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)67.5) ) ) {
            result[0] += 0.08974358974358974;
            result[1] += 0.2692307692307692;
            result[2] += 0.00641025641025641;
            result[3] += 0;
            result[4] += 0.6282051282051282;
            result[5] += 0.00641025641025641;
          } else {
            result[0] += 0.8833481745325023;
            result[1] += 0.016918967052537846;
            result[2] += 0.004452359750667854;
            result[3] += 0.024933214603739984;
            result[4] += 0.048975957257346395;
            result[5] += 0.0213713268032057;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89.5) ) ) {
            result[0] += 0.011764705882352941;
            result[1] += 0.0058823529411764705;
            result[2] += 0.26176470588235295;
            result[3] += 0.37058823529411766;
            result[4] += 0.026470588235294117;
            result[5] += 0.3235294117647059;
          } else {
            result[0] += 0.7722772277227723;
            result[1] += 0.009900990099009901;
            result[2] += 0.09900990099009901;
            result[3] += 0;
            result[4] += 0.1188118811881188;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)82.5) ) ) {
      if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)103) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3333333333333333;
            result[4] += 0.030303030303030304;
            result[5] += 0.6363636363636364;
          } else {
            result[0] += 0.42857142857142855;
            result[1] += 0;
            result[2] += 0.42857142857142855;
            result[3] += 0.14285714285714285;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)83) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.16129032258064518;
            result[3] += 0.8064516129032259;
            result[4] += 0;
            result[5] += 0.03225806451612904;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7;
            result[3] += 0.3;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)77) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.14285714285714285;
            result[4] += 0;
            result[5] += 0.8571428571428571;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.75;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.25;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)96.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4;
            result[3] += 0.5;
            result[4] += 0.1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.06451612903225806;
            result[2] += 0.8548387096774194;
            result[3] += 0.03225806451612903;
            result[4] += 0.016129032258064516;
            result[5] += 0.03225806451612903;
          }
        }
      }
    } else {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)98) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)83) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.21052631578947367;
            result[3] += 0.7368421052631579;
            result[4] += 0;
            result[5] += 0.05263157894736842;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9444444444444444;
            result[3] += 0.05555555555555555;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.375;
            result[3] += 0.125;
            result[4] += 0;
            result[5] += 0.5;
          }
        }
      } else {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)85.5) ) ) {
            result[0] += 0.0660377358490566;
            result[1] += 0;
            result[2] += 0.6886792452830188;
            result[3] += 0.20754716981132076;
            result[4] += 0;
            result[5] += 0.03773584905660377;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9155844155844156;
            result[3] += 0.08441558441558442;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)74.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.3333333333333333;
          } else {
            result[0] += 0.0016339869281045752;
            result[1] += 0;
            result[2] += 0.9787581699346405;
            result[3] += 0.017973856209150325;
            result[4] += 0;
            result[5] += 0.0016339869281045752;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)81) ) ) {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)88) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)45) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.004672897196261683;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9532710280373833;
            result[5] += 0.042056074766355145;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9047619047619048;
            result[5] += 0.09523809523809523;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.046176046176046176;
            result[4] += 0.027417027417027416;
            result[5] += 0.9264069264069265;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
            result[0] += 0.736842105263158;
            result[1] += 0;
            result[2] += 0.01754385964912281;
            result[3] += 0;
            result[4] += 0.2456140350877193;
            result[5] += 0;
          } else {
            result[0] += 0.004405286343612335;
            result[1] += 0.0022026431718061676;
            result[2] += 0.013215859030837005;
            result[3] += 0.36784140969163;
            result[4] += 0.06387665198237885;
            result[5] += 0.5484581497797357;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)56.5) ) ) {
        if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)108) ) ) {
            result[0] += 0;
            result[1] += 0.15789473684210525;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8421052631578947;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)93.5) ) ) {
            result[0] += 0;
            result[1] += 0.7692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.23076923076923078;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)68.5) ) ) {
            result[0] += 0.11965811965811966;
            result[1] += 0.07692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7863247863247863;
            result[5] += 0.017094017094017096;
          } else {
            result[0] += 0.8330508474576273;
            result[1] += 0.01779661016949153;
            result[2] += 0.012711864406779664;
            result[3] += 0.03559322033898306;
            result[4] += 0.05254237288135594;
            result[5] += 0.04830508474576272;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)111.5) ) ) {
            result[0] += 0.018691588785046728;
            result[1] += 0.04361370716510903;
            result[2] += 0.3146417445482866;
            result[3] += 0.3364485981308411;
            result[4] += 0.024922118380062305;
            result[5] += 0.2616822429906542;
          } else {
            result[0] += 0.7040816326530612;
            result[1] += 0;
            result[2] += 0.20408163265306123;
            result[3] += 0.02040816326530612;
            result[4] += 0.07142857142857142;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
      if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)91.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.6666666666666666;
          }
        } else {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)68.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0.044444444444444446;
            result[4] += 0.044444444444444446;
            result[5] += 0.8;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)80.5) ) ) {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0.03225806451612903;
            result[2] += 0.06451612903225806;
            result[3] += 0.7580645161290323;
            result[4] += 0.016129032258064516;
            result[5] += 0.12903225806451613;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7222222222222223;
            result[3] += 0.11111111111111112;
            result[4] += 0;
            result[5] += 0.16666666666666669;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.711864406779661;
            result[3] += 0.2542372881355932;
            result[4] += 0;
            result[5] += 0.03389830508474576;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)104.5) ) ) {
        if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)116) ) ) {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)65.5) ) ) {
            result[0] += 0.6666666666666666;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7700534759358288;
            result[3] += 0.1925133689839572;
            result[4] += 0;
            result[5] += 0.0374331550802139;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)82) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)75.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)69.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9678714859437751;
            result[3] += 0.028112449799196786;
            result[4] += 0;
            result[5] += 0.004016064257028112;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)62.5) ) ) {
      if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)91.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)48.5) ) ) {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.0064516129032258064;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.964516129032258;
            result[5] += 0.02903225806451613;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)48) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.96;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.04;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)56.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0.3333333333333333;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9926108374384236;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.007389162561576354;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)109) ) ) {
            result[0] += 0;
            result[1] += 0.3076923076923077;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6923076923076923;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)86) ) ) {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)56.5) ) ) {
            result[0] += 0.9736842105263158;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.02631578947368421;
            result[5] += 0;
          } else {
            result[0] += 0.0019474196689386564;
            result[1] += 0.016553067185978577;
            result[2] += 0;
            result[3] += 0.07497565725413827;
            result[4] += 0.04673807205452775;
            result[5] += 0.8597857838364168;
          }
        } else {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.018518518518518517;
            result[3] += 0.24074074074074073;
            result[4] += 0.07407407407407407;
            result[5] += 0.6666666666666666;
          } else {
            result[0] += 0.0625;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7678571428571429;
            result[4] += 0.026785714285714284;
            result[5] += 0.14285714285714285;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)69.5) ) ) {
            result[0] += 0.05405405405405406;
            result[1] += 0.06756756756756757;
            result[2] += 0;
            result[3] += 0.13513513513513514;
            result[4] += 0.33783783783783783;
            result[5] += 0.40540540540540543;
          } else {
            result[0] += 0.860062893081761;
            result[1] += 0.009433962264150943;
            result[2] += 0.01729559748427673;
            result[3] += 0.02279874213836478;
            result[4] += 0.06682389937106918;
            result[5] += 0.02358490566037736;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
            result[0] += 0.004901960784313725;
            result[1] += 0.058823529411764705;
            result[2] += 0.06372549019607843;
            result[3] += 0.28431372549019607;
            result[4] += 0.14705882352941177;
            result[5] += 0.4411764705882353;
          } else {
            result[0] += 0.5608108108108109;
            result[1] += 0;
            result[2] += 0.31756756756756754;
            result[3] += 0.033783783783783786;
            result[4] += 0.033783783783783786;
            result[5] += 0.05405405405405406;
          }
        }
      }
    }
  } else {
    if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)101.5) ) ) {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)91.5) ) ) {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)63.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)45.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)85.5) ) ) {
            result[0] += 0.05555555555555555;
            result[1] += 0;
            result[2] += 0.027777777777777776;
            result[3] += 0.05555555555555555;
            result[4] += 0;
            result[5] += 0.8611111111111112;
          } else {
            result[0] += 0;
            result[1] += 0.024390243902439025;
            result[2] += 0.07317073170731707;
            result[3] += 0.6707317073170732;
            result[4] += 0.036585365853658534;
            result[5] += 0.1951219512195122;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04545454545454546;
            result[3] += 0.9090909090909092;
            result[4] += 0;
            result[5] += 0.04545454545454546;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6538461538461539;
            result[3] += 0.3076923076923077;
            result[4] += 0;
            result[5] += 0.038461538461538464;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6052631578947368;
            result[3] += 0.3157894736842105;
            result[4] += 0;
            result[5] += 0.07894736842105263;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9615384615384616;
            result[3] += 0.038461538461538464;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)79.5) ) ) {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)114.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)111.5) ) ) {
            result[0] += 0.04545454545454546;
            result[1] += 0;
            result[2] += 0.7954545454545455;
            result[3] += 0.15909090909090912;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.4375;
            result[1] += 0;
            result[2] += 0.5625;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)60) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)99.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8041958041958042;
            result[3] += 0.14685314685314685;
            result[4] += 0;
            result[5] += 0.04895104895104895;
          } else {
            result[0] += 0.0030581039755351682;
            result[1] += 0;
            result[2] += 0.9755351681957186;
            result[3] += 0.021406727828746176;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)70.5) ) ) {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)79.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)102) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.921161825726141;
            result[5] += 0.07883817427385892;
          } else {
            result[0] += 0;
            result[1] += 0.8;
            result[2] += 0;
            result[3] += 0.13333333333333333;
            result[4] += 0.06666666666666667;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)55) ) ) {
            result[0] += 0;
            result[1] += 0.9230769230769231;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.07692307692307693;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3333333333333333;
            result[5] += 0.6666666666666666;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)71) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.6842105263157895;
            result[5] += 0.3157894736842105;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
            result[0] += 0.008739076154806492;
            result[1] += 0;
            result[2] += 0.003745318352059925;
            result[3] += 0.03495630461922597;
            result[4] += 0.03245942571785269;
            result[5] += 0.920099875156055;
          } else {
            result[0] += 0.038461538461538464;
            result[1] += 0;
            result[2] += 0.019230769230769232;
            result[3] += 0.3605769230769231;
            result[4] += 0.038461538461538464;
            result[5] += 0.5432692307692307;
          }
        }
      }
    } else {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.08333333333333333;
            result[4] += 0.9166666666666666;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.7;
            result[2] += 0;
            result[3] += 0.1;
            result[4] += 0.2;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)55.5) ) ) {
            result[0] += 0;
            result[1] += 0.9974619289340102;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0025380710659898475;
            result[5] += 0;
          } else {
            result[0] += 0.13725490196078435;
            result[1] += 0.7647058823529412;
            result[2] += 0;
            result[3] += 0.03921568627450981;
            result[4] += 0.05882352941176471;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)73) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)96.5) ) ) {
            result[0] += 0.5135135135135135;
            result[1] += 0.014553014553014554;
            result[2] += 0.010395010395010396;
            result[3] += 0.09147609147609148;
            result[4] += 0.23076923076923078;
            result[5] += 0.1392931392931393;
          } else {
            result[0] += 0.8819068255687974;
            result[1] += 0.041170097508125676;
            result[2] += 0.005417118093174431;
            result[3] += 0.009750812567713976;
            result[4] += 0.06175514626218852;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)87.5) ) ) {
            result[0] += 0.008908685968819599;
            result[1] += 0.0334075723830735;
            result[2] += 0.155902004454343;
            result[3] += 0.3674832962138085;
            result[4] += 0.0378619153674833;
            result[5] += 0.39643652561247217;
          } else {
            result[0] += 0.6964285714285715;
            result[1] += 0.07142857142857144;
            result[2] += 0.10714285714285715;
            result[3] += 0.00892857142857143;
            result[4] += 0.11607142857142859;
            result[5] += 0;
          }
        }
      }
    }
  } else {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)80.5) ) ) {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0.8333333333333334;
          }
        } else {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.12962962962962962;
            result[3] += 0.7037037037037037;
            result[4] += 0;
            result[5] += 0.16666666666666666;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)75.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 1;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)111.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.92;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.08;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.25;
            result[3] += 0.25;
            result[4] += 0;
            result[5] += 0.5;
          }
        }
      }
    } else {
      if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)87.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)87.5) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)75.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2222222222222222;
            result[3] += 0.2222222222222222;
            result[4] += 0;
            result[5] += 0.5555555555555556;
          } else {
            result[0] += 0.028;
            result[1] += 0;
            result[2] += 0.74;
            result[3] += 0.224;
            result[4] += 0;
            result[5] += 0.008;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9353233830845772;
            result[3] += 0.05472636815920399;
            result[4] += 0;
            result[5] += 0.009950248756218907;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)89.5) ) ) {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8695652173913043;
            result[3] += 0.13043478260869565;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    }
  }
  if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)94.5) ) ) {
    if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)97.5) ) ) {
      if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)47) ) ) {
            result[0] += 0;
            result[1] += 0.8571428571428572;
            result[2] += 0;
            result[3] += 0.0816326530612245;
            result[4] += 0.06122448979591837;
            result[5] += 0;
          } else {
            result[0] += 0.01875;
            result[1] += 0.03125;
            result[2] += 0;
            result[3] += 0.01875;
            result[4] += 0.89375;
            result[5] += 0.0375;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)69) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.19230769230769232;
            result[3] += 0;
            result[4] += 0.21153846153846154;
            result[5] += 0.5961538461538461;
          } else {
            result[0] += 0.9276807980049875;
            result[1] += 0.012468827930174564;
            result[2] += 0.004987531172069825;
            result[3] += 0.017456359102244388;
            result[4] += 0.0199501246882793;
            result[5] += 0.017456359102244388;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)85.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0.0011061946902654867;
            result[1] += 0.0011061946902654867;
            result[2] += 0.00331858407079646;
            result[3] += 0.07411504424778761;
            result[4] += 0.049778761061946904;
            result[5] += 0.870575221238938;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)80.5) ) ) {
            result[0] += 0.01877133105802048;
            result[1] += 0.042662116040955635;
            result[2] += 0.008532423208191127;
            result[3] += 0.4402730375426621;
            result[4] += 0.09726962457337884;
            result[5] += 0.3924914675767918;
          } else {
            result[0] += 0.45320197044334976;
            result[1] += 0.029556650246305417;
            result[2] += 0.07389162561576355;
            result[3] += 0.09852216748768473;
            result[4] += 0.15270935960591134;
            result[5] += 0.1921182266009852;
          }
        }
      }
    } else {
      if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)102.5) ) ) {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)54.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)66.5) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9929245283018868;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.007075471698113208;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0.9375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.0625;
            result[5] += 0;
          } else {
            result[0] += 0.5;
            result[1] += 0.375;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.125;
            result[5] += 0;
          }
        }
      } else {
        result[0] += 1;
        result[1] += 0;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    }
  } else {
    if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)75.5) ) ) {
      if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)82.5) ) ) {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)77) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)79) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
            result[0] += 0.9812206572769953;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.001564945226917058;
            result[4] += 0.017214397496087636;
            result[5] += 0;
          } else {
            result[0] += 0.46153846153846156;
            result[1] += 0;
            result[2] += 0.15384615384615385;
            result[3] += 0.3076923076923077;
            result[4] += 0.07692307692307693;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)116) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)104.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0.058823529411764705;
            result[1] += 0;
            result[2] += 0.9411764705882353;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)82) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)101) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2;
            result[3] += 0.7142857142857143;
            result[4] += 0;
            result[5] += 0.08571428571428572;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.4827586206896552;
            result[3] += 0.1724137931034483;
            result[4] += 0;
            result[5] += 0.3448275862068966;
          }
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)108.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6171875;
            result[3] += 0.3125;
            result[4] += 0;
            result[5] += 0.0703125;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9347826086956522;
            result[3] += 0.06521739130434782;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)77) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)78) ) ) {
            result[0] += 0.8125;
            result[1] += 0;
            result[2] += 0.0625;
            result[3] += 0;
            result[4] += 0.125;
            result[5] += 0;
          } else {
            result[0] += 0.07894736842105263;
            result[1] += 0;
            result[2] += 0.5263157894736842;
            result[3] += 0.18421052631578946;
            result[4] += 0;
            result[5] += 0.21052631578947367;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)77) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6315789473684211;
            result[3] += 0.052631578947368425;
            result[4] += 0;
            result[5] += 0.31578947368421056;
          } else {
            result[0] += 0.007453416149068323;
            result[1] += 0;
            result[2] += 0.9453416149068323;
            result[3] += 0.043478260869565216;
            result[4] += 0;
            result[5] += 0.0037267080745341614;
          }
        }
      }
    }
  }
  if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)74.5) ) ) {
    if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)47.5) ) ) {
        if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)39.5) ) ) {
          if ( ( !(data[27].missing != -1) || (data[27].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 0;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 1;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)34) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.8333333333333334;
            result[5] += 0.16666666666666666;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)86.5) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)58) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.875;
            result[5] += 0.125;
          } else {
            result[0] += 0.9444444444444444;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.05555555555555555;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)59) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9166666666666666;
            result[5] += 0.08333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.0037688442211055275;
            result[3] += 0.031407035175879394;
            result[4] += 0.04522613065326633;
            result[5] += 0.9195979899497487;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)82) ) ) {
          if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)54.5) ) ) {
            result[0] += 0.8620689655172413;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.13793103448275862;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.04020100502512563;
            result[1] += 0.035175879396984924;
            result[2] += 0.07035175879396985;
            result[3] += 0.1457286432160804;
            result[4] += 0.16080402010050251;
            result[5] += 0.5477386934673367;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.06896551724137931;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.9310344827586207;
          } else {
            result[0] += 0.041509433962264156;
            result[1] += 0;
            result[2] += 0.11320754716981134;
            result[3] += 0.679245283018868;
            result[4] += 0.007547169811320756;
            result[5] += 0.15849056603773587;
          }
        }
      }
    }
  } else {
    if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)77) ) ) {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)90.5) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)44) ) ) {
            result[0] += 0;
            result[1] += 0.8888888888888888;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1111111111111111;
            result[5] += 0;
          } else {
            result[0] += 0.07462686567164178;
            result[1] += 0.1044776119402985;
            result[2] += 0.04477611940298507;
            result[3] += 0;
            result[4] += 0.7313432835820896;
            result[5] += 0.04477611940298507;
          }
        } else {
          if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)62) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.7692307692307693;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.23076923076923078;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)73) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)71.5) ) ) {
            result[0] += 0.13541666666666666;
            result[1] += 0.041666666666666664;
            result[2] += 0.03125;
            result[3] += 0.15625;
            result[4] += 0.052083333333333336;
            result[5] += 0.5833333333333334;
          } else {
            result[0] += 0.9000000000000001;
            result[1] += 0.00892857142857143;
            result[2] += 0.0008928571428571429;
            result[3] += 0.01160714285714286;
            result[4] += 0.0705357142857143;
            result[5] += 0.008035714285714287;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89) ) ) {
            result[0] += 0.011627906976744186;
            result[1] += 0.005813953488372093;
            result[2] += 0.21511627906976744;
            result[3] += 0.3953488372093023;
            result[4] += 0.023255813953488372;
            result[5] += 0.3488372093023256;
          } else {
            result[0] += 0.7777777777777778;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0;
            result[4] += 0.1111111111111111;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)88) ) ) {
          if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)86) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.4;
            result[4] += 0.2;
            result[5] += 0.4;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
            result[0] += 0.08695652173913043;
            result[1] += 0.043478260869565216;
            result[2] += 0.13043478260869565;
            result[3] += 0.08695652173913043;
            result[4] += 0.17391304347826086;
            result[5] += 0.4782608695652174;
          } else {
            result[0] += 0.06153846153846154;
            result[1] += 0;
            result[2] += 0.16923076923076924;
            result[3] += 0.7384615384615385;
            result[4] += 0;
            result[5] += 0.03076923076923077;
          }
        }
      } else {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0.5652173913043478;
            result[2] += 0.08695652173913043;
            result[3] += 0;
            result[4] += 0.34782608695652173;
            result[5] += 0;
          } else {
            result[0] += 0.05194805194805195;
            result[1] += 0.006493506493506494;
            result[2] += 0.5714285714285714;
            result[3] += 0.2012987012987013;
            result[4] += 0.012987012987012988;
            result[5] += 0.15584415584415584;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)71.5) ) ) {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0.4166666666666667;
            result[3] += 0.08333333333333333;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0034403669724770644;
            result[1] += 0;
            result[2] += 0.9243119266055045;
            result[3] += 0.06995412844036697;
            result[4] += 0;
            result[5] += 0.0022935779816513763;
          }
        }
      }
    }
  }
  if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
    if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)61.5) ) ) {
      if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)88) ) ) {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)42.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.005025125628140704;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9849246231155779;
            result[5] += 0.010050251256281407;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)89) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.72;
            result[5] += 0.28;
          } else {
            result[0] += 0;
            result[1] += 0.6666666666666666;
            result[2] += 0;
            result[3] += 0.3333333333333333;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        result[0] += 0;
        result[1] += 1;
        result[2] += 0;
        result[3] += 0;
        result[4] += 0;
        result[5] += 0;
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)59.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)62) ) ) {
            result[0] += 0.020000000000000004;
            result[1] += 0;
            result[2] += 0.04000000000000001;
            result[3] += 0;
            result[4] += 0.8600000000000001;
            result[5] += 0.08000000000000002;
          } else {
            result[0] += 0.6122448979591837;
            result[1] += 0;
            result[2] += 0.02040816326530612;
            result[3] += 0;
            result[4] += 0.12244897959183673;
            result[5] += 0.24489795918367346;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)65.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.023668639053254437;
            result[4] += 0.014792899408284023;
            result[5] += 0.9615384615384616;
          } else {
            result[0] += 0;
            result[1] += 0.0053475935828877;
            result[2] += 0.040106951871657755;
            result[3] += 0.24598930481283424;
            result[4] += 0.08288770053475936;
            result[5] += 0.6256684491978609;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.041666666666666664;
            result[4] += 0;
            result[5] += 0.9583333333333334;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.860655737704918;
            result[4] += 0;
            result[5] += 0.13934426229508196;
          } else {
            result[0] += 0.08695652173913043;
            result[1] += 0;
            result[2] += 0.30434782608695654;
            result[3] += 0.34782608695652173;
            result[4] += 0;
            result[5] += 0.2608695652173913;
          }
        }
      }
    }
  } else {
    if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)103.5) ) ) {
        if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)66.5) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)47) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.11971830985915494;
            result[1] += 0.14084507042253525;
            result[2] += 0.007042253521126762;
            result[3] += 0.021126760563380285;
            result[4] += 0.619718309859155;
            result[5] += 0.0915492957746479;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)74.5) ) ) {
            result[0] += 0.2317596566523605;
            result[1] += 0.008583690987124463;
            result[2] += 0.030042918454935622;
            result[3] += 0.2317596566523605;
            result[4] += 0.055793991416309016;
            result[5] += 0.44206008583690987;
          } else {
            result[0] += 0.8349593495934959;
            result[1] += 0.013821138211382113;
            result[2] += 0.016260162601626018;
            result[3] += 0.032520325203252036;
            result[4] += 0.08211382113821138;
            result[5] += 0.02032520325203252;
          }
        }
      } else {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)82) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)69) ) ) {
            result[0] += 0;
            result[1] += 0.5;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.5;
            result[5] += 0;
          } else {
            result[0] += 0.0025906735751295338;
            result[1] += 0.9974093264248705;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      }
    } else {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.05;
            result[3] += 0.7625;
            result[4] += 0;
            result[5] += 0.1875;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3478260869565218;
            result[3] += 0.28260869565217395;
            result[4] += 0;
            result[5] += 0.3695652173913044;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)80.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.2173913043478261;
            result[3] += 0.7391304347826088;
            result[4] += 0;
            result[5] += 0.04347826086956522;
          } else {
            result[0] += 0.02197802197802198;
            result[1] += 0;
            result[2] += 0.7912087912087912;
            result[3] += 0.10989010989010989;
            result[4] += 0.04395604395604396;
            result[5] += 0.03296703296703297;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)93.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)73.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.125;
            result[3] += 0.875;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)86.5) ) ) {
            result[0] += 0.0463768115942029;
            result[1] += 0;
            result[2] += 0.8115942028985508;
            result[3] += 0.12753623188405797;
            result[4] += 0;
            result[5] += 0.014492753623188406;
          } else {
            result[0] += 0.0032733224222585926;
            result[1] += 0;
            result[2] += 0.9656301145662848;
            result[3] += 0.02127659574468085;
            result[4] += 0;
            result[5] += 0.009819967266775777;
          }
        }
      }
    }
  }
  if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)81) ) ) {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)71.5) ) ) {
      if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)59.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)65) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)55.5) ) ) {
            result[0] += 0.8947368421052632;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.10526315789473684;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.11764705882352941;
            result[4] += 0.4117647058823529;
            result[5] += 0.47058823529411764;
          }
        }
      } else {
        if ( ( !(data[2].missing != -1) || (data[2].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03409090909090909;
            result[4] += 0.6477272727272727;
            result[5] += 0.3181818181818182;
          } else {
            result[0] += 0.0012195121951219512;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.03780487804878049;
            result[4] += 0.02195121951219512;
            result[5] += 0.9390243902439024;
          }
        } else {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)63.5) ) ) {
            result[0] += 0;
            result[1] += 0.048;
            result[2] += 0.008;
            result[3] += 0.12;
            result[4] += 0.16;
            result[5] += 0.664;
          } else {
            result[0] += 0.004545454545454545;
            result[1] += 0;
            result[2] += 0.013636363636363636;
            result[3] += 0.4681818181818182;
            result[4] += 0.05909090909090909;
            result[5] += 0.45454545454545453;
          }
        }
      }
    } else {
      if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)51.5) ) ) {
        if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)97) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9878048780487805;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.012195121951219513;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 0.7142857142857143;
            result[2] += 0.2857142857142857;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[1].missing != -1) || (data[1].fvalue <= (float)65) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)45.5) ) ) {
            result[0] += 0;
            result[1] += 0.8953488372093024;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.10465116279069768;
            result[5] += 0;
          } else {
            result[0] += 0.07086614173228346;
            result[1] += 0.047244094488188976;
            result[2] += 0;
            result[3] += 0.031496062992125984;
            result[4] += 0.8110236220472441;
            result[5] += 0.03937007874015748;
          }
        } else {
          if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)73) ) ) {
            result[0] += 0.8602999210734017;
            result[1] += 0.010260457774269928;
            result[2] += 0.0055248618784530384;
            result[3] += 0.03235990528808208;
            result[4] += 0.050513022888713496;
            result[5] += 0.04104183109707971;
          } else {
            result[0] += 0.135;
            result[1] += 0.0025;
            result[2] += 0.1875;
            result[3] += 0.3375;
            result[4] += 0.0625;
            result[5] += 0.275;
          }
        }
      }
    }
  } else {
    if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)96.5) ) ) {
        if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)67.5) ) ) {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)67.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          }
        } else {
          if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)85.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.13114754098360656;
            result[3] += 0.819672131147541;
            result[4] += 0;
            result[5] += 0.04918032786885246;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)95.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)81) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1111111111111111;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0.8888888888888888;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)103) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8888888888888888;
            result[3] += 0.1111111111111111;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5714285714285714;
            result[3] += 0.35714285714285715;
            result[4] += 0;
            result[5] += 0.07142857142857142;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)100.5) ) ) {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)82.5) ) ) {
          if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)78.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9117647058823529;
            result[3] += 0.08823529411764706;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)122.5) ) ) {
            result[0] += 0.006079027355623101;
            result[1] += 0;
            result[2] += 0.8571428571428572;
            result[3] += 0.10030395136778117;
            result[4] += 0;
            result[5] += 0.03647416413373861;
          } else {
            result[0] += 0.5;
            result[1] += 0;
            result[2] += 0.5;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[30].missing != -1) || (data[30].fvalue <= (float)87) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0018975332068311196;
            result[1] += 0;
            result[2] += 0.9734345351043643;
            result[3] += 0.024667931688804556;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[35].missing != -1) || (data[35].fvalue <= (float)73.5) ) ) {
      if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)61.5) ) ) {
        if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)60.5) ) ) {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)98.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9818181818181818;
            result[5] += 0.01818181818181818;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)45.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.39603960396039606;
            result[1] += 0.009900990099009901;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.36633663366336633;
            result[5] += 0.22772277227722773;
          }
        }
      } else {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)59.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.7857142857142857;
            result[5] += 0.21428571428571427;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.04257767548906789;
            result[4] += 0.04142692750287687;
            result[5] += 0.9159953970080552;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)73) ) ) {
            result[0] += 0.05128205128205128;
            result[1] += 0.06837606837606838;
            result[2] += 0;
            result[3] += 0.3076923076923077;
            result[4] += 0.05982905982905983;
            result[5] += 0.5128205128205128;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.7596153846153846;
            result[4] += 0;
            result[5] += 0.2403846153846154;
          }
        }
      }
    } else {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)63.5) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)93.5) ) ) {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)47.5) ) ) {
            result[0] += 0;
            result[1] += 0.625;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.375;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.01818181818181818;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9818181818181818;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[33].missing != -1) || (data[33].fvalue <= (float)53) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.6530612244897959;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.3469387755102041;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[3].missing != -1) || (data[3].fvalue <= (float)73.5) ) ) {
            result[0] += 0.22916666666666666;
            result[1] += 0.05555555555555555;
            result[2] += 0;
            result[3] += 0.05555555555555555;
            result[4] += 0.2152777777777778;
            result[5] += 0.4444444444444444;
          } else {
            result[0] += 0.9031954887218046;
            result[1] += 0.013157894736842105;
            result[2] += 0;
            result[3] += 0.012218045112781954;
            result[4] += 0.06296992481203008;
            result[5] += 0.008458646616541353;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)89.5) ) ) {
            result[0] += 0.012658227848101266;
            result[1] += 0;
            result[2] += 0.2320675105485232;
            result[3] += 0.3206751054852321;
            result[4] += 0.012658227848101266;
            result[5] += 0.4219409282700422;
          } else {
            result[0] += 0.7555555555555555;
            result[1] += 0;
            result[2] += 0.08888888888888889;
            result[3] += 0;
            result[4] += 0.14444444444444443;
            result[5] += 0.011111111111111112;
          }
        }
      }
    }
  } else {
    if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)80.5) ) ) {
      if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)84.5) ) ) {
        if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)72.5) ) ) {
          if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)68) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8;
            result[3] += 0.06666666666666667;
            result[4] += 0;
            result[5] += 0.13333333333333333;
          } else {
            result[0] += 0;
            result[1] += 0.45;
            result[2] += 0;
            result[3] += 0.1;
            result[4] += 0.25;
            result[5] += 0.2;
          }
        } else {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0.25;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.2222222222222222;
            result[4] += 0.037037037037037035;
            result[5] += 0.7407407407407407;
          }
        }
      } else {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)101.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)85) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.11538461538461539;
            result[3] += 0.7243589743589743;
            result[4] += 0;
            result[5] += 0.16025641025641027;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)72.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.23529411764705882;
            result[3] += 0.5882352941176471;
            result[4] += 0;
            result[5] += 0.17647058823529413;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.6666666666666666;
            result[3] += 0.2;
            result[4] += 0;
            result[5] += 0.13333333333333333;
          }
        }
      }
    } else {
      if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)103) ) ) {
        if ( ( !(data[32].missing != -1) || (data[32].fvalue <= (float)65) ) ) {
          result[0] += 1;
          result[1] += 0;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)83.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.35294117647058826;
            result[3] += 0.5;
            result[4] += 0.058823529411764705;
            result[5] += 0.08823529411764706;
          } else {
            result[0] += 0.04938271604938271;
            result[1] += 0;
            result[2] += 0.7777777777777778;
            result[3] += 0.1728395061728395;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)86.5) ) ) {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)95) ) ) {
            result[0] += 0.027303754266211604;
            result[1] += 0.013651877133105802;
            result[2] += 0.8464163822525598;
            result[3] += 0.10921501706484642;
            result[4] += 0;
            result[5] += 0.0034129692832764505;
          } else {
            result[0] += 0.7586206896551724;
            result[1] += 0;
            result[2] += 0.2413793103448276;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[28].missing != -1) || (data[28].fvalue <= (float)77.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.0019193857965451055;
            result[1] += 0;
            result[2] += 0.982725527831094;
            result[3] += 0.015355086372360844;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[24].missing != -1) || (data[24].fvalue <= (float)79.5) ) ) {
    if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)72.5) ) ) {
      if ( ( !(data[29].missing != -1) || (data[29].fvalue <= (float)65) ) ) {
        if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)103) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)65) ) ) {
            result[0] += 0;
            result[1] += 0.018726591760299626;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.9550561797752809;
            result[5] += 0.026217228464419477;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.41025641025641024;
            result[5] += 0.5897435897435898;
          }
        } else {
          if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)56.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.8846153846153846;
            result[2] += 0;
            result[3] += 0.11538461538461539;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)83.5) ) ) {
          if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)57.5) ) ) {
            result[0] += 0.6140350877192983;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.017543859649122806;
            result[4] += 0.2982456140350877;
            result[5] += 0.07017543859649122;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.050647820965842166;
            result[4] += 0.04122497055359246;
            result[5] += 0.9081272084805654;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)82) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.28888888888888886;
            result[4] += 0;
            result[5] += 0.7111111111111111;
          } else {
            result[0] += 0.014492753623188406;
            result[1] += 0;
            result[2] += 0.028985507246376812;
            result[3] += 0.7608695652173914;
            result[4] += 0;
            result[5] += 0.1956521739130435;
          }
        }
      }
    } else {
      if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)53.5) ) ) {
        if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)88) ) ) {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)122.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)87.5) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0.9;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.1;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[12].missing != -1) || (data[12].fvalue <= (float)73) ) ) {
          if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)69.5) ) ) {
            result[0] += 0.08391608391608393;
            result[1] += 0.18881118881118883;
            result[2] += 0;
            result[3] += 0.034965034965034975;
            result[4] += 0.6223776223776225;
            result[5] += 0.06993006993006995;
          } else {
            result[0] += 0.8276740237691002;
            result[1] += 0.008488964346349746;
            result[2] += 0.00933786078098472;
            result[3] += 0.03395585738539898;
            result[4] += 0.053480475382003394;
            result[5] += 0.06706281833616298;
          }
        } else {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)84) ) ) {
            result[0] += 0.017467248908296946;
            result[1] += 0.030567685589519653;
            result[2] += 0.15720524017467252;
            result[3] += 0.40174672489082974;
            result[4] += 0.06986899563318778;
            result[5] += 0.3231441048034935;
          } else {
            result[0] += 0.4551282051282051;
            result[1] += 0.01282051282051282;
            result[2] += 0.3974358974358974;
            result[3] += 0.03205128205128205;
            result[4] += 0.07692307692307693;
            result[5] += 0.02564102564102564;
          }
        }
      }
    }
  } else {
    if ( ( !(data[10].missing != -1) || (data[10].fvalue <= (float)99) ) ) {
      if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)95.5) ) ) {
        if ( ( !(data[8].missing != -1) || (data[8].fvalue <= (float)61.5) ) ) {
          if ( ( !(data[16].missing != -1) || (data[16].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.9230769230769231;
            result[3] += 0;
            result[4] += 0.07692307692307693;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[34].missing != -1) || (data[34].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0.9523809523809523;
          } else {
            result[0] += 0;
            result[1] += 0.021052631578947368;
            result[2] += 0.10526315789473684;
            result[3] += 0.4;
            result[4] += 0.021052631578947368;
            result[5] += 0.45263157894736844;
          }
        }
      } else {
        if ( ( !(data[22].missing != -1) || (data[22].fvalue <= (float)96.5) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)70.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0.16666666666666666;
            result[4] += 0;
            result[5] += 0.5;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 1;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[14].missing != -1) || (data[14].fvalue <= (float)121) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8541666666666666;
            result[3] += 0.14583333333333334;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.6666666666666666;
            result[1] += 0;
            result[2] += 0.3333333333333333;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    } else {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)95.5) ) ) {
        if ( ( !(data[7].missing != -1) || (data[7].fvalue <= (float)99.5) ) ) {
          if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)79.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.28205128205128205;
            result[3] += 0.48717948717948717;
            result[4] += 0.02564102564102564;
            result[5] += 0.20512820512820512;
          } else {
            result[0] += 0.10869565217391304;
            result[1] += 0.021739130434782608;
            result[2] += 0.7608695652173914;
            result[3] += 0.043478260869565216;
            result[4] += 0;
            result[5] += 0.06521739130434782;
          }
        } else {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
          if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)89.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 1;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8277511961722488;
            result[3] += 0.14354066985645933;
            result[4] += 0;
            result[5] += 0.028708133971291867;
          }
        } else {
          if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)91.5) ) ) {
            result[0] += 1;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.002861230329041488;
            result[1] += 0;
            result[2] += 0.9670958512160229;
            result[3] += 0.030042918454935622;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  if ( ( !(data[4].missing != -1) || (data[4].fvalue <= (float)78.5) ) ) {
    if ( ( !(data[6].missing != -1) || (data[6].fvalue <= (float)91.5) ) ) {
      if ( ( !(data[5].missing != -1) || (data[5].fvalue <= (float)62.5) ) ) {
        if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)42.5) ) ) {
          result[0] += 0;
          result[1] += 1;
          result[2] += 0;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)99.5) ) ) {
            result[0] += 0.0033783783783783786;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.013513513513513514;
            result[4] += 0.9425675675675675;
            result[5] += 0.04054054054054054;
          } else {
            result[0] += 0;
            result[1] += 0.75;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0.25;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)69.5) ) ) {
          if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)62.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.14285714285714285;
            result[3] += 0;
            result[4] += 0.6666666666666666;
            result[5] += 0.19047619047619047;
          } else {
            result[0] += 0.0010741138560687433;
            result[1] += 0;
            result[2] += 0.0010741138560687433;
            result[3] += 0.06874328678839957;
            result[4] += 0.03329752953813104;
            result[5] += 0.8958109559613319;
          }
        } else {
          if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)66.5) ) ) {
            result[0] += 0.6548223350253807;
            result[1] += 0.05583756345177665;
            result[2] += 0.01015228426395939;
            result[3] += 0.005076142131979695;
            result[4] += 0.25888324873096447;
            result[5] += 0.015228426395939087;
          } else {
            result[0] += 0.017391304347826087;
            result[1] += 0;
            result[2] += 0.07391304347826087;
            result[3] += 0.34782608695652173;
            result[4] += 0.07391304347826087;
            result[5] += 0.48695652173913045;
          }
        }
      }
    } else {
      if ( ( !(data[9].missing != -1) || (data[9].fvalue <= (float)58.5) ) ) {
        if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)93) ) ) {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.3333333333333333;
            result[4] += 0.6666666666666666;
            result[5] += 0;
          } else {
            result[0] += 0.05357142857142857;
            result[1] += 0.6607142857142857;
            result[2] += 0;
            result[3] += 0.14285714285714285;
            result[4] += 0.14285714285714285;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[0].missing != -1) || (data[0].fvalue <= (float)58) ) ) {
            result[0] += 0;
            result[1] += 1;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.1724137931034483;
            result[1] += 0.8275862068965517;
            result[2] += 0;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)71.5) ) ) {
          if ( ( !(data[31].missing != -1) || (data[31].fvalue <= (float)69.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.4090909090909091;
            result[4] += 0.045454545454545456;
            result[5] += 0.5454545454545454;
          } else {
            result[0] += 0.8939096267190569;
            result[1] += 0.030451866404715127;
            result[2] += 0;
            result[3] += 0.01768172888015717;
            result[4] += 0.05108055009823183;
            result[5] += 0.0068762278978389;
          }
        } else {
          if ( ( !(data[11].missing != -1) || (data[11].fvalue <= (float)89) ) ) {
            result[0] += 0.02358490566037736;
            result[1] += 0.02358490566037736;
            result[2] += 0.16037735849056603;
            result[3] += 0.5047169811320755;
            result[4] += 0.03773584905660377;
            result[5] += 0.25;
          } else {
            result[0] += 0.8181818181818182;
            result[1] += 0;
            result[2] += 0.030303030303030304;
            result[3] += 0.045454545454545456;
            result[4] += 0.07575757575757576;
            result[5] += 0.030303030303030304;
          }
        }
      }
    }
  } else {
    if ( ( !(data[21].missing != -1) || (data[21].fvalue <= (float)96.5) ) ) {
      if ( ( !(data[19].missing != -1) || (data[19].fvalue <= (float)78.5) ) ) {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)67.5) ) ) {
          if ( ( !(data[26].missing != -1) || (data[26].fvalue <= (float)71.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0;
            result[4] += 1;
            result[5] += 0;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.04878048780487805;
            result[3] += 0.04878048780487805;
            result[4] += 0.024390243902439025;
            result[5] += 0.8780487804878049;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)88.5) ) ) {
            result[0] += 0;
            result[1] += 0.03508771929824561;
            result[2] += 0.05263157894736842;
            result[3] += 0.3508771929824561;
            result[4] += 0;
            result[5] += 0.5614035087719298;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.1728395061728395;
            result[3] += 0.654320987654321;
            result[4] += 0;
            result[5] += 0.1728395061728395;
          }
        }
      } else {
        if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)78.5) ) ) {
          if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)100.5) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0;
            result[3] += 0.9642857142857143;
            result[4] += 0;
            result[5] += 0.03571428571428571;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.5636363636363636;
            result[3] += 0.18181818181818182;
            result[4] += 0.07272727272727272;
            result[5] += 0.18181818181818182;
          }
        } else {
          if ( ( !(data[25].missing != -1) || (data[25].fvalue <= (float)91.5) ) ) {
            result[0] += 0;
            result[1] += 0.15789473684210525;
            result[2] += 0.10526315789473684;
            result[3] += 0.3684210526315789;
            result[4] += 0.3157894736842105;
            result[5] += 0.05263157894736842;
          } else {
            result[0] += 0.02702702702702703;
            result[1] += 0.013513513513513514;
            result[2] += 0.8783783783783784;
            result[3] += 0.04054054054054054;
            result[4] += 0;
            result[5] += 0.04054054054054054;
          }
        }
      }
    } else {
      if ( ( !(data[20].missing != -1) || (data[20].fvalue <= (float)77) ) ) {
        if ( ( !(data[18].missing != -1) || (data[18].fvalue <= (float)111.5) ) ) {
          result[0] += 0;
          result[1] += 0;
          result[2] += 1;
          result[3] += 0;
          result[4] += 0;
          result[5] += 0;
        } else {
          if ( ( !(data[15].missing != -1) || (data[15].fvalue <= (float)89) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 1;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          } else {
            result[0] += 0.9642857142857143;
            result[1] += 0;
            result[2] += 0.03571428571428571;
            result[3] += 0;
            result[4] += 0;
            result[5] += 0;
          }
        }
      } else {
        if ( ( !(data[13].missing != -1) || (data[13].fvalue <= (float)93) ) ) {
          if ( ( !(data[17].missing != -1) || (data[17].fvalue <= (float)98) ) ) {
            result[0] += 0;
            result[1] += 0.07142857142857142;
            result[2] += 0.19047619047619047;
            result[3] += 0.047619047619047616;
            result[4] += 0;
            result[5] += 0.6904761904761905;
          } else {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.8636363636363636;
            result[3] += 0.13636363636363635;
            result[4] += 0;
            result[5] += 0;
          }
        } else {
          if ( ( !(data[23].missing != -1) || (data[23].fvalue <= (float)84) ) ) {
            result[0] += 0;
            result[1] += 0;
            result[2] += 0.7929292929292929;
            result[3] += 0.19696969696969696;
            result[4] += 0.005050505050505051;
            result[5] += 0.005050505050505051;
          } else {
            result[0] += 0.004672897196261683;
            result[1] += 0;
            result[2] += 0.9688473520249222;
            result[3] += 0.026479750778816202;
            result[4] += 0;
            result[5] += 0;
          }
        }
      }
    }
  }
  
  // Average tree outputs
  result[0] /= 100;
  result[1] /= 100;
  result[2] /= 100;
  result[3] /= 100;
  result[4] /= 100;
  result[5] /= 100;
  
  // Apply base_scores
  result[0] += 0;
  result[1] += 0;
  result[2] += 0;
  result[3] += 0;
  result[4] += 0;
  result[5] += 0;
  
  // Apply postprocessor
  if (!pred_margin) { postprocess(result); }
}

void postprocess(float* result) {
  // Do nothing
}


int main() {
    float result[MAX_N_CLASS];
    union Entry input[TEST_DATA_COLS];
    char line[1024];
    

    FILE* file = fopen("./codegen/dataset_146/split_2/test_data.csv", "r");
    if (file == NULL) {
        printf("Error opening file\n");
        return 1;
    }

    while (fgets(line, sizeof(line), file)) {
        char *ptr = line;
        for (int i = 0; i < TEST_DATA_COLS; i++) {
            sscanf(ptr, "%f", &(input[i].fvalue));
            input[i].missing = -1;
            while (*ptr != ',' && *ptr != '\n' && *ptr != '\0') ptr++;  // Skip to next comma
            if (*ptr == ',') ptr++;  // Move past the comma
        }
        predict(input, 0, result);
        
    }
    

    return 0;
}
